...
class VI_WOO_CART_ALL_IN_ONE_Frontend_Menu_Cart {
	...
	public function create_menu_cart( $items, $args ) {
		...
		if ( ! $is_wcaio_menu ) {
			...
			$nav_title          = $mc_nav_page === 'cart' ? __( 'View your shopping cart', 'woo-cart-all-in-one' ) : __( 'Quick checkout', 'woo-cart-all-in-one' );
			$cart_content_count = is_admin() ? 0 : $wc_cart->get_cart_contents_count();
			if ( $mc_cart_total === 'total' ) {
				$cart_total = is_admin() ? wc_price( 0 ) : $wc_cart->get_total();
			} else {
				$cart_total = is_admin() ? wc_price( 0 ) : $wc_cart->get_cart_subtotal();
			}
			$class   = array(
				'vi-wcaio-menu-cart',
			);
			...
			$class[] = !$mc_empty_enable && $wc_cart->is_empty() ? 'vi-wcaio-disabled' : '';
            ...
            <li class="<?php echo esc_attr( $class ); ?>" data-empty_enable="<?php echo esc_attr( $mc_empty_enable ?: '' ); ?>">
				...
				<?php
				if ( is_customize_preview() || ( $mc_content && ! wp_is_mobile() && ! is_cart() && ! is_checkout() ) ) {
					?>
