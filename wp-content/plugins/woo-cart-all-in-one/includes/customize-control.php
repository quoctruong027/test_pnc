			...
			protected function render_content() {
                ...
                <label>
					...
					if ( ! empty( $this->choices['button'] ) ) {
						echo sprintf( '<a class="button vi-wcaio-premium" href="https://1.envato.market/bW20B"  target="_blank" >%s</a>', __( 'Unlock This Feature', 'woo-cart-all-in-one' ));
					}
			...
			protected function render_content() {
                ...
                <label>
                    ...
                    <div class="vi-ui toggle checkbox vi-wcaio-customize-checkbox-wrap">
                        <input type="checkbox" id="<?php echo esc_attr( $this->id ); ?>" class="vi-wcaio-customize-checkbox" value="1"
                               name="<?php echo esc_attr( $this->id ); ?>" <?php $this->link();
						checked( $this->value(), 1 ) ?>><label></label>
                    </div>
                </label>
				<?php
			}
