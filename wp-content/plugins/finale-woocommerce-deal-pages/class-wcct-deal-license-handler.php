...
if ( ! class_exists( 'WCCT_Deal_License_Handler' ) ) :

	/**
	 * EDD_License Class
	 */
	class WCCT_Deal_License_Handler {
		...
		private function includes() {
			if ( ! class_exists( 'WCCT_Deal_Plugin_Updater' ) ) {
				require_once 'class-wcct-deal-plugin-updater.php';
			}
		}
		...
		private function auto_updater() {
			...
			// Setup the updater
			$edd_updater = new WCCT_Deal_Plugin_Updater( $this->api_url, $this->file, array(
		...
		public function activate_license( $license ) {
			...
			if ( class_exists( 'XL_admin_notifications' ) && $license_data->license == 'valid' ) {
				do_action( 'xl_license_activated', WCCT_DEAL_PAGE_PLUGIN_FILE );
