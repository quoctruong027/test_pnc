...
$template = apply_filters( 'ywdpd_table_orientation', YITH_WC_Dynamic_Pricing()->get_option( 'quantity_table_orientation' ) );
		...
		<tr class="quantity_row">
			...
			<?php foreach ( $rules as $rule ) : ?>
				<td class="qty-info" data-qtymin="<?php echo esc_attr( $rule['min_quantity'] ); ?>"
					...
					<?php
					$maxQ = '';
					$minQ = $rule['min_quantity'];
					if ( $rule['max_quantity'] !== $rule['min_quantity'] ) {
						$maxQ = ( '*' !== $rule['max_quantity'] ) ? '-' . $rule['max_quantity'] : '+' ;
					}
					
					echo esc_html( $rule['min_quantity'] . $maxQ ); 
					?>
