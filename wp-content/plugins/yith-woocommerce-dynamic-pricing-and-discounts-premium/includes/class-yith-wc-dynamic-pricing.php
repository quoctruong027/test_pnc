...
if ( ! class_exists( 'YITH_WC_Dynamic_Pricing' ) ) {
	...
	class YITH_WC_Dynamic_Pricing {
		 ...
		 * @return bool
		 */
		function check_discount( $_product ) {

			if ( apply_filters( 'ywdpd_exclude_products_from_discount', false, $_product ) ) {
				return false;
			}
			...
			foreach ( $this->validated_rules as $rule ){
				$show_in_loop = !empty( $rule['show_in_loop'] ) ? yith_plugin_fw_is_true( $rule['show_in_loop'] ) : false;

			   if ( $is_single_check && is_object( $product )  && !$show_in_loop && ! is_null( $woocommerce_loop ) && ! is_single( $product->get_id() ) ) {
			   	return false;
			   }
				if ( ( YITH_WC_Dynamic_Pricing_Helper()->valid_product_to_apply_bulk( $rule, $_product, true ) && YITH_WC_Dynamic_Pricing_Helper()->valid_product_to_adjustment_bulk( $rule, $_product, true ) ) && ( $is_single_check || ( isset( $rule['show_in_loop'] ) && ywdpd_is_true( $rule['show_in_loop'] ) && $rule['apply_adjustment'] == 'same_product' ) ) ) {
					$return = true;
				}
			}

			$this->check_discount[ $_product->get_id() ] = $return;
		...
		public function apply_discount( $cart_item, $cart_item_key, $reset = false ) {
			...
			if ( version_compare( WC()->version, '4.4.0', '<' ) ) {;
				...
				WC()->cart->cart_contents[ $cart_item_key ]['ywdpd_discounts']['default_price'] = ( WC()->cart->get_tax_price_display_mode() == 'excl' ) ? yit_get_price_excluding_tax( $product ) : yit_get_price_including_tax( $product );
			}
