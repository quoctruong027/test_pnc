	...
	class YITH_WC_Dynamic_Discounts {
		...
		function filter_valid_rules( $cart_rules ) {
			...
			foreach ( $cart_rules as $key => $cart_rule ) {
				...
				$schedule_from = isset(  $cart_rule['schedule_from'] ) ?  $cart_rule['schedule_from'] : '';
				$schedule_to = isset(  $cart_rule['schedule_to'] ) ?  $cart_rule['schedule_to'] : '';
				if ( $schedule_from != '' || $schedule_to != '' ) {
					if ( ! YITH_WC_Dynamic_Pricing_Helper()->validate_schedule( $schedule_from, $schedule_to ) ) {
						continue;
					}
				}
		...
		function apply_coupon_cart_discount() {
			...
			$discount_subtotal =$this->get_cart_subtotal();
			$coupon_type = 'fixed_cart';
			foreach ( $rules as $key => $rule) {
				$discount_subtotal -= $this->get_cart_subtotal_to_discount( $rule['rules'] );
			}
		...
		function get_discount_amount() {
			...
			if ( ! empty( $this->discount_rules ) ) {
				...
				foreach ( $this->discount_rules as $rule ) {
					...
					$new_discount = 0;
					$rules = empty( $rule['rules'] ) ? false : $rule['rules'];
					if( $rules ) {
						$subtotal = $this->get_cart_subtotal_to_discount( $rules );
