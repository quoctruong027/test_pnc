	...
	class YITH_WC_Dynamic_Pricing_Frontend {
				...
				if ( isset( $discount['status'] ) && 'applied' == $discount['status'] ) {
					if ( ( (float) $cart_item['ywdpd_discounts']['default_price'] > (float) $cart_item['data']->get_price() ) && wc_price( $cart_item['ywdpd_discounts']['default_price'] ) !== WC()->cart->get_product_price( $cart_item['data'] ) ) {
						...
						if ( isset( $cart_item['ywsbs-subscription-info'] ) ) {
							WC()->cart->cart_contents[ $cart_item_key ]['ywsbs-subscription-info']['recurring_price'] = $cart_item['data']->get_price();
						}
						break;
					} else {
						return $price;
		...
		public function get_price_html( $price, $product ) {
			...
			if ( $product->is_type( 'variable' ) ) {
				...
				$min_variation_regular_price_displayed = wc_get_price_to_display( $product, array(
					$price,
				...
				if ( $prices[0] == $prices[1] && $min_variation_regular_price_displayed == $prices[0] ) {
					...
					if ( $discount != $min_variation_regular_price || $discount_max != $max_variation_regular_price ) {
						...
						$new_price = str_replace( '%original_price%', '<span class="fix-break">'.$display_regular_price.'</span>', $new_price );
