...
$so_reasons                 = $vars['reasons']['default'];
$so_reasons_list_items_html = '';
$incr                    	= 0;

$sa_plugin_name 		 	= self::$plugin_name;
$plugin_data 				= SA_Smart_Offers::get_smart_offers_plugin_data();
$so_version 				= $plugin_data['Version'];

foreach ( $so_reasons as $reason ) {
	...
	$so_reasons_list_items_html .= '<li class="' . $list_item_classes . '" data-input-type="' . $sa_reason_input_type . '" data-input-placeholder="' . $sa_reason_input_placeholder . '"><label><span><input type="radio" name="selected-reason" value="' . $sa_reason_id . '" ' . $selected . '/></span><span>' . $sa_reason_text . '</span></label>' . $reason_html . '</li>';
	$incr ++;
}
...
<style type="text/css">
	...
	#the-list .deactivate > .sa-so-slug {
		display: none
	}
	.sa-modal li.reason-hide {
		display: none;
	}
	.sa-modal .sa-skip-deactivate-survey {
	    float: right;
	    font-size: 13px;
	    color: #ccc;
	...
	.sa-modal .sa-skip-deactivate-survey:hover {
		color: #00a0d2;
...
<script type="text/javascript">
	var SOCurrentPluginBaseName = "";
	var SOCustomReasons = {};
	var SODefaultReason = {};
	var SOFormName = 'SO Deactivation Reason';
	var SOPluginName = '<?php echo $sa_plugin_name; ?>';
	var SOPluginVersion = '<?php echo $so_version; ?>';
	( function ($) {
		var $deactivateLinks = {};
		var reasonsHtml = <?php echo json_encode( $so_reasons_list_items_html ); ?>,
			modalHtml =
				...
				+ '         <div class="sa-modal-panel active" data-panel-id="reasons"><h2 class="sa-modal-deactivation-headline"><?php printf( SO_Admin_Deactivation::load_str( 'deactivation-headline' ) ); ?></h2><hr><h3 class="sa-modal-deactivation-second-headline"><?php printf( SO_Admin_Deactivation::load_str( 'deactivation-share-reason' ) ); ?>:</h3><ul id="reasons-list">' + reasonsHtml + '</ul></div>'
				+ '     </div>'
				+ '     <div class="sa-modal-footer">'
				+ '         <a href="#" class="button button-primary button-large button-deactivate"><?php printf( SO_Admin_Deactivation::load_str( 'deactivation-modal-button-submit' ) ); ?></a>'
				+ '         <a href="#" class="sa-skip-deactivate-survey"><?php printf( SO_Admin_Deactivation::load_str( 'deactivation-modal-button-cancel' ) ); ?></a>'
				+ '     </div>'
				+ ' </div>'
				+ '</div>',
			$modal = $(modalHtml),

			$deactivateLink = $('#the-list .deactivate > .sa-so-slug').prev();

		for( var i = 0; i < $deactivateLink.length; i++ ) {
			$deactivateLinks[ $( $deactivateLink[i] ).siblings( ".sa-so-slug" ).attr( 'data-slug' ) ] = $deactivateLink[i].href;
		}
		...
		function registerEventHandlers() {
			...
			$deactivateLink.on( "click", function (evt) {
				evt.preventDefault();
				SOCurrentPluginBaseName = $(this).siblings( ".sa-so-slug" ).attr( 'data-slug' );
				showModal();
			});

			$modal.on( 'click', '.sa-skip-deactivate-survey', function (evt) {
				window.location.href = $deactivateLinks[SOCurrentPluginBaseName];
			});
				...
				if( _this.hasClass( 'allow-deactivate' ) ) {
					...
					if ( $radio.length == 1 ) {
						if( $modal.find('.sa-modal-footer .error').length == 0 ) {
							$modal.find('.sa-modal-footer').prepend('<span class="error"><?php printf( SO_Admin_Deactivation::load_str( 'deactivation-modal-error' ) ); ?></span>');
						}
						return;
					}
					...
					if( $radio.length == 0 ) {
						var data = {
							'action': 'so_submit_survey',
							'rm_lead_name': SOFormName,
							'sa_plugin_name': SOPluginName,
							'sa_plugin_version': SOPluginVersion,
							'reason_id': 0,
			...
			$modal.on('click', 'input[type="radio"]', function () {
				...
				$modal.find('.button-deactivate').text('<?php printf( SO_Admin_Deactivation::load_str( 'deactivation-modal-button-submit' ) ); ?>');
		...
		function resetModal() {
			if ( SOCustomReasons.hasOwnProperty(SOCurrentPluginBaseName) === true ) {
				$modal.find("ul#reasons-list").html(SOCustomReasons[SOCurrentPluginBaseName]);
			} else {
				$modal.find("ul#reasons-list").html(reasonsHtml);

			}
			var defaultSelect = SODefaultReason[SOCurrentPluginBaseName];
		...
		function updateButtonLabels() {
			...
			if ( 'confirm' === getCurrentPanel() ) {
				$deactivateButton.text('<?php printf( SO_Admin_Deactivation::load_str( 'deactivation-modal-button-confirm' ) ); ?>');
			} else {
				var $radio = $('input[type="radio"]:checked');
				if( $radio.length == 0 ) {
					$deactivateButton.text('<?php printf( SO_Admin_Deactivation::load_str( 'deactivation-modal-button-deactivate' ) ); ?>');
				} else {
					...
					$modal.find('.reason-input').remove();
					$modal.find('.button-deactivate').text('<?php printf( SO_Admin_Deactivation::load_str( 'deactivation-modal-skip-deactivate' ) ); ?>');
