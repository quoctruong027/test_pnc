...
//		echo '<pre>'.print_r(wc()->session,true).'</pre>';die;
		...
		} elseif ( wp_verify_nonce( $_POST['_nonce'], '_woopb_add_to_woocommerce' ) ) { /*Process add to WooCommerce cart*/
	...
	public function share_link_callback() {
		...
		if ( isset( $_GET['woopb_share'] ) ) {
			...
			if ( isset( $data['page_id'], $data['woopb_id'] ) ) {
				...
				wp_safe_redirect( site_url( "woo_product_builder/?page_id={$data['page_id']}&woopb_preview=1" ) );
				exit;
			}
		}
	}
