...
final class WFACP_admin {
	...
	protected function __construct() {
		...
		add_action( 'add_meta_boxes_' . WFACP_Common::get_post_type_slug(), [ $this, 'add_meta_boxes_for_shortcodes' ], 10, 2 );
		add_action( 'wfacp_builder_design_after_template', [ $this, 'add_short_code_wrapper' ] );
		...
		/*** bwf general setting ***/
		add_filter( 'woofunnels_global_settings', [ $this, 'woofunnels_global_settings' ] );
		add_filter( 'bwf_general_settings_link', [ $this, 'bwf_general_settings_link' ] );
		add_filter( 'bwf_general_settings_fields', [ $this, 'bwf_general_settings_fields' ] );
		...
		add_action( 'admin_footer', [ $this, 'permalink_box' ], 50 );
		add_filter('bwf_general_settings_default_config', [$this,'add_default_value_of_permalink_base']);
	}
	...
	public function maybe_register_breadcrumbs() {
		if ( WFACP_Common::is_load_admin_assets( 'builder' ) ) {
			...
			if ( empty( BWF_Admin_Breadcrumbs::$nodes ) ) {
				BWF_Admin_Breadcrumbs::register_node( array( 'text' => __( 'Checkouts', 'woofunnels-aero-checkout' ), 'link' => admin_url( 'admin.php?page=wfacp' ) ) );
			}
			//$funnel_id = WFOCU_Core()->funnels->get_funnel_id();
			BWF_Admin_Breadcrumbs::register_node( array( 'text' => WFACP_Common::get_page_name(), 'link' => '', 'class' => 'wfacp_page_title' ) );
		}
	...
	public function register_admin_menu() {
		...
		$hook = add_submenu_page( 'woofunnels', 'Checkouts', 'Checkouts', 'manage_woocommerce', 'wfacp', [
			$this,
			'admin_page',
		] );
		if ( isset( $_GET['page'] ) && 'wfacp' == $_GET['page'] && ! isset( $_GET['wfacp_id'] ) ) {
			add_action( "load-$hook", [ $this, 'wfacp_screen_options' ] );
		}
	}
	...
	public function admin_enqueue_assets() {
		...
		if ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'wfacp' ) {
			...
			if ( isset( $_GET['section'] ) && $_GET['section'] == 'bwf_settings' ) {
				BWF_Admin_General_Settings::get_instance()->maybe_add_js();
			}

			if ( defined( 'BWF_DEV' ) ) {
				wp_enqueue_script( 'wfacp', $this->get_admin_url() . '/assets/js/wfacp_combined.js', array( 'jquery', 'underscore', 'backbone', 'updates' ), WFACP_VERSION_DEV );
			} else {
				wp_enqueue_script( 'wfacp', $this->get_admin_url() . '/assets/js/wfacp_combined.min.js', array( 'jquery', 'underscore', 'backbone', 'updates' ), WFACP_VERSION_DEV );
			}
	...
	public function global_dependency_messages() {
		...
		if ( wc_shipping_enabled() ) {
			...
			$msg = __( sprintf( 'Your store has <a href="%s">shipping location</a> enabled. Depending upon shipping method configuration, checkout may need "Shipping Method" or "Order Summary" field. Please drag Shipping Method field to place in form. Note: If items in cart have no shipping applicable, this field will be automatically hidden.', $shipping_location, $shipping_methods ), 'woofunnels-aero-checkout' );
...
#order_data #wfacp_admin_advanced_field span.wfacp_radio_options_group:after{    clear: both;} </style>";
	...
	public function admin_page() {
		if ( isset( $_GET['page'] ) && $_GET['page'] === 'wfacp' ) {
			...
			if ( isset( $_GET['section'] ) && $_GET['section'] === 'export' ) {
				include_once __DIR__ . "/views/flex-export.php";

				return;
			} else if ( isset( $_GET['section'] ) && $_GET['section'] === 'import' ) {
				include_once __DIR__ . "/views/flex-import.php";

				return;
			} else if ( isset( $_GET['section'] ) && $_GET['section'] === 'bwf_settings' ) {
			...
			if ( '' != $has_data ) {
				...
				if ( $field['type'] == 'multiselect' ) {
					...
					if ( is_array( $has_data ) && count( $has_data ) > 0 ) {
						...
						$temp_data                                = strtolower( $temp_data );
						$field['custom_attributes']['data_value'] = $temp_data;
					}
					$has_data = null;
				}
	...
	public function admin_footer() {
        ...
        <script>
            var multiselect = jQuery('.wfacp_custom_field_multiselect select');
            if (multiselect.length > 0) {
                multiselect.each(function () {

                    let value = jQuery(this).attr('data_value');
                    if (value != undefined && value != '') {
                        let selected_options = value.split(',');
                        jQuery(this).val(selected_options);
                        jQuery(this).select2();
                    }
                })
		...
		if ( ! empty( $_POST ) && count( $_POST ) > 0 ) {
			...
			foreach ( $post_data as $key => $value ) {
				...
				if ( false !== strpos( $key, 'wfacp_' ) ) {
					$save_key = str_replace( 'wfacp_', '', $key );

					update_post_meta( $post_id, $save_key, $post_data[ $key ] );
				}
			}
	...
	public function creating_aero_default_pages() {
		...
		$templates = [
			...
			[ 'slug' => 'shopcheckout-three', 'name' => 'ShopCheckout (Multistep)', 'is_multi' => true ],
		];
	...
	public function add_short_code_wrapper() {
		include_once __DIR__ . '/views/sections/design/short-code.php';
	}


	public function save_meta_box_value() {
		if ( isset( $_REQUEST['post_type'] ) && 'page' == $_REQUEST['post_type'] && isset( $_REQUEST['post_ID'] ) ) {
			$id = $_POST['post_ID'];
	...
	public function add_meta_boxes_for_make_page_as_checkout() {
		add_meta_box( 'woofunnels-aero-checkout', __( 'Aero Checkout', 'woofunnels-aero-checkout' ), [ $this, 'render_my_meta_box' ], 'page', 'side', 'default' );
	}

	public function add_meta_boxes_for_shortcodes() {
		$post_type = WFACP_Common::get_post_type_slug();
		add_meta_box( 'woofunnels-aero-checkout-shortcode', __( 'Aero Checkout', 'woofunnels-aero-checkout' ), [ $this, 'render_shortcode_meta_box' ], $post_type, 'side', 'default' );
	}
	...
	public function render_my_meta_box() {
		...
		echo "<input type='checkbox' id='aero-checkout-checkbox' name='aero-checkout-checkbox' " . $is_checked . "><label  for='aero-checkout-checkbox'>" . __( 'Make this page as a checkout page', 'woofunnels-aero-checkout' ) . '</label>';
	...
	public function render_shortcode_meta_box() {
        ...
        <div class="wfacp-scodes-notes">
            <i> <?php printf( __( 'Edit Page to embed this shortcode & generate Preview. Use parameter id=%s to embed this form anywhere.' ), $id ) ?>
                <a href="//buildwoofunnels.com/docs/aerocheckout/embed-form/id-parameter" target="_blank"><?php _e( 'Know More', 'woofunnels-aero-checkout' ) ?></a></i>

        </div>

        <script id="wfacp-back-button-template" type="text/html">
            <div id="wfacp-switch-mode">
                <a id="elementor-back-button" class="button button-default button-large" href="<?php echo admin_url( 'admin.php?page=wfacp&wfacp_id=' . $id . '&section=design' ); ?>">
					<?php echo __( '&#8592; Back to Checkout', 'elementor' ); ?>
                </a>
            </div>
        </script>
        ...
        <script>
            window.addEventListener('load', function () {
                (function ($) {
                    ...
                    let back_button = $($('#wfacp-back-button-template').html());
	...
	public function add_back_button() {
        ...
        <div id="wfacp-switch-mode">
            <a id="wfacp-back-button" class="button button-default button-large" href="<?php echo admin_url( 'admin.php?page=wfacp&wfacp_id=' . $this->wfacp_id . '&section=design' ); ?>">
				<?php echo __( '&#8592; Back to Checkout', 'elementor' ); ?>
            </a>
        </div>
	...
	public function wfacp_screen_options() {
		...
		$args = array(
			'label'   => __( 'Checkout per page', 'woofunnels-aero-checkout' ),
			'default' => 20,
			'option'  => 'wfacp_per_page'
		);
		add_screen_option( $option, $args );
		include_once WFACP_PLUGIN_DIR . '/includes/class-wfacp-post-table.php';
		$this->wfacp_pages_table = new WFACP_Post_Table();
	}
	...
	function get_advanced_field() {
		if ( isset( $_REQUEST['post'] ) && $_REQUEST['post'] > 0 ) {
			$wfacp_id = get_post_meta( $_REQUEST['post'], '_wfacp_post_id', true );
	...
	public function woofunnels_global_settings( $menu ) {
	...
	public function bwf_general_settings_fields( $fields ) {
	 ...
	 */
	public function add_default_value_of_permalink_base( $fields ) {
