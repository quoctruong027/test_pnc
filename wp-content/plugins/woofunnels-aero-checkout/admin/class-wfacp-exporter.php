...
class WFACP_Exporter {
	...
	public function maybe_export() {
		...
		if ( ! wp_verify_nonce( $_POST['wfacp-action-nonce'], 'wfacp-action-nonce' ) ) {
			return;
		}
	...
	public function maybe_export_single() {
		...
		if ( ! wp_verify_nonce( $_GET['_wpnonce'], 'wfacp-export' ) ) {
			return;
		}
		...
		$acps_to_export    = [];
		$acps_to_export[0] = $this->get_acp_array_for_json( $_GET['id'] );
		$acps_to_export    = apply_filters( 'wfacp_export_data', $acps_to_export );
