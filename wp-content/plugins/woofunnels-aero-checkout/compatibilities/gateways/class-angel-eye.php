...
class WFACP_Compatibility_With_Angel_Eye {
	...
	public function add_buttons( $buttons ) {
		...
		if ( 'yes' != $settings['enabled'] || 'regular' == $settings['show_on_checkout'] || 'no' == $settings['show_on_checkout'] ) {
			return $buttons;
		}
//		VERSION_PFW
	...
	public function make_session_empty() {
		remove_filter( 'woocommerce_get_checkout_url', [ $this, 'change_checkout_url' ] );
		if ( ! is_null( WC()->session ) ) {
			WC()->session->set( 'wfacp_angell_eye_error', '' );
		}
	}
