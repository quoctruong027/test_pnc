...
Place your license key here', 'woofunnels' ) . '" />' . "\n";
		...
		if ( $this->is_license_expire( $item ) ) {
			...
			if ( empty( $item['_data']['activated'] ) ) {
				...
				$deactivate_url = wp_nonce_url( add_query_arg( 'action', 'woofunnels_deactivate-product', add_query_arg( 'filepath', $item['product_file_path'], add_query_arg( 'page', $_GET['page'], add_query_arg( 'tab', 'licenses' ), network_admin_url( 'admin.php' ) ) ) ), 'bulk-licenses' ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification, WordPress.Security.ValidatedSanitizedInput
