...
class WooFunnels_Process {
	...
	public function parse_request_and_process() {
		...
		if ( isset( $_GET['page'] ) && 'woofunnels' === sanitize_text_field( $_GET['page'] ) && isset( $_GET['tab'] ) && 'licenses' === sanitize_text_field( $_GET['tab'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			WooFunnels_licenses::get_instance();
		}

		//Handling Optin
		if ( isset( $_GET['woofunnels-optin-choice'] ) && isset( $_GET['_woofunnels_optin_nonce'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			if ( ! wp_verify_nonce( sanitize_text_field( $_GET['_woofunnels_optin_nonce'] ), 'woofunnels_optin_nonce' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			...
			$optin_choice = sanitize_text_field( $_GET['woofunnels-optin-choice'] ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			if ( $optin_choice === 'yes' ) {
				WooFunnels_optIn_Manager::Allow_optin();
				if ( isset( $_GET['ref'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		...
		//Initiating the license instance to handle submissions  (submission can redirect page two that can cause "header already sent" issue to be arised)
		// Initiating this to over come that issue
		if ( isset( $_GET['page'] ) && 'woofunnels' === sanitize_text_field( $_GET['page'] ) && isset( $_GET['tab'] ) && 'support' === sanitize_text_field( $_GET['tab'] ) && isset( $_POST['woofunnels_submit_support'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
