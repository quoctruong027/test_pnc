...
if ( $available_methods ) {
	foreach ( $available_methods as $method ) {
		if ( $chosen_method == $method->get_id() ) {
            ...
            <tr class="shipping_total_fee">
                <td colspan="<?php echo empty( $colspans['first'] ) ? '1' : $colspans['first'] ?>"><?php echo $package_name; ?><?php echo $shippingLabelText ?></td>
	...
	echo "<td {$colspans['first'] }>" . wp_kses_post( $package_name ) . "</td>";
	echo "<td {$colspans['second']} style=text-align:right;'>";
