...
if ( WFACP_Common::is_theme_builder() ) {
	?>
    <div class="wfacp_shipping_options <?php echo $classes; ?>" id="shipping_calculator_field" <?php echo WFACP_Common::get_fragments_attr() ?> >
...
if ( $shipping_hidden_fields && ! wp_doing_ajax() ) {
	...
	if ( is_array( $shippingMethods ) && count( $shippingMethods ) > 0 && ! wp_doing_ajax() ) {
		foreach ( $shippingMethods as $key => $value ) {
			...
			echo '<input type="hidden" class="wfacp_hidden_shipping" name="shipping_method[' . $key . ']"  value="' . $value . '" >';
