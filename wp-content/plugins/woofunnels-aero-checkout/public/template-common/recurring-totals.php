...
<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
    ...
    <tr class="cart-subtotal recurring-total">
		...
		if ( $display_th ) :
			$display_th = false;
			?>
            <th colspan="<?php echo $recurring_colspan_heading; ?>" rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>"><?php esc_html_e( 'Subtotal', 'woocommerce-subscriptions' ); ?></th>
		...
		<?php foreach ( $recurring_cart->get_coupons() as $recurring_code => $recurring_coupon ) : ?>
            ...
            <tr class="cart-discount coupon-<?php echo esc_attr( $code ); ?> recurring-total">
				...
				if ( $display_th ) :
					$display_th = false;
					?>
                    <th colspan="<?php echo $recurring_colspan_heading; ?>" rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>"><?php wc_cart_totals_coupon_label( $coupon ); ?></th>
	...
	<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
		...
		<?php foreach ( $recurring_cart->get_fees() as $recurring_fee ) : ?>
            ...
            <tr class="fee recurring-total">
                <th colspan="<?php echo $recurring_colspan_heading; ?>"><?php echo esc_html( $fee->name ); ?></th>
				...
				<?php foreach ( $recurring_cart->get_tax_totals() as $recurring_code => $recurring_tax ) : ?>
                    ...
                    <tr class="tax-rate tax-rate-<?php echo esc_attr( sanitize_title( $recurring_code ) ); ?> recurring-total">
						...
						if ( $display_th ) :
							$display_th = false;
							?>
                            <th colspan="<?php echo $recurring_colspan_heading; ?>"><?php echo esc_html( $recurring_tax->label ); ?></th>
		...
		<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
            ...
            <tr class="tax-total recurring-total">
				...
				if ( $display_th ) :
					$display_th = false;
					?>
                    <th colspan="<?php echo $recurring_colspan_heading; ?>"><?php echo esc_html( WC()->countries->tax_or_vat() ); ?></th>
...
<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
    ...
    <tr class="order-total recurring-total">
		...
		if ( $display_th ) :
			$display_th = false;
			?>
            <th colspan="<?php echo $recurring_colspan_heading; ?>" rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>"><?php esc_html_e( 'Recurring Total', 'woocommerce-subscriptions' ); ?></th>
