...
<div class="wfacp_main_form woocommerce <?php echo implode( ' ', $form_class ); ?>">
    ...
    <form name="checkout" method="post" class="checkout woocommerce-checkout" action="<?php echo esc_url( $permalink ); ?>" enctype="multipart/form-data" id="wfacp_checkout_form">
        ...
        <input type="hidden" id="billing_shipping_index" value="<?php esc_html_e( $instance->get_shipping_billing_index() ); ?>">
		...
		if ( ! $instance->have_shipping_address() && ! $instance->have_billing_address() && class_exists( 'WC_Geolocation' ) ) {
			...
			?>
            <input type="hidden" name="billing_country" id='billing_country' value="<?php esc_html_e( $default_country ) ?>"/>
			<?php
				...
				if ( ! empty( $sections ) ) {
					foreach ( $sections as $section_index => $section ) {
						...
						if ( isset( $section['html_fields'] ) && is_array( $section['html_fields'] ) ) {
							$htmlsfields = $section['html_fields'];
							$class_here  = '';

							if ( array_key_exists( 'shipping_calculator', $htmlsfields ) ) {
								$class_here = ' wfacp_shipping_method';
							}
							if ( array_key_exists( 'product_switching', $htmlsfields ) ) {
								$class_here = ' wfacp_product_switcher';
							}
							if ( array_key_exists( 'order_summary', $htmlsfields ) ) {
								$class_here = ' wfacp_order_summary_box';
							}

							if ( array_key_exists( 'order_coupon', $htmlsfields ) ) {
                                ...
                                <div class="wfacp-row">
									...
									foreach ( $fields as $field ) {
										...
										if ( 'billing_email' === $key && isset( $field['placeholder'] ) && 'abc@exmple.com' === $field['placeholder'] ) {
											$field['placeholder'] = 'john.doe@example.com ';
										}
		...
		if ( $instance->have_shipping_address() ) {
            ...
            <div id='ship-to-different-address'>
                <input id="ship-to-different-address-checkbox" class="ship_to_different_address" type="checkbox" name="ship_to_different_address" style="display:none" <?php echo esc_attr( $is_checked ) ?>>
            </div>
			<?php
...
if ( function_exists( 'woocommerce_photoswipe' ) && true === apply_filters( 'wfacp_wc_photoswipe_enable', true ) ) {
