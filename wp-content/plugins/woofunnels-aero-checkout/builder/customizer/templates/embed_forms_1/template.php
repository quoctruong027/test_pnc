...
final class WFACP_Template_Custom_Page extends WFACP_Pre_Built {
	...
	protected function __construct() {
		...
		add_filter( 'wfacp_layout_default_setting', [ $this, 'change_default_setting_disabled_stepbar' ], 11, 2 );
				...
				foreach ( $form_border as $fkey => $fborder ) {
					...
					if ( $fkey == 'border-width' ) {
						...
						if ( isset( $data[ $section_key ]['layout']['disable_steps_bar'] ) && $data[ $section_key ]['layout']['disable_steps_bar'] == false ) {
							...
							if ( $fborder > 0 ) {
//								$this->steps_inline_styles['desktop']['body .wfacp_form .wfacp-payment-title']['margin-left']  = "-" . $fborder . $px;
//								$this->steps_inline_styles['desktop']['body .wfacp_form .wfacp-payment-title']['margin-right'] = "-" . $fborder . $px;
							}
...
//		}
	...
	public function change_default_setting_disabled_stepbar( $panel_details, $panel_key ) {
	...
	public function change_default_setting1( $panel_details, $panel_key ) {
		...
		$file = WFACP_PLUGIN_DIR . '/importer/embed-forms/' . $selectedSlug . '.php';
