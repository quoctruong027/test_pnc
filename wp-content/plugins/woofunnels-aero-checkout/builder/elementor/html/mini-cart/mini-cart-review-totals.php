...
$colspan_attr_1 = "colspan='$colspan_first'";
$colspan_attr_2 = "colspan='$colspan_second'";
...
?>
    <table class="shop_table <?php echo $instance->get_template_slug(); ?> wfacp_mini_cart_reviews mini_cart_wrap_here" id="wfacp_mini_cart_reviews_<?php echo $widget_id ?>">
        ...
        <tr class="cart-subtotal">
            <th colspan="<?php echo $colspan_first ?>"><?php _e( 'Subtotal', 'woocommerce' ); ?></th>
		...
		<?php foreach ( WC()->cart->get_coupons() as $code => $coupon ) : ?>
            <tr class="cart-subtotal cart-discount coupon-<?php echo esc_attr( sanitize_title( $code ) ); ?>">
                <th colspan="<?php echo $colspan_first ?>"><?php $instance->wc_cart_totals_coupon_label( $coupon ) ?></th>
		...
		<?php foreach ( WC()->cart->get_fees() as $fee ) : ?>
            <tr class="cart-subtotal fee">
                <th colspan="<?php echo $colspan_first ?>"><?php echo esc_html( $fee->name ); ?></th>
			...
			<?php if ( 'itemized' === get_option( 'woocommerce_tax_total_display' ) ) : ?>
				<?php foreach ( WC()->cart->get_tax_totals() as $code => $tax ) : ?>
                    <tr class="cart-subtotal tax-rate tax-rate-<?php echo sanitize_title( $code ); ?>">
                        <th colspan="<?php echo $colspan_first ?>"><?php echo esc_html( $tax->label ); ?></th>
                        <td colspan="<?php echo $colspan_second ?>"><?php echo wp_kses_post( $tax->formatted_amount ); ?></td>
                    </tr>
				<?php endforeach; ?>
			<?php else : ?>
                <tr class="cart-subtotal tax-total">
                    <th colspan="<?php echo $colspan_first ?>"><?php echo esc_html( WC()->countries->tax_or_vat() ); ?></th>
        ...
        <tr class="order-total">
            <th colspan="<?php echo $colspan_first ?>"><?php _e( 'Total', 'woocommerce' ); ?></th>
