jQuery(function ($) {
/*
	$("#a2wl_bulk_order_place").click(function(){
		$('.hover_a2wl_fulfillment').show();
		return false;
	});*/
	...
	$(document).on("click", ".a2wl_aliexpress_order_fulfillment", function () {
		...
		a2wl_start_order_process(ids, 1);
   
	//	$.a2wl_ali_fulfill_order(id);
	...
	var a2wl_start_order_process = function(ids, total_ids) {
		if (total_ids > 0) {
			a2wl_reset_blocks();
			...
			var on_load = function (response_state, state, data, ids) {
				...
				if (response_state == "error"){
					...
					if (data.stage === 1) {
						a2wl_show_tip(a2wl_sprintf(a2wl_ali_orderfulfill_js.lang.please_connect_chrome_extension_check_d, 'https://ali2woo.com/codex/ali2woo-lite-google-chrome-extension/'), true);

						$('.hover_a2wl_fulfillment .continue').off('click').click(function () {
							rsp_timer = app_rsp_timer_run();
							a2wl_start_order_process(ids, ids.length);
						});
					}
	...
	$("#doaction, #doaction2").click(function (event) {
		...
		if ('a2wl_order_place_bulk' === check_action) {
			...
			a2wl_start_order_process(ids, total_ids);
	...
	var a2wl_show_block = function(message){
		$('.hover_a2wl_fulfillment .pr').html(message)
		$('.hover_a2wl_fulfillment').show();
	}

	var a2wl_hide_block = function(){
		$('.hover_a2wl_fulfillment').hide();
	}

	var a2wl_show_tip = function(message, _continue, skip, payall, close, solve){

		a2wl_reset_blocks();

		$('.hover_a2wl_fulfillment .tip').html(message)
		$('.hover_a2wl_fulfillment .tip').show();

		if (_continue){
			$('.hover_a2wl_fulfillment .continue').show();
		}
		if (skip){
			$('.hover_a2wl_fulfillment .skip').show();
		}
		if (payall){
			$('.hover_a2wl_fulfillment .payall').show();
		}

		if (typeof close !== "undefined" && close){
			$('.hover_a2wl_fulfillment .close').show();
		}

		if (typeof solve !== "undefined" && solve){
			$('.hover_a2wl_fulfillment .solve').show();
		}
	}

	var a2wl_reset_blocks = function(){
		$('.hover_a2wl_fulfillment .tip').html('');
		$('.hover_a2wl_fulfillment .tip').hide();
		$('.hover_a2wl_fulfillment .continue').hide();
		$('.hover_a2wl_fulfillment .skip').hide();
		$('.hover_a2wl_fulfillment .payall').hide();
		$('.hover_a2wl_fulfillment .close').hide();
		$('.hover_a2wl_fulfillment .solve').hide();
	}
