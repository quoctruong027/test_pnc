var a2wl_isExternal = function (url) {
    ...
    $(function () {
        ...
        $('#input-mask-image-file').on('change', function () {
            ...
            if (file) {
                ...
                $(".a2wl-edit-photo-container .block-item.input-wrapper").after('<div class="block-item"><a href="#" class="sticker"><img src="' + imgUrl + '"/></a></div>');

                setTimeout(function () {
                    var form_data = new FormData();
                    form_data.append('action', 'a2wl_upload_sticker');
        ...
        $(".product_images .image").append('<a href="#" class="a2wl-photo-edit">Edit</a>');

        $("#postimagediv .inside").append('<a href="#" class="a2wl-photo-edit">Edit</a>');

        $('.a2wl-product-import-list [rel="images"] .image').append('<a href="#" class="a2wl-photo-edit">Edit</a>');

        $(".a2wl-modal-wrapper .a2wl-modal-close").click(function () {
            $(this).parents('.a2wl-modal-wrapper').hide();
            return false;
        });


        $('.a2wl-edit-photo-container').on('click', '.sticker', function (e) {
        ...
        function loadImage(image_url) {
            $('.a2wl-modal-content .a2wl-edit-photo-loader').show();
            if(image_url){
                image_url = a2wl_isExternal(image_url) ? a2wl_wc_pe_script_data.a2wl_cdn_url + '?url=' + image_url : image_url;
                imageEditor.loadImageFromURL(image_url, 'SampleImage').then(function (sizeValue) {
                    // calc initial size
                    imageEditor.resizeCanvasDimension({width: 1200, height: $('.a2wl-modal-content').height()});
                    imageEditor.clearUndoStack();
                    previewCrop('original');
                    $('.a2wl-modal-content .a2wl-edit-photo-loader').hide();
                });    
            }
            
            $('.a2wl-modal-toolbar .spinner').removeClass('is-active');
            $("#a2wl-edit-image").show();
        }

        $(".product_images .image .a2wl-photo-edit").click(function () {
            $('.a2wl-edit-photo-container').attr('data-attachment_id', $(this).parents('.image').attr('data-attachment_id'));
            $('.a2wl-edit-photo-container').attr('data-view', 'product');
            $('.a2wl-edit-photo-container').attr('data-product_id', '');
            
            var data = {action: 'a2wl_get_image_by_id', attachment_id: $(this).parents('.image').attr('data-attachment_id')};
        ...
        $("#postimagediv .inside .a2wl-photo-edit").click(function () {
            $('.a2wl-edit-photo-container').attr('data-attachment_id', $(this).parents('.inside').find('#_thumbnail_id').val());
            $('.a2wl-edit-photo-container').attr('data-view', 'product');
            $('.a2wl-edit-photo-container').attr('data-product_id', '');
        ...
        $('.a2wl-product-import-list [rel="images"] .image .a2wl-photo-edit').click(function () {
            $('.a2wl-edit-photo-container').attr('data-attachment_id', $(this).parents('.image').attr('id'));
            $('.a2wl-edit-photo-container').attr('data-view', 'import');
            $('.a2wl-edit-photo-container').attr('data-product_id', $(this).parents('.product').attr('data-id'));
        ...
        $("#a2wl-edit-image .save-image").click(function () {
            var _this_btn = $(this);
            $(_this_btn).attr('disabled', 'disabled');
            $('.a2wl-modal-toolbar .spinner').addClass('is-active');
            var current_attachment_id = $('.a2wl-edit-photo-container').attr('data-attachment_id');
            var save = function () {
                var data = {action: 'a2wl_save_image', attachment_id: current_attachment_id, product_id: $('.a2wl-edit-photo-container').attr('data-product_id'), view: $('.a2wl-edit-photo-container').attr('data-view'), data: imageEditor.toDataURL(), name: imageEditor.getImageName()};
                jQuery.post(ajaxurl, data).done(function (response) {
                    var json = jQuery.parseJSON(response);
                    if (json.state !== 'ok') {
                        console.log(json);
        ...
        $("#a2wl-edit-image .cancel-image").click(function () {
            $('#a2wl-edit-image').hide();
        });
