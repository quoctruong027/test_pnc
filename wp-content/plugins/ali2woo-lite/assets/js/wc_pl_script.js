...
var a2wl_update_action_lock = false;

function a2wl_get_product_proc(params, callback) {
    if (typeof a2wl_get_product === "function") {
        a2wl_get_product(params, callback);
    } else {
        callback('error', false, 'Please install and activate the ali2woo-lite chrome extension in your browser: <a href="' + a2wl_wc_pl_script.chrome_url + '">Get Chrome Extension</a>');
    }
}
...
(function ($, window, document, undefined) {
    $(function () {
        ...
        $(document).on("click", ".a2wl-product-info", function () {
            var id = $(this).attr('id').split('-')[1];
            $.a2wl_show(id);
            return false;
        });

        $.a2wl_show = function (id) {
            $('<div id="a2wl-dialog' + id + '"></div>').dialog({
                dialogClass: 'a2wl-dialog',
                width: "400px",
                modal: true,
                title: "Aliexpress Info (ID: " + id + ")",
                open: function () {
                    $('#a2wl-dialog' + id).html(a2wl_wc_pl_script.lang.please_wait_data_loads);
                    var data = { 'action': 'a2wl_product_info', 'id': id };
                    ...
                    $.post(ajaxurl, data, function (response) {
                        ...
                        if (json.state === 'error') {
                            ...
                            $('#a2wl-dialog' + json.data.id).html(json.data.content.join('<br/>'));
                        }
        ...
        function a2wl_js_update_product(products_to_update, state, on_load_calback) {
            if (products_to_update.length > 0) {
                ...
                if (a2wl_wc_pl_script.chrome_ext_import && data.action == 'a2wl_sync_products') {
                    var external_id = jQuery('#a2wl-' + data.ids[0]).attr('data-external-id');
                    a2wl_get_product_proc({ id: external_id, locale: a2wl_wc_pl_script.locale, curr: a2wl_wc_pl_script.currency }, function (apd_state, apd, msg) {
                        if (apd_state !== 'error') {
        ...
        $("#doaction, #doaction2").click(function (e) {
            ...
            if (!a2wl_update_action_lock && ('a2wl_product_update_manual' === check_action || 'a2wl_product_update_reviews_manual' === check_action)) {
                e.preventDefault();
                a2wl_update_action_lock = true;
                ...
                var cnt = 0;
                var total_ids = 0;
                var data = { 'action': ('a2wl_product_update_manual' === check_action) ? 'a2wl_sync_products' : 'a2wl_sync_products_reviews', 'ids': [] }
                $('input:checkbox[name="post[]"]:checked').each(function () {
                    ...
                    if (cnt === update_per_request) {
                        ...
                        cnt = 0;
                        data = { 'action': ('a2wl_product_update_manual' === check_action) ? 'a2wl_sync_products' : 'a2wl_sync_products_reviews', 'ids': [] };
                    }
                });
                ...
                if (total_ids > 0) {
                    a2wl_show_block($('.wp-list-table.posts'), a2wl_sprintf(a2wl_wc_pl_script.lang.process_update_d_of_d, 0, total_ids));

                    a2wl_sprintf(a2wl_wc_pl_script.lang.process_update_d_of_d, 0, total_ids)
                    var on_load = function (response_state, state) {
                        if ((state.update_cnt + state.update_error_cnt) === state.num_to_update) {
                            a2wl_update_block($(".wp-list-table.posts"), a2wl_sprintf(a2wl_wc_pl_script.lang.complete_result_updated_d_erros_d, state.update_cnt, state.update_error_cnt));
                            a2wl_update_action_lock = false;
                            location.reload();
                        } else {
                            a2wl_update_block($(".wp-list-table.posts"), a2wl_sprintf(a2wl_wc_pl_script.lang.process_update_d_of_d_erros_d, state.update_cnt, state.num_to_update, state.update_error_cnt));
                        }
                    };

                    var state = { num_to_update: total_ids, update_cnt: 0, update_error_cnt: 0 };
                    a2wl_js_update_product(products_to_update, state, on_load);
                }
            }
        });
    });
...
function a2wl_show_block(element, message) {
...
function a2wl_update_block(element, message) {
    jQuery(element).find('.blockUI.blockMsg').html(message);
}

function a2wl_hide_block(element) {
