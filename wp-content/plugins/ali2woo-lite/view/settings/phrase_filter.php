...
<div class="panel panel-primary mt20" id="phrase_list">
    <div class="panel-heading">
        <h3 class="panel-title"><?php _ex('Phrase Filtering (case sensitive)', 'Setting title', 'ali2woo-lite'); ?></h3>
        <span class="pull-right">
            <a class="disabled" style="display: none;"><?php _e('You have unsaved changes', 'ali2woo-lite'); ?></a>
            <a href="#" class="apply-phrase-rules btn"><?php _e('Apply Filter to your Shop', 'ali2woo-lite'); ?></a></span>
    </div>


    <div class="panel-body">
        <div class="panel panel-default" id="a2wl-panel-info" style="display: none;">
            <div class="panel-heading"><?php _e('Applying filter progress', 'ali2woo-lite'); ?>  <button type="button" class="close" data-target="#a2wl-panel-info" data-dismiss="alert"> <span aria-hidden="true">&times;</span><span class="sr-only"><?php _e('Close'); ?></span>
        ...
        <div class="row">
            <div class="col-md-4 pb20">
                <strong><?php _e('Phrase', 'ali2woo-lite'); ?></strong>
            </div>
            <div class="col-md-8 pb20">
                <strong><?php _e('Replacement', 'ali2woo-lite'); ?></strong>   
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group input-block no-margin">
                        <input type="text" value="<?php echo $phrase->phrase; ?>" class="form-control small-input a2wl_phrase" placeholder="<?php _e('some phrase or word', 'ali2woo-lite'); ?>" />
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group input-block no-margin">
                        <input type="text" value="<?php echo $phrase->phrase_replace; ?>" class="form-control small-input a2wl_phrase_replace" placeholder="<?php _e('sreplacement or empty', 'ali2woo-lite'); ?>" />
                    </div>   
                </div>
                <div class="col-md-4">
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="form-group input-block no-margin">
                    <input type="text" class="form-control small-input a2wl_phrase" placeholder="<?php _e('some phrase or word', 'ali2woo-lite'); ?>" />
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group input-block no-margin">
                    <input type="text" class="form-control small-input a2wl_phrase_replace" placeholder="<?php _e('sreplacement or empty', 'ali2woo-lite'); ?>" />
                </div>   
            </div>
            <div class="col-md-4">
...
<div class="container-fluid">
    <div class="row pt20 border-top">
        <div class="col-sm-12">
            <input class="btn btn-success" id="save-phrases"  type="submit" value="<?php _e('Save settings', 'ali2woo-lite'); ?>"/>
        </div>
    </div>
</div>
...
<div class="modal-overlay modal-apply-phrases">
    <div class="modal-content">
        <div class="modal-header">
            <h3 class="modal-title"><?php _e('Apply Filter to your Shop', 'ali2woo-lite'); ?></h3>
            <a class="modal-btn-close" href="#"><svg><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-cross"></use></svg></a>
        </div>
        <div class="modal-body">
            <label><?php _e('Select the update type', 'ali2woo-lite'); ?></label>
            <div style="padding-bottom: 20px;">
                <div class="type btn-group" role="group">
                    <button type="button" class="btn btn-default" value="products"><?php _ex('Products', 'Apply Phrases', 'ali2woo-lite'); ?></button>
                    <button type="button" class="btn btn-default" value="reviews"><?php _ex('Reviews', 'Aplly Phrases', 'ali2woo-lite'); ?></button>
                    <?php /*
                      <button type="button" class="btn btn-default" value="shippings"><?php _ex('Shipping methods', 'Apply Phrases', 'ali2woo-lite'); ?></button>
                     */ ?>
                    <button type="button" class="btn btn-default" value="all_types"><?php _ex('All', 'Apply Phrases', 'ali2woo-lite'); ?></button>
                </div>
            </div>
            <div class="scope">
                <label><?php _e('Select the update scope', 'ali2woo-lite'); ?></label>
                <div>
                    <div class="scope btn-group" role="group">
                        <button type="button" class="btn btn-default" value="shop"><?php _ex('Shop', 'Apply Phrases', 'ali2woo-lite'); ?></button>
                        <button type="button" class="btn btn-default" value="import"><?php _ex('Import List', 'Apply Phrases', 'ali2woo-lite'); ?></button>
                        <button type="button" class="btn btn-default" value="all"><?php _ex('Shop and Import List', 'Apply Phrases', 'ali2woo-lite'); ?></button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    (function ($) {
        ...
        var set_progress_bar_value = function (c, v) {
            jQuery('#a2wl-panel-info ' + c + ' .progress-bar')
                    .css('width', v + '%')
        ...
        var get_status_filter = function (show) {
            ...
            var data = {action: 'a2wl_get_status_apply_phrase_rules'};
            jQuery.post(ajaxurl, data).done(function (response) {
                ...
                if (json.state === 'error') {
                    ...
                    if (typeof json.review_valuenow !== 'undefined') {
                        ...
                        jQuery('#a2wl-panel-info').fadeIn(400);
        ...
        $(".modal-apply-phrases .apply-btn").on("click", function () {
            ...
            var data = {action: 'a2wl_apply_phrase_rules', type: $(".modal-apply-phrases .btn-group.type").data().value, scope: $(".modal-apply-phrases .btn-group.scope").data().value};
            jQuery.post(ajaxurl, data).done(function (response) {
        ...
        function check_phrases() {
            ...
            $('#phrase_list > .panel-body .row:gt(0)').each(function () {
                if (!$(this).is(":last-child") && $(this).find(".a2wl_phrase").length > 0 && $.trim($(this).find(".a2wl_phrase").val()) == '') {
                    $(this).find(".a2wl_phrase").addClass('has-error');
                    empty_check = false;
                }
            });
        ...
        function set_last_phrase_row_enability(show) {
            ...
            if (show) {
                row.find('.a2wl_phrase_replace').removeClass('opacity50');
                row.find('.a2wl_phrase_replace').prop('disabled', false);

            } else {
                row.find('.a2wl_phrase_replace').removeClass('opacity50').addClass('opacity50');

                row.find('.a2wl_phrase_replace').prop('disabled', true);
        ...
        function add_phrase_row(this_row) {
            ...
            new_row.find('.a2wl_phrase').val('');
            new_row.find('.a2wl_phrase_replace').val('');
        ...
        $('#phrase_list > .panel-body').on('keyup', 'input[type="text"]', function () {
            ...
            keyup_timer = setTimeout(function () {
                ...
                if (check_phrases() && $.trim($(this_row).parents('.panel-body').find(".row:last-child .a2wl_phrase").val()) != '') {
        ...
        $('#save-phrases').on('click', function () {
            ...
            var data = {'action': 'a2wl_update_phrase_rules', 'phrases': []};

            $('#phrase_list > .panel-body .row').each(function () {
                if (!$(this).is(":last-child") && !$(this).is(":first-child")) {
                    var rule = {'phrase': $(this).find('.a2wl_phrase').val(),
                        'phrase_replace': $(this).find('.a2wl_phrase_replace').val()
                    };
