...
/**
 * Description of A2WL_AliexpressLocalizator
 *
 * @author Andrey
 */
if (!class_exists('A2WL_AliexpressLocalizator')) {

    class A2WL_AliexpressLocalizator {
        ...
        protected function __construct() {
            $this->language = strtolower(a2wl_get_setting('import_language'));
            $this->currency = strtoupper(a2wl_get_setting('local_currency'));
            ...
            if (a2wl_check_defined('A2WL_CUSTOM_CURRENCY')) {
                $cca = explode(";", A2WL_CUSTOM_CURRENCY);
                if (is_array($cca)) {
                    foreach ($cca as $cc) {
                        if($cc) {
                            $tmp_cur=explode("#", $cc);
