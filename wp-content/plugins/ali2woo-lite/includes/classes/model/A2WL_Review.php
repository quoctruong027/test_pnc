...
/**
 * Description of A2WL_Review
 *
 * @author MA_GROUP
 */
if (!class_exists('A2WL_Review')) {
    ...
    class A2WL_Review {
        ...
        public function __construct() {
            $this->aliexpress_loader = new A2WL_Aliexpress();
            $this->attachment_model = new A2WL_Attachment();
            $this->helper = new A2WL_Helper();

            $this->allowed_countries = a2wl_get_setting('review_allow_country');
            if (!empty($this->allowed_countries)) {
                $this->allowed_countries = explode(',', $this->allowed_countries);
            }

            $this->review_translated = a2wl_get_setting('review_translated');
            $this->review_load_attributes = a2wl_get_setting('review_load_attributes');

            $this->raiting_from = intval(a2wl_get_setting('review_raiting_from', 1));
            $this->raiting_to = intval(a2wl_get_setting('review_raiting_to', 5));

            $tmp = intval(a2wl_get_setting('review_max_per_product'));
            ...
            if ($remaining_comment_number > 0) {
                ...
                if ($res['state'] !== 'error' && !empty($res['reviews'])) {
                    ...
                    foreach ($res['reviews'] as $item) {
                        ...
                        $has_same_comment = $wpdb->get_var($wpdb->prepare("SELECT count(c.comment_ID) FROM {$wpdb->comments} c LEFT JOIN {$wpdb->commentmeta} cm ON (c.comment_ID = cm.comment_id and cm.meta_key='a2wl_cash') WHERE cm.meta_value=%s", $review_cash)) > 0;
                        ...
                        if ($this->review_load_attributes && isset($item['skuInfo'])) {
                            $tmp_text = $tmp_text . "<br/><br/>" . preg_replace('#([\w\-]+:)#', '<b>$1</b>', str_replace(':', ': ', A2WL_PhraseFilter::apply_filter_to_text($item['skuInfo'])));
                            //$tmp_text = $tmp_text . "<br/><br/>" . str_replace(':', ': ', A2WL_PhraseFilter::apply_filter_to_text($item['skuInfo']));
                        }
                        $review_text = A2WL_PhraseFilter::apply_filter_to_text($tmp_text);

                        $author = A2WL_PhraseFilter::apply_filter_to_text($item['buyerName']);

                        $date = date('Y-m-d H:i:s', strtotime($item['evalDate']));

                        $comment_approved = a2wl_get_setting('moderation_reviews') ? 0 : 1;
                        ...
                        add_comment_meta($comment_id, 'a2wl_cash', $review_cash, true);
                        add_comment_meta($comment_id, 'a2wl_country', $item['buyerCountry'], true);

                        if (a2wl_get_setting('review_avatar_import')) {
                        ...
                        if (a2wl_get_setting('review_show_image_list')) {
