...
/**
 * Description of A2WL_ProductImport
 *
 * @author andrey
 */
if (!class_exists('A2WL_ProductImport')) {

    class A2WL_ProductImport {
        ...
        public function add_product($product) {
            ...
            if (!isset($product['is_affiliate'])) {
                $aliexpress_loader = new A2WL_Aliexpress();
        ...
        public function get_product($product_id, $processing = false) {
            ...
            if ($product) {
                ...
                if (!isset($product['product_type'])) {
                    $product['product_type'] = a2wl_get_setting('default_product_type');
                }
                if (!isset($product['product_status'])) {
                    $product['product_status'] = a2wl_get_setting('default_product_status');
                }
        ...
        public function get_product_id_list($limit=null, $ofset=null) {
            ...
            if (a2wl_check_defined('A2WL_SAVE_TRANSIENT_AS_OPTION')) {
                $results = $wpdb->get_results("select option_name from {$wpdb->options} where option_name like 'a2wl_product#%' order by option_id $limitStr", ARRAY_A);
            } else {
                $results = $wpdb->get_results("select option_name from {$wpdb->options} where option_name like '_transient_a2wl_product#%' order by option_id $limitStr", ARRAY_A);
            }
            $ids = array();
        ...
        public function sort_list() {
            return array('id-asc'=>__('Sort by External Id', 'ali2woo-lite'),'title-asc'=>__('Sort by Product title', 'ali2woo-lite'),'date_add-asc'=>__('Sort by Date add (old first)', 'ali2woo-lite'),'date_add-desc'=>__('Sort by Date add (new first)', 'ali2woo-lite'));
        }
