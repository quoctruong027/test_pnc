...
/**
 * Description of A2WL_Override
 *
 * @author Andrey
 */

if (!class_exists('A2WL_Override')) {
    class A2WL_Override {
        ...
        public function override($product_id, $external_id, $override_images=false, $override_title_description=false, $order_variations=array()){
            global $wpdb;
            ...
            if($this->has_override($product_id)){
                $result = array(
                    "state"=>"error", 
                    "message"=>__("You've already selected to override this product. Check your import list and confirm the override to continue.", "ali2woo-lite")
                );
            }else{
                ...
                if($override_product){
                    ...
                    if($product){
                        ...
                        $product['override_order_variations'] = $order_variations;
        ...
        public function cancel_override($external_id){
            ...
            if($product){
                ...
                unset($product['override_order_variations']);
        ...
        public function override_message($product_id, $product_title){
            $msg = sprintf(__('This product will override <a href="%s">%s</a> Click "Override" to proceed.', 'ali2woo-lite'), 
                admin_url('post.php?post='.intval($product_id).'&action=edit'), 
                $product_title
            );
            $btn = '<button class="btn btn-default cancel-override" type="button">'.__('Cancel Override', 'ali2woo-lite').'</button>';
