...
/**
 * Description of A2WL_ShippingPriceFormula
 *
 * @author Mikhail
 */
class A2WL_ShippingPriceFormula {
    ...
    public function load($id = false) {
        ...
        if ($load_id) {
            $formula_list = A2WL_ShippingPriceFormula::load_formulas_list(false);
    ...
    public function save() {
        ...
        a2wl_set_setting('shipping_formula_list', array_values($formula_list));
        return $this;
    }
    ...
    public static function deleteAll() {
        a2wl_del_setting('shipping_formula_list');
    }


    public static function load_formulas() {
        return A2WL_ShippingPriceFormula::load_formulas_list(true);
    }

    private static function load_formulas_list($asObject = true) {
        $result = array();
        $formula_list = a2wl_get_setting('shipping_formula_list');
        $formula_list = $formula_list && is_array($formula_list) ? $formula_list : array();
        if ($asObject) {
            foreach ($formula_list as $formula) {
                $fo = new A2WL_ShippingPriceFormula();
    ...
    public static function get_default_formula() {
        $formula = a2wl_get_setting('shipping_default_formula');
        return new A2WL_ShippingPriceFormula($formula && is_array($formula) ? $formula : array('value' => 1, 'sign' => '*'));
    }

    public static function set_default_formula($formula) {
        
        a2wl_set_setting('shipping_default_formula', get_object_vars($formula));
    }
    ...
    public static function allow_price_rule(){
        return a2wl_get_setting('aliship_frontend', false);
    }
    
    public static function allow_post_price_rule($post_id){
         $use_price_rule = get_post_meta($post_id, 'a2wl_use_price_rule', true);       
