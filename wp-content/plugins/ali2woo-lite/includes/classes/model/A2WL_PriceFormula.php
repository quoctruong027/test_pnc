...
/**
 * Description of A2WL_PriceFormula
 *
 * @author Andrey
 */
class A2WL_PriceFormula {
    ...
    public static function pricing_rules_types(){
        return array( 
            'sale_price_and_discount'=>array(  
                ...
                'name'=>__('Use sale price and discount', 'ali2woo-lite'), 
                'description'=> __('Ali2Woo Lite gets your <strong>sale price</strong> by applying a pricing formula to the AliExpress sale price. Your <strong>regular price</strong> is calculated using the <i>Compared At Price</i> part of the formula.<br/> If you disable that part, the <strong>regular price</strong> is formed from your calculated sale price increased by the original AliExpress discount.<br/> To learn more check out <a href="https://ali2woo.com/codex/pricing-markup-formula/">this article</a>', 'ali2woo-lite')
            ),
            'sale_price_as_base'=>array(
                'value'=>'sale_price_as_base', 
                'name'=>__('Use sale price as base', 'ali2woo-lite'), 
                'description'=> __('Your pricing formula is applied to the AliExpress sale price. To learn more check out <a href="https://ali2woo.com/codex/pricing-markup-formula/">this article</a>', 'ali2woo-lite')
            ),
            'regular_price_as_base'=>array(
                'value'=>'regular_price_as_base', 
                'name'=>__('Use regular price as base', 'ali2woo-lite'), 
                'description'=>__('Your pricing formula is applied to the AliExpress regular price. To learn more check out <a href="https://ali2woo.com/codex/pricing-markup-formula/">this article</a>', 'ali2woo-lite')
            )
        );
    }
    ...
    public function load($id = false) {
        ...
        if ($load_id) {
            $formula_list = A2WL_PriceFormula::load_formulas_list(false);
    ...
    public function save() {
        ...
        a2wl_set_setting('formula_list', array_values($formula_list));
        return $this;
    }
    ...
    public static function apply_formula($product, $round = 2, $type = 'all') {
        $pricing_rules_type = a2wl_get_setting('pricing_rules_type');

        if (a2wl_check_defined('A2WL_USE_SEPARATE_FORMULA')) {
            $formula = A2WL_PriceFormula::get_formula_by_product($product, "price");
            $formula_regular = A2WL_PriceFormula::get_formula_by_product($product, "regular_price");
        }else{
            $price_type = $pricing_rules_type=='regular_price_as_base' ? "regular_price" : "price";
            $formula = A2WL_PriceFormula::get_formula_by_product($product, $price_type);
            $formula_regular = A2WL_PriceFormula::get_formula_by_product($product, $price_type);
        }

        $product_price = A2WL_PriceFormula::normalize_product_price($product);
        
        if ($formula && $formula_regular && $product_price['price']) {
            $use_compared_price_markup = a2wl_get_setting('use_compared_price_markup');
            $price_cents = a2wl_get_setting('price_cents');
            $price_compared_cents = a2wl_get_setting('price_compared_cents');
            ...
            if ($type === 'all' || $type === 'price' || !isset($product['calc_price'])) {
                ...
                if ($formula->sign == "=") {
                    ...
                    $product['calc_price'] = round(floatval($price) * floatval($formula->value), $round);
                } else if ($formula->sign == "+") {
                    $product['calc_price'] = round(floatval($price) + floatval($formula->value), $round);
                }
                ...
                if($pricing_rules_type=='sale_price_and_discount'){
                    ...
                    if ($use_compared_price_markup) {
                        if ($formula_regular->compared_sign == "=") {
                            $product['calc_regular_price'] = round(floatval($formula_regular->compared_value), $round);
                        } else if ($formula_regular->compared_sign == "*") {
                            $product['calc_regular_price'] = round(floatval($product_price['price']) * floatval($formula_regular->compared_value), $round);
                        } else if ($formula_regular->compared_sign == "+") {
                            $product['calc_regular_price'] = round(floatval($product_price['price']) + floatval($formula_regular->compared_value), $round);
                        }
                    }
                    ...
                    if ($use_compared_price_markup) {
                        if ($formula_regular->compared_sign == "=") {
                            ...
                            $product['calc_regular_price'] = round(floatval($price) * floatval($formula_regular->compared_value), $round);
                        } else if ($formula_regular->compared_sign == "+") {
                            $product['calc_regular_price'] = round(floatval($price) + floatval($formula_regular->compared_value), $round);
                        }
                    }else{
                        if ($formula_regular->sign == "=") {
                            $product['calc_regular_price'] = round(floatval($formula_regular->value), $round);
                        } else if ($formula_regular->sign == "*") {
                            $product['calc_regular_price'] = round(floatval($price) * floatval($formula_regular->value), $round);
                        } else if ($formula_regular->sign == "+") {
                            $product['calc_regular_price'] = round(floatval($price) + floatval($formula_regular->value), $round);
                        }
                    }
            ...
            if (isset($product['sku_products']['variations']) && $product['sku_products']['variations']) {
                foreach ($product['sku_products']['variations'] as &$var) {
                    if (a2wl_check_defined('A2WL_USE_SEPARATE_FORMULA')) {
                        $formula = A2WL_PriceFormula::get_formula_by_product($var, "price");
                        $formula_regular = A2WL_PriceFormula::get_formula_by_product($var, "regular_price");
                    }else{
                        $price_type = $pricing_rules_type=='regular_price_as_base' ? "regular_price" : "price";
                        $formula = A2WL_PriceFormula::get_formula_by_product($var, $price_type);
                        $formula_regular = A2WL_PriceFormula::get_formula_by_product($var, $price_type);
                    }
                    ...
                    if ($formula && $formula_regular) {
                        if ($type === 'all' || $type === 'price' || !isset($var['calc_price'])) {
                            ...
                            if ($formula->sign == "=") {
                                ...
                                $var['calc_price'] = round(floatval($price) * floatval($formula->value), $round);
                            } else if ($formula->sign == "+") {
                                $var['calc_price'] = round(floatval($price) + floatval($formula->value), $round);
                            }
                        ...
                        if ($type === 'all' || $type === 'regular_price' || !isset($var['calc_regular_price'])) {
                            ...
                            if($pricing_rules_type=='sale_price_and_discount'){
                                ...
                                if ($use_compared_price_markup) {
                                    if ($formula_regular->compared_sign == "=") {
                                        $var['calc_regular_price'] = round(floatval($formula_regular->compared_value), $round);
                                    } else if ($formula_regular->compared_sign == "*") {
                                        $var['calc_regular_price'] = round(floatval($var['price']) * floatval($formula_regular->compared_value), $round);
                                    } else if ($formula_regular->compared_sign == "+") {
                                        $var['calc_regular_price'] = round(floatval($var['price']) + floatval($formula_regular->compared_value), $round);
                                    }
                                }
                                ...
                                if ($use_compared_price_markup) {
                                    if ($formula_regular->compared_sign == "=") {
                                        ...
                                        $var['calc_regular_price'] = round(floatval($price) * floatval($formula_regular->compared_value), $round);
                                    } else if ($formula_regular->compared_sign == "+") {
                                        $var['calc_regular_price'] = round(floatval($price) + floatval($formula_regular->compared_value), $round);
                                    }
                                }else{
                                    if ($formula_regular->sign == "=") {
                                        $var['calc_regular_price'] = round(floatval($formula_regular->value), $round);
                                    } else if ($formula_regular->sign == "*") {
                                        $var['calc_regular_price'] = round(floatval($price) * floatval($formula_regular->value), $round);
                                    } else if ($formula_regular->sign == "+") {
                                        $var['calc_regular_price'] = round(floatval($price) + floatval($formula_regular->value), $round);
                                    }
                                }
    ...
    public static function get_default_formula() {
        $formula = a2wl_get_setting('default_formula');
        return new A2WL_PriceFormula($formula && is_array($formula) ? $formula : array('value' => 1, 'sign' => '*', 'compared_value' => 1, 'compared_sign' => '*'));
    }

    public static function set_default_formula($formula) {
        a2wl_set_setting('default_formula', get_object_vars($formula));
    }

    public static function get_default_formulas() {
        $f1 = new A2WL_PriceFormula();
        $f1->id = 1;
        $f1->min_price = 0;
        $f1->max_price = 10;
        $f1->sign = "*";
    ...
    public static function get_formula_by_product($product, $base_price_type="price") {
        $res_formula = false;
        $use_extended_price_markup = a2wl_get_setting('use_extended_price_markup');
        ...
        if ($use_extended_price_markup) {
            ...
            if($product_price){
                $formula_list = A2WL_PriceFormula::load_formulas_list();
                foreach ($formula_list as $formula) {
                    $check = true;
                ...
                a2wl_error_log("can't find normalize_product_price for ".$product['id']);
            }
        }

        return $res_formula ? $res_formula : A2WL_PriceFormula::get_default_formula();
    }
}
