...
/**
 * Description of A2WL_Woocommerce
 *
 * @author andrey
 */
if (!class_exists('A2WL_Woocommerce')) {

    class A2WL_Woocommerce {
        ...
        public function __construct() {
            $this->attachment_model = new A2WL_Attachment();
            $this->reviews_model = new A2WL_Review();
            $this->helper = new A2WL_Helper();
            $this->product_change_model = new A2WL_ProductChange();
        }
        ...
        public function build_steps($product) {
            ...
            $images_to_preload = A2WL_Utils::get_all_images_from_product($product, true, !$override_product || $override_images, !$override_product || $override_title_description);
        ...
        private function need_import_variations($product, $product_type = false) {
            $product_type = $product_type?$product_type:((isset($product['product_type']) && $product['product_type']) ? $product['product_type'] : a2wl_get_setting('default_product_type', 'simple'));
        ...
        private function prepare_product_images($product){
            ...
            $thumb_url = '';
            $tmp_all_images = A2WL_Utils::get_all_images_from_product($product);

            if (isset($product['thumb_id'])) {
                foreach ($tmp_all_images as $img_id => $img) {
                    if ($img_id === $product['thumb_id']) {
                        $thumb_url = A2WL_Utils::clear_url($img['image']);
                        break;
                    }
                }
            }
        ...
        public function add_product($product, $params = array()) {
            if (!A2WL_Woocommerce::is_woocommerce_installed()) {
                return A2WL_ResultBuilder::buildError("Woocommerce is not installed");
            }
            ...
            $product_type = (isset($product['product_type']) && $product['product_type']) ? $product['product_type'] : a2wl_get_setting('default_product_type', 'simple');
            $product_status = (isset($product['product_status']) && $product['product_status']) ? $product['product_status'] : a2wl_get_setting('default_product_status', 'publish');
            ...
            if($step === false || $step === 'init'){
                ...
                $post = array(
                    ...
                    'meta_input' => array('_stock_status' => 'instock',
                        ...
                        '_a2w_import_lang' => !empty($product['import_lang']) ? $product['import_lang'] : A2WL_AliexpressLocalizator::getInstance()->language
                    ),
                );
                ...
                if($override_product && $this->is_product_exist($product_id)){
                    ...
                    if($override_images) {
                        ...
                        foreach ($attachments as $attachment_id) {
                            A2WL_Utils::delete_attachment($attachment_id, true);
                        }
                    ...
                    foreach ($variations as $var_id) {
                        ...
                        $var->delete(true);
                        A2WL_Utils::delete_post_images($var_id);
                    }
                ...
                if (isset($product['attribute']) 
                    && $product['attribute'] 
                    && !a2wl_get_setting('not_import_attributes', false)
                ...
                $default_shipping_class = a2wl_get_setting('default_shipping_class');
                if ($default_shipping_class) {
                    wp_set_object_terms($product_id, intval($default_shipping_class), 'product_shipping_class');
                }

                if($step !== false) return $result = A2WL_ResultBuilder::buildOk(array('product_id'=>$product_id, 'step'=>$step));
            }
            
            if($step !== false && !$this->is_product_exist($product_id)){
                return A2WL_ResultBuilder::buildError("Error! Processing processing product($product_id) not found");
            }

            if(substr($step, 0, strlen('preload_images')) === 'preload_images'){
                $images_to_preload = A2WL_Utils::get_all_images_from_product($product, true);
                $cnt = 0;
                foreach($images_to_preload as $img_id=>$image){
                    $cnt++;
                ...
                return A2WL_ResultBuilder::buildOk(array('product_id'=>$product_id, 'step'=>$step));
            }
            ...
            if($step === false || substr($step, 0, strlen('variations')) === 'variations'){
                ...
                if($step !== false) return A2WL_ResultBuilder::buildOk(array('product_id'=>$product_id, 'step'=>$step));
            }
            ...
            if($step === false || $step === 'finishing'){
                ...
                if (a2wl_get_setting('load_review')) {
                ...
                A2WL_Utils::update_post_terms_count($product_id);

                if($step !== false) return apply_filters('a2wl_woocommerce_after_add_product', A2WL_ResultBuilder::buildOk(array('product_id' => $product_id, 'step'=>$step)), $product_id, $product, $params);
            }
            return apply_filters('a2wl_woocommerce_after_add_product', A2WL_ResultBuilder::buildOk(array('product_id' => $product_id)), $product_id, $product, $params);
        }
        ...
        public function upd_product($product_id, $product, $params = array()) {
            ...
            $on_not_available_product = a2wl_get_setting('on_not_available_product');
            $on_not_available_variation = a2wl_get_setting('on_not_available_variation');
            ...
            if (a2wl_check_defined('A2WL_FIX_RELOAD_IMAGES') && isset($product['images'])) {
                ...
                if ($old_thumb_id) {
                    A2WL_Utils::delete_attachment($old_thumb_id, true);
                    delete_post_meta($product_id, '_thumbnail_id');
                }
                ...
                if ($old_image_gallery) {
                    ...
                    foreach ($image_ids as $image_id) {
                        A2WL_Utils::delete_attachment($image_id, true);
                    }
                    delete_post_meta($product_id, '_product_image_gallery');
                }
        ...
        public function build_description($product_id, $product) {
            ...
            if(class_exists('DOMDocument')){
                ...
                for ($i = $elements->length; --$i >= 0;) {
                    ...
                    } else if (!a2wl_get_setting('use_external_image_urls')) {
                        $tmp_title = isset($product['title']) && $product['title'] ? $product['title'] : "Product " . $product['id'];

                        // if have edited image, than user initial url
                        $clear_image_url = !empty($product['tmp_edit_images'][$img_id]) ? $e->getAttribute('src') : A2WL_Utils::clear_image_url($e->getAttribute('src'));
        ...
        private function set_attributes($product_id, $attributes) {
            if (defined('A2WL_IMPORT_EXTENDED_ATTRIBUTE')) {
                ...
                $extended_attribute = a2wl_get_setting('import_extended_attribute');
            }
        ...
        private function add_variation($product_id, $product, $is_update = false, $params=array()) {
            ...
            $on_new_variation_appearance = $disable_add_new_variants ? "nothing" : a2wl_get_setting('on_new_variation_appearance');

            $localCurrency = strtoupper(a2wl_get_setting('local_currency'));
            ...
            if ($localCurrency) {
                ...
                $currency_conversion_factor = floatval(a2wl_get_setting('currency_conversion_factor'));
            }
            ...
            foreach ($variations['attributes'] as $key => $attr) {
                ...
                foreach ($attr['value'] as &$val) {
                    ...
                    if (a2wl_get_setting('use_external_image_urls')){
                    ...
                    if (($step === false || $step === 'variations#attributes') && $attr_image) {
                        ...
                        $attr_image = A2WL_Utils::clear_image_url($attr_image);
                        ...
                        if ($is_update && $RELOAD_ATTR_IMAGES) {
                            if (intval($old_attachment_id) > 0) {
                                A2WL_Utils::delete_attachment($old_attachment_id, true);
                            }
        ...
        public function save_tracking_code($order_id, $tracking_codes) {
            ...
            if (!$order) {
                $result = A2WL_ResultBuilder::buildError(_x('Didn`t find the Woocommerce order №', 'Error text', 'ali2woo-lite') . $order_id);
            } else {
                ...
                $tracking_code_order_status = a2wl_get_setting('tracking_code_order_status');
        ...
        function get_product_by_post_id($post_id, $with_vars = true) {
            ...
            if ($external_id) {
                ...
                $product['import_lang'] = $import_lang ? $import_lang : A2WL_AliexpressLocalizator::getInstance()->language;
