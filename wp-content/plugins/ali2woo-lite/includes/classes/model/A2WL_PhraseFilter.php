...
/**
 * Description of A2WL_PhraseFilter
 *
 * @author ma_group
 */
class A2WL_PhraseFilter {
    ...
    public function save() {
        A2WL_PhraseFilter::$_instance_objects = null;
        A2WL_PhraseFilter::$_instance_array = null;
        
        $formula_list = A2WL_PhraseFilter::load_phrase_list(false);
        ...
        a2wl_set_setting('phrase_list', array_values($formula_list));
        return $this;
    }
    ...
    public static function apply_filter_to_text($text, $phrase_list=array()) {
        ...
        if(!$phrase_list){
            $phrase_list = A2WL_PhraseFilter::load_phrase_list(false);
        }
    ...
    public static function apply_filter_to_products() {
        ...
        foreach ($tmp_product_ids as $row) {
            $new_title = A2WL_PhraseFilter::apply_filter_to_text($row[1], $phrase_list);
            $new_content = A2WL_PhraseFilter::apply_filter_to_text($row[2], $phrase_list);
    ...
    public static function apply_filter_to_reviews() {
        ...
        $tmp_comment_ids = $wpdb->get_results("SELECT c.comment_ID, c.comment_author, c.comment_content from {$wpdb->comments} c LEFT JOIN {$wpdb->commentmeta} as cm ON cm.comment_id = c.comment_ID WHERE (".implode(" OR ", $phrase_query_array).") AND cm.meta_key = 'a2wl_country'", ARRAY_N);
        foreach ($tmp_comment_ids as $row) {
            $new_author = A2WL_PhraseFilter::apply_filter_to_text($row[1], $phrase_list);
            $new_content = A2WL_PhraseFilter::apply_filter_to_text($row[2], $phrase_list);
    ...
    public static function deleteAll() {
        a2wl_del_setting('phrase_list');
    }

    public static function load_phrases() {
        return A2WL_PhraseFilter::load_phrase_list(true);
    }

    private static function load_phrase_list($asObject = true) {
        $result = array();

        if ($asObject && A2WL_PhraseFilter::$_instance_objects) {
            return A2WL_PhraseFilter::$_instance_objects;
        } else if (!$asObject && A2WL_PhraseFilter::$_instance_array) {
            return A2WL_PhraseFilter::$_instance_array;
        }
        
        $formula_list = a2wl_get_setting('phrase_list');
        $formula_list = $formula_list && is_array($formula_list) ? $formula_list : array();

        if ($asObject) {
            foreach ($formula_list as $formula) {
                $fo = new A2WL_PhraseFilter();
                ...
                $result[] = $fo;
                A2WL_PhraseFilter::$_instance_objects = $result;
            }
        } else {
            $result = $formula_list;
            A2WL_PhraseFilter::$_instance_array = $result;
        }
...
if (!function_exists('a2wl_phrase_apply_filter_to_text')) {

    function a2wl_phrase_apply_filter_to_text($text) {
        return A2WL_PhraseFilter::apply_filter_to_text($text);
    }
