...
/**
 * Description of A2WL_Attachment
 *
 * @author Andrey
 */
if (!class_exists('A2WL_Attachment')) {

    class A2WL_Attachment {

        private $utils;
        private $use_external_image_urls = false;

        public function __construct($use_external = 'cfg') {
            $this->utils = new A2WL_Utils();
            if ('external' === $use_external || 'local' === $use_external) {
                $this->use_external_image_urls = ('external' === $use_external);
            } else {
                $this->use_external_image_urls = a2wl_get_setting('use_external_image_urls');
            }
        }
        ...
        public function create_attachment($post_id, $image_path, $params = array()) {
            ...
            $image_path = A2WL_Utils::clear_image_url($image_path);
        ...
        public function create_attachment_from_data($post_id, $file_data, $params = array()) {
            ...
            $ext = A2WL_Attachment::mime2ext($matches[1]);
            if (!$ext) {
        ...
        private function set_attachment_metadata($attach_id, $image_url) {
            update_post_meta($attach_id, '_wp_a2wl_attached_file', 1);
        ...
        public static function find_products_with_external_images() {
            global $wpdb;
            $result_ids = array();
            $tmp_product_ids = $wpdb->get_results("select distinct if(p.post_parent = 0, p.id, p.post_parent) as id from $wpdb->posts p, (select distinct p1.post_parent as id from $wpdb->posts p1 LEFT join $wpdb->postmeta pm1 on(p1.id = pm1.post_id) where p1.post_type = 'attachment' and pm1.meta_key='_wp_a2wl_attached_file' and pm1.meta_value='1') pp WHERE p.ID = pp.id", ARRAY_N);
        ...
        public static function calc_total_external_images() {
            global $wpdb;
            $cnt = $wpdb->get_var("select count(id) from (select distinct p1.id as id from $wpdb->posts p1 LEFT join $wpdb->postmeta pm1 on(p1.id = pm1.post_id) where p1.post_type = 'attachment' and pm1.meta_key='_wp_a2wl_attached_file' and pm1.meta_value='1') as pp");
        ...
        public static function find_external_images($page_size = 1000) {
            global $wpdb;
            $result_ids = array();
            $tmp_product_ids = $wpdb->get_results("select distinct p1.id as id from $wpdb->posts p1 LEFT join $wpdb->postmeta pm1 on(p1.id = pm1.post_id) where p1.post_type = 'attachment' and pm1.meta_key='_wp_a2wl_attached_file' and pm1.meta_value='1' LIMIT $page_size", ARRAY_N);
            foreach ($tmp_product_ids as $row) {
                $result_ids[] = $row[0];
            }
        ...
        public function load_external_image($post_id) {
            ...
            if ($post_id) {
                ...
                if ($post->post_type === 'attachment') {
                    $tmp = get_post_meta($post_id, '_wp_a2wl_attached_file', true);
                    if ($tmp && intval($tmp) === 1) {
                        ...
                        if ($new_image_id) {
                            ...
                            A2WL_Utils::delete_attachment($post_id, true);
