...
/**
 * Description of A2WL_AliexpressAccount
 *
 * @author Andrey
 */
if (!class_exists('A2WL_AliexpressAccount')) {

    class A2WL_AliexpressAccount extends A2WL_AbstractAccount  {
        ...
        public function getDeeplink($hrefs) {
            $result = array();
            if ($hrefs) {
                $account = A2WL_Account::getInstance()->get_aliexpress_account();
                if (!empty($account['appkey']) && !empty($account['secretkey']) && !empty($account['trackingid'])) {
                    ...
                    if($resp->resp_result->resp_code == 200){
                        ...
                        a2wl_error_log('A2WL_AliexpressAccount::getDeeplink error: '.(!empty($resp->resp_result->resp_msg)?$resp->resp_result->resp_msg:'Unknown error'));
                    }
                }
            }
            return $result;
