...
/**
 * Description of A2WL_ShippingFrontendPageController
 *
 * @author MA_GROUP
 * @autoload: true
 *
 */
if (!class_exists('A2WL_ShippingFrontendPageController')):

    class A2WL_ShippingFrontendPageController extends A2WL_AbstractController {
        ...
        function __construct() {
            ...
            if (a2wl_get_setting('aliship_frontend') && A2WL_Woocommerce::is_woocommerce_installed() ) {
        ...
        public function init() {
            ...
            $a2wl_shipping = new A2WL_ShippingMethod();
            if ($a2wl_shipping->use_title === "yes")
        ...
        public function validate_postcode ($valid, $postcode, $country){
            return isset($_POST['a2wl_skip_postcode_validate'])?true:$valid;
        }
        ...
        public function render_country_dropdown_in_cart() {
            $countries = array_merge(array('' => __('Select a Country', 'ali2woo-lite')), $this->get_countries());
        ...
        public function render_shipping_dropdown_cart($cart_item = null, $cart_item_key = null) {
            if ($cart_item_key && is_cart()) {
                ...
                if($ext_id){
                    ...
                    $shipping_data = $this->shipping_loader->load(new A2WL_ShippingMeta($product_id, $ext_id, $ship_to, $quantity, $price, $price));
                    ...
                    if (!empty($shipping_methods)) {
                        ...
                        foreach ($shipping_methods as $method) {
                            ...
                            $normalized_shipping_methods[$method['serviceName']] = $method['company'] . ", " . $method['time'] . " " . __('days', 'ali2woo-lite') . ", " . ($method['price'] > 0 ? $method['price'] . " " . $method['currency'] : __('free shipping', 'ali2woo-lite'));
                        }
                    ...
                    $woocommerce->cart->cart_contents[$cart_item_key]['a2wl_shipping_method'] = $default_shipping_method;
        ...
        function assets() {
            if (/*is_singular('product') || */is_cart() || is_checkout()) {
                wp_enqueue_script('a2wl-aliexpress-shipping-script', A2WL()->plugin_url() . '/assets/js/shipping.js', array(), A2WL()->version, true);

                wp_enqueue_style('a2wl-aliexpress-shipping-product-style', A2WL()->plugin_url() . '/assets/css/shipping.css', array(), A2WL()->version);

                $script_data = array(
                    'lang' => array(
                        'select_shipping_method' => __('Select a Shipping method...', 'ali2woo-lite'),
                        'shipping_country_should_be_the_same' => __('Shipping country should be the same for all items in your order. You can reset it, if clear your shopping cart.', 'ali2woo-lite')),
                    'ajaxurl' => admin_url('admin-ajax.php'),
                );
                wp_localize_script('a2wl-aliexpress-shipping-script', 'a2wl_ali_ship_data', $script_data);
            }
        }

        public function add_order_item_data_legacy($item_id, $values) {

            if (isset($values['a2wl_shipping_method'])) {
                $service_name = $values['a2wl_shipping_method'];
                $shipping_title = A2WL_ShippingPostType::get_item(false, $service_name);
                ...
                woocommerce_add_order_item_meta($item_id, 'a2wl_shipping_code', $service_name);
            }
        }
        ...
        public function add_order_item_data($item_id, $order_item) {
            ...
            if ( isset($order_item->legacy_values) ){
                ...
                if (isset($values['a2wl_shipping_method'])) {
                    ...
                    wc_add_order_item_meta($item_id, 'a2wl_shipping_code', $service_name);
                }
            }
        ...
        public function display_item_meta($html, $item, $args){
            ...
            $a2wl_formatted_key    = rawurldecode( 'a2wl_shipping_code');
            $a2wl_attribute_key = str_replace( 'attribute_', '', $a2wl_formatted_key );
            $a2wl_display_key   = wc_attribute_label( $a2wl_attribute_key, $product );

            foreach ( $item->get_formatted_meta_data() as $meta_id => $meta ) {
                if ($meta->display_key != $a2wl_display_key) {
        ...
        private function get_default_shipping_country() {
            ...
            if (is_null($country) || empty($country)) {
                $country = $_POST['calc_shipping_country'] = a2wl_get_setting('aliship_shipto');
