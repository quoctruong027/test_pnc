...
/**
 * Description of A2WL_ShippingAjaxConroller
 *
 * @author MA_GROUP
 * @autoload: true
 */
if (!class_exists('A2WL_ShippingAjaxConroller')):

    class A2WL_ShippingAjaxConroller extends A2WL_AbstractController {
        ...
        function __construct() {
            ...
            add_action('wp_ajax_a2wl_update_shipping_method_in_cart_item', array($this, 'update_shipping_method_in_cart_item'));

            add_action('wp_ajax_nopriv_a2wl_update_shipping_method_in_cart_item', array($this, 'update_shipping_method_in_cart_item'));

            //backend only: 
            add_action('wp_ajax_a2wl_aliship_get_products_by_filter', array($this, 'get_products_by_filter'));

            add_filter('a2wl_ajax_product_info', array($this, 'product_info'), 4, 9000);
        }
        ...
        public function update_shipping_method_in_cart_item() {
            ...
            foreach ($cart as $key => $item) {
                if ($key == $cart_item_key) {
                    $woocommerce->cart->cart_contents[$key]['a2wl_shipping_method'] = $tariff_code;
                    $woocommerce->cart->maybe_set_cart_cookies();
              
                     A2WL_ShippingFrontendPageController::update_shipping_and_totals_in_cart();
        ...
        public function get_products_by_filter() {
            ...
            $loader = (isset($filter['type']) && $filter['type']) ? a2wl_get_loader($filter['type']) : false;

            if ($loader) {
                $upload_dir = wp_upload_dir();
                $file_url = $upload_dir['basedir'] . "/a2wl_aliship_import.csv";
