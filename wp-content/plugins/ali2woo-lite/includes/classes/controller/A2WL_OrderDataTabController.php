<?php
/* * class
 * Description of A2WL_OrderDataTabController
 *
 * @author MA_GROUP
 * 
 * @autoload: a2wl_init
 */
 if (!class_exists('A2WL_OrderDataTabController') ) {

    class A2WL_OrderDataTabController {
        private $woocommerce_model;

        public function __construct() {
            
            if ( !A2WL_Utils::wcae_strack_active() ) {
                ...
                add_action('wp_ajax_a2wl_add_order_id_manually', array($this, 'ajax_add_order_id_manually'));
                add_action('wp_ajax_a2wl_delete_order_id', array($this, 'ajax_delete_order_id'));
                                  
                add_action('wp_ajax_a2wl_delete_tracking_codes', array($this, 'ajax_delete_tracking_codes'));
                add_action('wp_ajax_a2wl_add_tracking_codes_manually', array($this, 'ajax_add_tracking_codes_manually'));
        ...
        public function add_tab( $order ){ ?>
            <br class="clear" />
            <h3><?php _e('A2WL Data', 'ali2woo-lite') ?><a href="#" class="edit_address"><?php _e('Edit') ?></a></h3>
            <?php
            ...
            <?php _e('AliExpress order ID(s)', 'ali2woo-lite') ?>: <?php /* echo $order_external_id */?>
            ...
            <?php endforeach; ?>
             ...
             <a role="button" class="a2wl_delete_order_id" href="#"><?php _e('Delete order ID(s)', 'ali2woo-lite') ?></a><br/>
            <?php endif; ?>
               <a role="button" class="a2wl_order_id_manually" href="#"><?php _e('Add order ID(s) manually', 'ali2woo-lite') ?></a><br/><br/>  
            
            <?php if ($order_tracking_codes) : ?>         
            <?php _e('AliExpress tracking numbers', 'ali2woo-lite') ?>:
            ...
            <a role="button" class="a2wl_delete_codes" href="#"><?php _e('Delete tracking numbers', 'ali2woo-lite') ?></a><br/>
          
        
            <?php endif; ?>
              <a role="button" class="a2wl_codes_manually" href="#"><?php _e('Add tracking numbers manually', 'ali2woo-lite') ?></a>
            </div>
            <div class="edit_address"><?php
                
                foreach ($order_external_id_array as $k => $order_external_id) : 
                    woocommerce_wp_text_input( array(
                        'id' => 'a2wl_external_order_id',
                        'name' => 'a2wl_external_order_id[]',
                        'label' => __('AliExpress Order ID', 'ali2woo-lite') . ' ' . ($k+1),
                        'value' => $order_external_id,
                        'wrapper_class' => 'form-field-wide a2wl_tracking_code_id'
                    ) );
                ...
                foreach ($order_tracking_codes as $k => $tracking_code) : 
                    ...
                    woocommerce_wp_text_input( array(
                        'id' => 'a2wl_tracking_code',
                        'name' => 'a2wl_tracking_code[]',
                        'label' => __('Tracking number', 'ali2woo-lite') . ' ' . ($k+1) ,
                        'value' => $tracking_code,
                        'wrapper_class' => 'form-field-wide a2wl_tracking_code_data'
                    ) );
        ...
        public function save_tab_data($ord_id){
           /* if (isset($_POST[ 'a2wl_external_order_id' ]))
                update_post_meta( $ord_id, '_a2w_external_order_id', wc_clean( $_POST[ 'a2wl_external_order_id' ] ) ); 
            */
            if (isset($_POST[ 'a2wl_external_order_id' ])){
                delete_post_meta($ord_id, '_a2w_external_order_id');
                $this->add_external_order_id($ord_id, $_POST[ 'a2wl_external_order_id' ]);
            } else {
                delete_post_meta($ord_id, '_a2w_external_order_id');
            }
            
            if (isset($_POST[ 'a2wl_tracking_code' ])){
                delete_post_meta($ord_id, '_a2w_tracking_code');
                $this->add_tracking_codes($ord_id, $_POST[ 'a2wl_tracking_code' ]);
            } else {
                delete_post_meta($ord_id, '_a2w_tracking_code');
            }
        }
