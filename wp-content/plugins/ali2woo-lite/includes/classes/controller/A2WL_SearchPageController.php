...
/**
 * Description of A2WL_SearchPage
 *
 * @author andrey
 * 
 * @autoload: a2wl_init
 */
if (!class_exists('A2WL_SearchPageController')) {

    class A2WL_SearchPageController extends A2WL_AbstractAdminPage {

        private $loader;
        private $product_import_model;

        public function __construct() {
            parent::__construct(__('Search Products', 'ali2woo-lite'), __('Search Products', 'ali2woo-lite'), 'import', 'a2wl_dashboard', 10);
            ...
            add_action('wp_ajax_a2wl_add_to_import', array($this, 'ajax_add_to_import'));
            add_action('wp_ajax_a2wl_remove_from_import', array($this, 'ajax_remove_from_import'));
            add_action('wp_ajax_a2wl_load_shipping_info', array($this, 'ajax_load_shipping_info'));
        }
        ...
        public function render($params = array()) {
            ...
            if (!empty($_REQUEST['a2wl_search'])) {
                ...
                $load_products_result = A2WL_ResultBuilder::buildError(__('Please enter some search keywords or select item from category list!', 'ali2woo-lite'));
            }
        ...
        public function ajax_add_to_import() {
            if (isset($_POST['id'])) {
                ...
                if (a2wl_check_defined('A2WL_ALLOW_PRODUCT_DUPLICATION') || !$post_id) {
                    ...
                    if ($res['state'] !== 'error') {
                        ...
                        if ($product) {
                            ...
                            echo json_encode(A2WL_ResultBuilder::buildError("Product not found in serach result"));
                        }
                    } else {
                        echo json_encode($res);
                    }
                } else {
                    echo json_encode(A2WL_ResultBuilder::buildError("Product already imported."));
                }
            } else {
                echo json_encode(A2WL_ResultBuilder::buildError("add_to_import: waiting for ID..."));
            }
            wp_die();
        }
        ...
        public function ajax_remove_from_import() {
            if (isset($_POST['id'])) {
                ...
                echo json_encode(A2WL_ResultBuilder::buildError("remove_from_import: waiting for ID..."));
            }
            wp_die();
        }
        ...
        public function ajax_load_shipping_info() {
            if (isset($_POST['id'])) {
                ...
                $product = false;
                $products = $page === 'search' ? a2wl_get_transient('a2wl_search_result') : array();
                $result = array();
                foreach ($ids as $id) {
                    ...
                    foreach ($products as &$product) {
                        if ($product['id'] == $id) {
                            ...
                            if ($country && (!isset($product['shipping_info']) || $country != $product_country)) {
                                ...
                                $res = $this->loader->load_shipping_info($product['id'], 1, $country,'', $product['price_min'], $product['price_max']);
                                if ($res['state'] !== 'error') {
                                    $product['shipping_info'] = $res['items'];
                                    $product['shipto'] = $res['items'];
                                } else {
                ...
                if($page === 'search'){
                    a2wl_set_transient('a2wl_search_result', $products);
                }

                echo json_encode(A2WL_ResultBuilder::buildOk(array('products' => $result)));
            } else {
                echo json_encode(A2WL_ResultBuilder::buildError("load_shipping_info: waiting for ID..."));
            }
            wp_die();
        }
