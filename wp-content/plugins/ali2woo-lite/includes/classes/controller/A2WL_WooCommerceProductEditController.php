...
/* * class
 * Description of A2WL_WooCommerceProductEditController
 *
 * @author andrey
 * 
 * @autoload: a2wl_init
 */
if (!class_exists('A2WL_WooCommerceProductEditController')) {

    class A2WL_WooCommerceProductEditController extends A2WL_AbstractController {
        ...
        function current_screen($current_screen) {
            if ($current_screen->in_admin() && ($current_screen->id == 'product' || $current_screen->id == 'ali2woo-lite_page_a2wl_import')) {
                ...
                wp_enqueue_script('tui-image-editor-fabric', A2WL()->plugin_url() . '/assets/js/image-editor/fabric.js', array('jquery'), A2WL()->version);
                wp_enqueue_script('tui-code-snippet', A2WL()->plugin_url() . '/assets/js/image-editor/tui-code-snippet.min.js', array('jquery'), A2WL()->version);
                ...
                $data = array(
                    'a2wl_cdn_url' => A2WL()->plugin_url() . '/includes/cdn.php',
                );

                wp_localize_script('a2wl-wc-pe-script', 'a2wl_wc_pe_script_data', $data);
            }
        }

        function edit_form_advanced($post) {
            $current_screen = get_current_screen();
            if ($current_screen && $current_screen->in_admin() && ($current_screen->id == 'product' || $current_screen->id == 'ali2woo-lite_page_a2wl_import')) {
        ...
        function ajax_get_image_by_id(){
            if (empty($_POST['attachment_id'])) {
                $result = A2WL_ResultBuilder::buildError("waiting for attachment_id...");
            } else {
                $image_url = wp_get_attachment_url($_POST['attachment_id']);
                if(!$image_url){
                    $result = A2WL_ResultBuilder::buildError("waiting for attachment_id...");
                }else{
                    $result = A2WL_ResultBuilder::buildOk(array('image_url' => $image_url));
                }
            }
            ...
            if (empty($_POST['view']) || !in_array($_POST['view'], array('product', 'import'))) {
                ...
                if ($view == 'product') {
                    ...
                    if (is_wp_error($new_attachment_id)) {
                        $result = A2WL_ResultBuilder::buildError($new_attachment_id->get_error_message($new_attachment_id->get_error_code()));
                    } else {
                        ...
                        $result = A2WL_ResultBuilder::buildOk(array('attachment_id'=>$new_attachment_id, 'attachment_url' => wp_get_attachment_url($new_attachment_id), 'croped_attachment_url' => $src?$src[0]:''));
                    }
                } else if ($view == 'import') {
                    ...
                    if (is_wp_error($new_attachment_id)) {
                        ...
                        if (isset($product['tmp_edit_images'][$attachment_id]['attachment_id'])) {
                            A2WL_Utils::delete_attachment($product['tmp_edit_images'][$attachment_id]['attachment_id'], true);
                        }
                        ...
                        $result = A2WL_ResultBuilder::buildOk(array('image_id'=>$attachment_id, 'attachment_id'=>$new_attachment_id, 'attachment_url' => $product['tmp_edit_images'][$attachment_id]['attachment_url']));
                    }
                }
            }
            ...
            if ($_FILES) {
                foreach ($_FILES as $file => $array) {
                    if ($_FILES[$file]['error'] !== UPLOAD_ERR_OK) {
                        $result = A2WL_ResultBuilder::buildError("upload_sticker_error : " . $_FILES[$file]['error']);
                    }else{
                        ...
                        if ($movefile && !isset($movefile['error'])) {
                            ...
                            $result = A2WL_ResultBuilder::buildOk(array('sticker_url' => $movefile['url']));
                        } else {
                            $result = A2WL_ResultBuilder::buildError("upload_sticker_error: " . $movefile['error']);
                        }
                    }
                }
            }
