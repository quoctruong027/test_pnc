<?php
/**
 * Description of A2WL_ProductDataTabController
 *
 * @author Andrey
 * 
 * @autoload: a2wl_init
 */
if (!class_exists('A2WL_ProductDataTabController')) {

    class A2WL_ProductDataTabController extends A2WL_AbstractController {
        ...
        public function __construct() {
            ...
            add_action('wp_ajax_a2wl_data_remove_deleted_attribute', array($this, 'ajax_remove_deleted_attribute'));
            add_action('wp_ajax_a2wl_data_remove_deleted_variation', array($this, 'ajax_remove_deleted_variation'));
            add_action('wp_ajax_a2wl_data_last_update_clean', array($this, 'ajax_last_update_clean'));
        ...
        public function variation_options_pricing($loop, $variation_data, $variation) {
            if (!empty($variation_data['_aliexpress_regular_price']) || !empty($variation_data['_aliexpress_price'])) {
                ...
                if (!empty($variation_data['_aliexpress_regular_price'])) {
                    $label = sprintf(__('Aliexpress Regular price (%s)', 'ali2woo-lite'), get_woocommerce_currency_symbol());
                ...
                if (!empty($variation_data['_aliexpress_price'])) {
                    $label = sprintf(__('Aliexpress Sale price (%s)', 'ali2woo-lite'), get_woocommerce_currency_symbol());
