...
/**
 * Description of A2WL_SynchProductController
 *
 * @author Andrey
 * 
 * @autoload: a2wl_init
 */
if (!class_exists('A2WL_SynchProductController')) {

    class A2WL_SynchProductController extends A2WL_AbstractController {
        ...
        public function init_reccurences($schedules) {
            $schedules['a2wl_5_mins'] = array('interval' => 5 * 60, 'display' => __('Every 5 Minutes', 'ali2woo-lite'));
            $schedules['a2wl_15_mins'] = array('interval' => 15 * 60, 'display' => __('Every 15 Minutes', 'ali2woo-lite'));
            return $schedules;
        }
        ...
        public function synch_event_check() {
            // check: is a2wl_update_products_event, update_reviews_event and a2wl_auto_synch_event exist. if no, create it.

            if (!wp_next_scheduled('a2wl_update_products_event') && a2wl_get_setting('auto_update')) {
                $this->schedule_event();
            }

            if (!wp_next_scheduled('a2wl_update_reviews_event') && a2wl_get_setting('load_review') && a2wl_get_setting('review_status')) {
                $this->schedule_reviews_event();
            }

            if (!wp_next_scheduled('a2wl_auto_synch_event')) {
                $this->schedule_synch_event();
            }

            if (!wp_next_scheduled('a2wl_email_alerts_event') && a2wl_get_setting('auto_update') &&  a2wl_get_setting('email_alerts')) {
        ...
        public function install() {
            ...
            if (a2wl_get_setting('auto_update') && a2wl_get_setting('email_alerts')) {
                $this->schedule_email_alerts_event();
            }
        ...
        public function update_products_event() {
            if (!a2wl_get_setting('auto_update') || $this->is_process_running('a2wl_update_products_event')) {
                return;
            }
            ...
            try {
                ...
                $on_price_changes = a2wl_get_setting('on_price_changes');
                $on_stock_changes = a2wl_get_setting('on_stock_changes');
        ...
        public function email_alerts_event(){
            ...
            if (!a2wl_get_setting('auto_update') || !a2wl_get_setting('email_alerts') || $this->is_process_running('a2wl_email_alerts_event')) {
                return;
            }
            ...
            try {
                ...
                $to = a2wl_get_setting('email_alerts_email');
                
                if ($to){
  
                    $items = $this->product_change_model->get_all();
                    //we update data of product_change_model in the add_variation method in A2WL_Woocommerce
        ...
        private function send_email_alert($to, $items){
            ...
            $this->model_put("email_heading", __('Ali2Woo Lite report: Changes in your products', 'ali2woo-lite'));
            $this->model_put("email_subheading", __('Changes occurred in the last half-hour in your store', 'ali2woo-lite'));                          
            $this->model_put("items", $items);
            $this->model_put("email_footer_text", __('The report is generated by the Email alerts module in Ali2Woo Lite at', 'ali2woo-lite') . ' ' . date("F j, Y, g:i a"));             
            ...
            $result =  wc_mail($to, __('Ali2Woo Lite report: Changes in your products', 'ali2woo-lite'), $message);
        ...
        public function update_reviews_event() {
            if (!a2wl_get_setting('load_review') || !a2wl_get_setting('review_status') || $this->is_process_running('a2wl_update_reviews_event')) {
                return;
            }
        ...
        private function schedule_synch_event() {
            if (!($timestamp = wp_next_scheduled('a2wl_auto_synch_event'))) {
                wp_schedule_single_event(time() + HOUR_IN_SECONDS * 6, 'a2wl_auto_synch_event');
            }
        }

        private function unschedule_synch_event() {
            wp_clear_scheduled_hook('a2wl_auto_synch_event');
        }
