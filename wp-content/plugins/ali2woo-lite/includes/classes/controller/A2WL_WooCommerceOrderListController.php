<?php
/* * class
 * Description of A2WL_WooCommerceOrderListController
 *
 * @author MA_GROUP
 * 
 * @autoload: a2wl_init
 */
if (!class_exists('A2WL_WooCommerceOrderListController')) :

    class A2WL_WooCommerceOrderListController {
        ...
        public function __construct() {
            if (is_admin()) {
                ...
                add_action('wp_ajax_a2wl_order_info', array($this, 'ajax_order_info'));

                add_action('wp_ajax_a2wl_get_fulfilled_orders', array($this, 'ajax_get_fulfilled_orders'));

                add_action('manage_posts_extra_tablenav', array($this, 'add_bulk_order_sunc_button'));

                add_action('wp_ajax_a2wl_save_tracking_code', array($this, 'ajax_save_tracking_code'));
                ...
                list($this->bulk_actions, $this->bulk_actions_text) = apply_filters('a2wl_wcol_bulk_actions_init', array($this->bulk_actions, $this->bulk_actions_text));
            }
        }
            ...
            if (isset($_GET['post_type']) && $_GET['post_type'] == "shop_order") :
                ...
                if ($fulfilled_order_count > 0) :
                    ...
                    <div class="alignleft actions">
                        <?php submit_button(__("Tracking Sync All", 'ali2woo-lite'), 'primary', 'a2wl_bulk_order_sync_manual', false); ?>
                    </div>
                    <?php
                endif;
        ...
        function ajax_get_fulfilled_orders() {
            ...
            try {
                $result = A2WL_ResultBuilder::buildOk(array('data' => $this->woocommerce_model->get_fulfilled_orders_data()));
        ...
        function ajax_order_info() {
            ...
            foreach ($items as $item) {
                ...
                if ($product_id > 0) {
                    ...
                    if ($product_url)
                        $tmp = $k . '). <a title="' . $product_name . '" href="' . $product_url . '" target="_blank" class="link_to_source product_url">' . _x('Product page', 'hint', 'ali2woo-lite') . '</a>';

                    if ($seller_url)
                        $tmp .= "<span class='seller_url_block'> | <a href='" . $seller_url . "' target='_blank' class='seller_url'>" . _x('Seller', 'hint', 'ali2woo-lite') . "</a></span>";
                } else {
                    $tmp .= $k . '). <span style="color:red;">' . _x('The product has been deleted', 'hint', 'ali2woo-lite') . '</span>';
                }
