...
/**
 * Description of A2WL_ReviewFrontendController
 * 
 * @author MA_GROUP
 * 
 * @autoload: true
 */
if (!class_exists('A2WL_ReviewFrontendController')) {

    class A2WL_ReviewFrontendController {
        ...
        function filter_text($comment_text, $comment = null) {
            ...
            if (a2wl_get_setting('review_show_image_list')) {
                $image_list = A2WL_Review::get_comment_photos($comment->comment_ID);

                if ($image_list) {
                    $comment_text .= "<div class='a2wl_review_images'>";
                    foreach ($image_list as $img) {
                        if(is_array($img)){
                            $comment_text .= "<a class='fancybox' rel='group{$comment->comment_ID}' href='{$img['image']}'><img src='{$img['thumb']}'/></a>";  
                        }else{
                            $comment_text .= "<a class='fancybox' rel='group{$comment->comment_ID}' href='{$img}'><img src='{$img}'/></a>";    
                        }
                    }
                    $comment_text .= "</div>";
                }
        ...
        function get_avatar_url($url, $id_or_email, $args) {
            ...
            if ($id_or_email instanceof WP_Comment) {
                ...
                $no_avatar_image = a2wl_get_setting('review_noavatar_photo', A2WL()->plugin_url() . '/assets/img/noavatar.png');
