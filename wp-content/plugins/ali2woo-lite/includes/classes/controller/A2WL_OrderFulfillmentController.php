...
/**
 * Description of A2WL_OrderFulfillmentController
 *
 * @author MA_GROUP
 *
 * @autoload: true
 */
if (!class_exists('A2WL_OrderFulfillmentController')) {

    class A2WL_OrderFulfillmentController extends A2WL_AbstractController  {
        ...
        function assets() {
            wp_enqueue_script('a2wl-ali-orderfulfill-js', A2WL()->plugin_url() . '/assets/js/orderfulfill.js', array(),  A2WL()->version, true);

            wp_enqueue_script('a2wl-sprintf-script', A2WL()->plugin_url() . '/assets/js/sprintf.js', array(), A2WL()->version);

            $lang_data = array(
                'placing_orders_d_of_d' => _x('Placing orders %d/%d...', 'Status', 'ali2woo-lite'),
                ...
                'install_chrome_ext' => _x('Please install and connect to your website the Ali2Woo Lite chrome extension to use this feature.', 'Status', 'ali2woo-lite'),
                'please_connect_chrome_extension_check_d' => _x('Please connect the Chrome extension to your store and then continue. Need help? Check out <a href="%s">the instruction</a>', 'Status', 'ali2woo-lite'),
                'we_found_old_order' => _x('We found an old order fulfillment process and removed it. Press the "Continue" button.', 'Status', 'ali2woo-lite'),
                'login_into_aliexpress_account' => _x('Please switch to AliExpress tab and login into your AliExpress account.', 'Status', 'ali2woo-lite'),
                'detected_old_aliexpress_interface' => _x('Detected old AliExpress interface. Please contact Ali2Woo Lite support.', 'Status', 'ali2woo-lite'),
                'your_customer_address_entered' => _x('Your customer address is entered. Wait...', 'Status', 'ali2woo-lite'),
                'product_is_added_to_cart' => _x('Product (%d) is added to the cart. Wait...', 'Status', 'ali2woo-lite'),
                'all_products_are_added' => _x('All products are added to the cart. Wait...', 'Status', 'ali2woo-lite'),
                'cart_is_cleared' => _x('The previous cart data is cleared. Wait...', 'Status', 'ali2woo-lite'),
                'get_no_responces_from_chrome_ext_d' => _x('Get no responces from the chrome extension for 30s. Check out <a href="%s">the instruction</a>', 'Status', 'ali2woo-lite'),
                'fill_order_note' => _x('Filling order notes...', 'Status', 'ali2woo-lite'),
                'cant_add_product_to_cart_d' => _x('Can`t add this product to the cart. Switch to AliExpress and choose another one or add a similar product from another supplier manually. Then continue. Check out <a href="%s">the instruction</a>', 'Status', 'ali2woo-lite'),
                'please_type_customer_address' => _x('Please switch to AliExpress tab, type the address or skip this order.', 'Status', 'ali2woo-lite'),
                'please_input_captcha' => _x('Please switch to AliExpress and input the Captcha code manually or wait for your Captcha solver to do the job...', 'Status', 'ali2woo-lite'),
                'order_is_placed' => _x('The order is placed. Wait...', 'Status', 'ali2woo-lite'),
                'internal_aliexpress_error' => _x('Internal AliExpress error. Please continue to try again or skip this order.', 'Status', 'ali2woo-lite'),
                'all_orders_are_placed' => _x('All orders are placed! Click "Orders List" to be directed to the orders list on the AliExpress website.', 'Status', 'ali2woo-lite'),
                'cant_process_your_orders' => _x('We can`t process your orders. Check out the "Status Page" for more details.', 'Status', 'ali2woo-lite'),
                'cant_get_order_id' => _x('Can`t get the external order ID, please copy it manually to your WC order. Then continue.', 'Status', 'ali2woo-lite'),
                'payment_is_failed' => _x('The payment is failed, please finish this order manually. Then continue.', 'Status', 'ali2woo-lite'),
                'done_pay_manually' => _x('Please switch to AliExpress and pay for the order.', 'Status', 'ali2woo-lite'),
                'choose_payment_method' => _x('Please switch to AliExpress and choose payment method.', 'Status', 'ali2woo-lite'),

                'please_activate_right_store_apikey_in_chrome' => _x('This website is not connected to the Ali2Woo Lite chrome extension. Please check that you choose right API key.', 'Status', 'ali2woo-lite'),

                'bad_product_id' => _x('Can`t find the WC order with a given ID.', 'Status', 'ali2woo-lite'),
                'no_variable_data' => _x('This order has a variable product but doesn`t contain the variable data for some reason. Check out <a href="%s">the instruction</a>', 'Status', 'ali2woo-lite'),
                'no_product_url' => _x('This order doesn`t contain the `product_url` field for some reason. Check out <a href="%s">the instruction</a>', 'Status', 'ali2woo-lite'),
                'no_ali_products' => _x('No AliExpress products in the current order. Check out <a href="%s">the instruction</a>', 'Status', 'ali2woo-lite'),

                'unknown_error' => _x('Unknown error occured. Please contact support.', 'Status', 'ali2woo-lite'),
                'server_error' => _x('Server error. Continue to try again.', 'Status', 'ali2woo-lite')
            );
        ...
        public function bulk_actions($params){
            $params[0][] = 'a2wl_order_place_bulk';
            $params[1]['a2wl_order_place_bulk'] = __("Place on AliExpress", 'ali2woo-lite');
        ...
        function get_aliexpress_order_data() {
            ...
            $def_prefship = a2wl_get_setting('fulfillment_prefship');
            $def_customer_note = a2wl_get_setting('fulfillment_custom_note');
            $def_phone_number = a2wl_get_setting('fulfillment_phone_number');
            $def_phone_code = a2wl_get_setting('fulfillment_phone_code');
            $a2wl_order_autopay = a2wl_get_setting('order_autopay');
            $a2wl_order_awaiting_payment = a2wl_get_setting('order_awaiting_payment');
            ...
            $content = array('id' => $post_id,
                ...
                'autopay' => $a2wl_order_autopay,
                'awaitingpay' => $a2wl_order_awaiting_payment,
            ...
            foreach ($items as $item) {
                ...
                if ($external_id) {
                    ...
                    $shipping_code = $normalized_item->get_A2WL_ShippingCode();
        ...
        private function translitirate($result){
            if (a2wl_get_setting('order_translitirate')){
                $result = A2WL_Utils::safeTransliterate($result);
            }
