<?php
/**
 * Description of A2WL_AddonsController
 *
 * @author andrey
 * 
 * @autoload: a2wl_init
...
if (!class_exists('A2WL_AddonsController')) {
    class A2WL_AddonsController extends A2WL_AbstractAdminPage {
        ...
        public function __construct() {
            ...
            parent::__construct(__('Add-ons/Extensions', 'ali2woo-lite'), __('Add-ons', 'ali2woo-lite'). ($new_addons_cnt ? ' <span class="update-plugins count-' . $new_addons_cnt . '"><span class="plugin-count">' . $new_addons_cnt . '</span></span>' : ''), 'manage_options', 'a2wl_addons', 100);

            if (empty($this->addons['next_update']) || $this->addons['next_update'] < time()) {
                $request = a2wl_remote_get(a2wl_get_setting('api_endpoint').'addons.php');
                ...
                update_option('a2wl_addons_data', $this->addons);
            }
        }
