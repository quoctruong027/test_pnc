...
/**
 * Description of A2WL_ImportPageController
 *
 * @author Andrey
 * 
 * @autoload: a2wl_init
 */
if (!class_exists('A2WL_ImportPageController')) {

    class A2WL_ImportPageController extends A2WL_AbstractAdminPage {
        ...
        public function __construct() {
            ...
            $this->country_model = new A2WL_Country();
            $this->import_process    = A2WL_ImportProcess::instance();
            $this->override_model = new A2WL_Override();
            
            $products_cnt = $this->product_import_model->get_products_count();

            parent::__construct(__('Import List', 'ali2woo-lite'), __('Import List', 'ali2woo-lite') .' '. ($products_cnt ? ' <span class="update-plugins count-' . $products_cnt . '"><span class="plugin-count">' . $products_cnt . '</span></span>' : ''), 'import', 'a2wl_import', 20);

            add_action('wp_ajax_a2wl_push_product', array($this, 'ajax_push_product'));
            add_action('wp_ajax_a2wl_delete_import_products', array($this, 'ajax_delete_import_products'));
            add_action('wp_ajax_a2wl_update_product_info', array($this, 'ajax_update_product_info'));
            add_action('wp_ajax_a2wl_link_to_category', array($this, 'ajax_link_to_category'));
            add_action('wp_ajax_a2wl_get_all_products_to_import', array($this, 'ajax_get_all_products_to_import'));
            add_action('wp_ajax_a2wl_get_product', array($this, 'ajax_get_product'));
            add_action('wp_ajax_a2wl_split_product', array($this, 'ajax_split_product'));
            add_action('wp_ajax_a2wl_import_images_action', array($this, 'ajax_import_images_action'));
            add_action('wp_ajax_a2wl_import_cancel_images_action', array($this, 'ajax_import_cancel_images_action'));
            add_action('wp_ajax_a2wl_search_tags', array($this, 'ajax_search_tags'));
            add_action('wp_ajax_a2wl_search_products', array($this, 'ajax_search_products'));
            add_action('wp_ajax_a2wl_override_product', array($this, 'ajax_override_product'));
            add_action('wp_ajax_a2wl_override_order_variations', array($this, 'ajax_override_order_variations'));
            add_action('wp_ajax_a2wl_cancel_override_product', array($this, 'ajax_cancel_override_product'));
                ...
                if($product = $this->product_import_model->get_product($_REQUEST['delete_id'])){
                    foreach($product['tmp_edit_images'] as $edit_image){
                        if(isset($edit_image['attachment_id'])){
                            A2WL_Utils::delete_attachment($edit_image['attachment_id'], true);
                        }
                    }
        ...
        public function render($params = array()) {
            ...
            $default_shipping_to_country = a2wl_get_setting('aliship_shipto', 'US');
            
            $products_cnt = $this->product_import_model->get_products_count();
            $paginator = A2WL_Paginator::build($products_cnt);
            ...
            $this->model_put("localizator", A2WL_AliexpressLocalizator::getInstance());
        ...
        public function ajax_push_product() {
            ...
            $background_import = a2wl_get_setting('background_import', true);
            ...
            if($background_import){
                ...
                if (isset($_POST['id']) && $_POST['id']) {
                    ...
                    if ($product) {
                        ...
                        $result = A2WL_ResultBuilder::buildError("Product " . $_POST['id'] . " not find.");
                    }
                } else {
                    $result = A2WL_ResultBuilder::buildError("import_product: waiting for ID...");
                }
            }else{
                // Old import method 
                ...
                try {
                    if (isset($_POST['id']) && $_POST['id']) {
                        ...
                        if ($product) {
                            ...
                            if(!a2wl_check_defined('A2WL_ALLOW_PRODUCT_DUPLICATION') && $import_wc_product_id){
                            ...
                            if ($result['state'] !== 'error') {
                                ...
                                $result = A2WL_ResultBuilder::buildError($result['message']);
                            }
                        } else {
                            $result = A2WL_ResultBuilder::buildError("Product " . $_POST['id'] . " not find.");
                        }
        ...
        public function ajax_update_product_info() {
            ...
            try {
                ...
                if (isset($_POST['id']) && $_POST['id'] && ($product = $this->product_import_model->get_product($_POST['id']))) {
                    ...
                    $result = A2WL_ResultBuilder::buildOk($out_data);
                } else {
                    $result = A2WL_ResultBuilder::buildError("update_product_info: waiting for ID...");
                }
        ...
        public function ajax_link_to_category() {
            if (!empty($_POST['categories']) && !empty($_POST['ids'])) {
                ...
                a2wl_set_setting('remember_categories', $new_categories);
            } else if (empty($_POST['categories'])) {
                a2wl_del_setting('remember_categories');
            }
            echo json_encode(A2WL_ResultBuilder::buildOk());
            wp_die();
        }
        
        public function ajax_get_all_products_to_import() {
            echo json_encode(A2WL_ResultBuilder::buildOk(array('ids' => $this->product_import_model->get_product_id_list())));
            wp_die();
        }
        ...
        public function ajax_get_product() {
            if (!empty($_POST['id'])) {
                if ($product = $this->product_import_model->get_product($_POST['id'])) {
                    $result = A2WL_ResultBuilder::buildOk(array('product'=>$product));
                }else{
                    $result = A2WL_ResultBuilder::buildError("product not found");
                }
            } else {
                $result = A2WL_ResultBuilder::buildError("get_product: waiting for ID...");
            }
            echo json_encode($result);
            wp_die();
        }
        ...
        public function ajax_split_product(){
            if (!empty($_POST['id']) && !empty($_POST['attr'])) {
                ...
                $result = A2WL_ResultBuilder::buildError("split_product: wrong parameters...");
            }
        ...
        public function ajax_import_images_action() {
            ...
            try {
                if (isset($_POST['id']) && $_POST['id'] && ($product = $this->product_import_model->get_product($_POST['id'])) && !empty($_POST['source']) && !empty($_POST['type']) && in_array($_POST['source'], array("description", "variant")) && in_array($_POST['type'], array("copy", "move"))) {
                    ...
                    $result = A2WL_ResultBuilder::buildError("Error in params");
                }
        ...
        public function ajax_override_product() {
            ...
            if($result['state']!='error') {
                $result['button'] = __('Override', 'ali2woo-lite');
            }
        ...
        public function ajax_cancel_override_product() {
            ...
            if($result['state']!='error') {
                $result['button'] = __('Push to Shop', 'ali2woo-lite');
                $result['override_action'] = '<li><a href="#" class="product-card-override-product">'.__('Select Product to Override', 'ali2woo-lite').'</a></li>';               
            }
