...
if (!class_exists('A2WL_JSON_API')) {

    class A2WL_JSON_API {

        function __construct($root_menu_slug) {
            $this->root_menu_slug = $root_menu_slug;
            $this->query = new A2WL_JSON_API_Query();
            $this->response = new A2WL_JSON_API_Response();
            ...
            // disable menu
            if (defined('A2WL_DEBUG_PAGE') && A2WL_DEBUG_PAGE) {
                add_action('admin_menu', array(&$this, 'admin_menu'));
            }
            
            add_action('update_option_a2wl_json_api_base', array(&$this, 'flush_rewrite_rules'));
            add_action('pre_update_option_a2wl_json_api_controllers', array(&$this, 'update_controllers'));
        }
        ...
        function template_redirect() {
            ...
            $enabled_controllers = explode(',', a2wl_get_setting('json_api_controllers'));
            ...
            if ($controller) {
                ...
                if ($method) {
                    ...
                    do_action("a2wl_json_api", $controller, $method);
                    do_action("a2wl_json_api-{$controller}-$method");
        ...
        function admin_options() {          
            if (!current_user_can('manage_options')) {
                wp_die(__('You do not have sufficient permissions to access this page.', 'ali2woo-lite'));
            }
            ...
            if (!empty($_REQUEST['_wpnonce']) && wp_verify_nonce($_REQUEST['_wpnonce'], "update-options")) {
                if ((!empty($_REQUEST['action']) || !empty($_REQUEST['action2'])) &&
                    ...
                    a2wl_set_setting('json_api_controllers', implode(',', $active_controllers));
                }
                if (isset($_REQUEST['a2wl_json_api_base'])) {
                    a2wl_set_setting('json_api_base', $_REQUEST['a2wl_json_api_base']);
                }
            }
            ?>
            <div class="wrap">
                ...
                <?php
                if ($active) {
                    echo '<a href="' . wp_nonce_url('admin.php?page=a2w-json-api&amp;action=deactivate&amp;controller=' . $controller, 'update-options') . '" title="' . __('Deactivate this controller', 'ali2woo-lite') . '" class="edit">' . __('Deactivate') . '</a>';
                } else if (!$error) {
                    echo '<a href="' . wp_nonce_url('admin.php?page=a2w-json-api&amp;action=activate&amp;controller=' . $controller, 'update-options') . '" title="' . __('Activate this controller', 'ali2woo-lite') . '" class="edit">' . __('Activate') . '</a>';
                }
            ...
            <?php $this->print_controller_actions('action2'); ?>
                    ...
                    <table class="form-table">
                        <tr valign="top">
                            ...
                            <td><code><?php bloginfo('url'); ?>/</code><input type="text" name="a2wl_json_api_base" value="<?php echo a2wl_get_setting('json_api_base'); ?>" size="15" /></td>
                        </tr>
                    </table>
            ...
            <?php } ?>
                    <p class="submit">
                        <input type="submit" class="button-primary" value="<?php _e('Save Changes', 'ali2woo-lite') ?>" />
                    </p>
                </form>
            </div>
                        <?php
        ...
        function get_method_url($controller, $method, $options = '') {
            ...
            $base = a2wl_get_setting('json_api_base');
        ...
        function get_controllers() {
            $controllers = array();
            $this->check_directory_for_controllers(A2WL_JSON_API_DIR."/controllers", $controllers);
            $this->check_directory_for_controllers(get_stylesheet_directory(), $controllers);
            $controllers = apply_filters('a2wl_json_api_controllers', $controllers);
            return array_map('strtolower', $controllers);
        }
        
        function get_enabled_controllers($default = 'core,auth') {
            return explode(',', a2wl_get_setting('json_api_controllers', $default));
        }
        ...
        function check_directory_for_controllers($dir, &$controllers) {
            ...
            while ($file = readdir($dh)) {
                if (preg_match('/(.+)\.php$/i', $file, $matches)) {
                    ...
                    if (preg_match("/class\s+A2WL_JSON_API_{$matches[1]}_Controller/i", $src)) {
        ...
        function controller_is_active($controller) {
            if (defined('A2WL_JSON_API_CONTROLLERS')) {
                $default = A2WL_JSON_API_CONTROLLERS;
            } else {
                $default = 'core,auth';
            }
            $active_controllers = explode(',', a2wl_get_setting('json_api_controllers', $default));
        ...
        function controller_class($controller) {
            return "a2wl_json_api_{$controller}_controller";
        }

        function controller_path($controller) {
            $json_api_path = A2WL_JSON_API_DIR."/controllers/$controller.php";
        ...
        function get_nonce_id($controller, $method) {
            ...
            return "a2wl_json_api-$controller-$method";
        }
