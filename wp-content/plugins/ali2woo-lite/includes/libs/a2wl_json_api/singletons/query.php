...
if (!class_exists('A2WL_JSON_API_Query')) {

    class A2WL_JSON_API_Query {
        ...
        function get_legacy_controller($json) {
            ...
            if ($json == 'submit_comment') {
                if ($a2wl_json_api->controller_is_active('respond')) {
                    return 'respond';
                } else {
                    $a2wl_json_api->error("The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > JSON API.");
                }
            } else if ($json == 'create_post') {
                if ($a2wl_json_api->controller_is_active('posts')) {
                    return 'posts';
                } else {
                    $a2wl_json_api->error("The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > JSON API.");
                }
            } else {
                return 'core';
            }
        ...
        function get_method($controller) {
            ...
            //
            // Note:
            //   The implicit outcome (3) is invoked by setting the a2w-json query var to a
            //   non-empty value on any WordPress page:
            //     * http://example.org/2009/11/10/hello-world/?a2w-json=1 (get_post)
            //     * http://example.org/2009/11/?a2w-json=1 (get_date_posts)
            //     * http://example.org/category/foo?a2w-json=1 (get_category_posts)

            $method = $this->get('a2w-json');
            ...
            if (empty($method)) {
                ...
                return false;
            } else if (method_exists("A2WL_JSON_API_{$controller}_Controller", $method)) {
                // Case 2: an explicit method was specified
                return $method;
            } else if ($controller == 'core') {
                $a2wl_json_api->error("Unknown method '$method'.");
                return 'info';
            }
