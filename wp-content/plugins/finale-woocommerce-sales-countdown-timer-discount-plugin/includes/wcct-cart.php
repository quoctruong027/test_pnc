...
class WCCT_cart {
	...
	public function detect_mini_cart_start() {
		...
		if ( is_array( $get_cart ) && $get_cart && count( $get_cart ) > 0 ) {
	...
	public function check_cart_item_stock() {
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $values ) {
			...
			if ( $allow_wc_hold_stock && get_option( 'woocommerce_hold_stock_minutes' ) > 0 && $_product->backorders_allowed() ) {
	...
	public function wcct_remove_cart_data( $cart_item_key, $cart ) {
		...
		$get_prev_evergreen_session = WC()->session->get( '_wcct_evergreen_cart_data_' );

		if ( is_array( $get_prev_evergreen_session ) && isset( $get_prev_evergreen_session[ $cart_item_key ] ) ) {
			unset( $get_prev_evergreen_session[ $cart_item_key ] );
			WC()->session->set( '_wcct_evergreen_cart_data_', $get_prev_evergreen_session );
		}
	}
	...
	public function wcct_add_cart_data( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
		...
		WCCT_Core()->public->wcct_genrate_actions( $get_item_id );
		...
		if ( isset( $single_data['campaign_meta'] ) && ! empty( $single_data['campaign_meta'] ) ) {
			...
			/** Check for evergreen campaign */
			$get_prev_evergreen_session = WC()->session->get( '_wcct_evergreen_cart_data_' );
			if ( ! is_array( $get_prev_evergreen_session ) ) {
				$get_prev_evergreen_session = array();
			}
			foreach ( $Camp_meta as $key => $value ) {
				if ( isset( $value['type'] ) && 'evergreen' === $value['type'] ) {
					$get_prev_evergreen_session[ $cart_item_key ] = $key;
				}
			}
			WC()->session->set( '_wcct_evergreen_cart_data_', $get_prev_evergreen_session );
		}
	...
	public function setup_cart_data_session( $cart ) {
		...
		if ( $get_cart && is_array( $get_cart ) && count( $get_cart ) > 0 ) {
			foreach ( $get_cart as $cartitem ) {
				...
				if ( ! is_null( $cartitem['data'] ) && $cartitem['data'] instanceof WC_product ) {
					$cartitem['data']->get_price();
				}
