...
if ( ! class_exists( 'XL_Common' ) ) {
	...
	class XL_Common {
		...
		public static function load_text_domain( $get_global_path ) {
			...
			wp_localize_script( 'wp-util', 'xlplgins', array(
...
register_deactivation_hook( WCCT_PLUGIN_BASENAME, array( 'XL_Common', 'reset_latest_installed' ) );

if ( is_array( $xl_ultimate_latest_core_all ) && ! $xl_ultimate_latest_core_all || is_array( $xl_ultimate_latest_core_all ) && count( $xl_ultimate_latest_core_all ) <= 0 ) {
