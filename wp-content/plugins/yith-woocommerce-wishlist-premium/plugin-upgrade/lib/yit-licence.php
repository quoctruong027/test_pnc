					...
					if ( ! ! $product_names ) {
						...
						<div id="yith-license-notice" class="notice notice-error is-dismissible" data-nonce="<?php echo esc_attr( $nonce ); ?>">
							<div style="margin: 8px 0">
								<?php $warning = esc_html__( 'Warning', 'yith-plugin-upgrade-fw' ) ?>
								<?php $add_license_key = esc_html__( 'Enter your license key, please', 'yith-plugin-upgrade-fw' ) ?>
								<?php $message = esc_html__( "You didn't set license key for the following YITH products: {product_list} which means you're missing out on updates and support", 'yith-plugin-upgrade-fw' ) ?>
								<?php $message = str_replace( '{product_list}', $product_list, $message ); ?>
								<?php $message = str_replace( '{}', $product_list, $message ); ?>
								<?php printf( "<strong>%s!</strong> %s. <a href='%s'>%s</a>", $warning, $message, $activation_url, $add_license_key ); ?>
							</div>
						</div>
						<?php
					}
