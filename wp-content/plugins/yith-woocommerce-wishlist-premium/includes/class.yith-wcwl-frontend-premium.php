...
if ( ! class_exists( 'YITH_WCWL_Frontend_Premium' ) ) {
	...
	class YITH_WCWL_Frontend_Premium extends YITH_WCWL_Frontend {
		...
		public function get_no_wishlist_message() {
			...
			$message = sprintf( __( 'You don\'t have any wishlist yet. <a href="%s" title="%s" %s>Create your first wishlist &rsaquo;</a>', 'yith-woocommerce-wishlist' ), $create_url, $create_title, $create_custom_attributes );
		...
		public function add_wishlist_notice() {
			$disable_wishlist = get_option( 'yith_wcwl_disable_wishlist_for_unauthenticated_users' );
			if( apply_filters( 'yith_wcwl_add_wishlist_notice', $disable_wishlist == 'yes' ) && isset( $_GET['wishlist_notice'] ) && $_GET['wishlist_notice'] == true && ! isset( $_POST['login'] ) && ! isset( $_POST['register'] ) ){
