...
class WC_CP_Products {
	...
	public static function init() {
		...
		// Always-on price filters used in cart context.
 		if ( 'filters' === self::get_composited_cart_item_discount_method() ) {
			...
			add_filter( 'woocommerce_bundled_item_discount', array( __CLASS__, 'filter_bundled_item_discount_cart' ), 10, 3 );
 		}
	}
	...
	*/
	...
	/**
 	 ...
 	 * @return boolean
 	 */
	public static function is_component_option_pricing_context( $product ) {
		return ( self::$filtered_component_option && self::$filtered_component_option->get_product_id() === $product->get_id() ) || isset( $product->composited_cart_item );
	}
	...
	/**
 	...
 	public static function get_composited_cart_item_discount_method() {
 		 ...
 		 */
 		return apply_filters( 'woocommerce_composited_cart_item_discount_method', 'filters' );
 	}
	...
	public static function add_filters( $component_option ) {
		...
		add_filter( 'woocommerce_bundles_update_price_meta', array( __CLASS__, 'filter_bundles_update_price_meta' ), 10, 2 );
		...
		add_filter( 'woocommerce_bundled_item_discount', array( __CLASS__, 'filter_bundled_item_discount' ), 10, 3 );

		/**
		 * NYP compatibility.
		 */

		add_filter( 'woocommerce_nyp_html', array( __CLASS__, 'filter_get_nyp_price_html' ), 15, 2 );
	...
	public static function remove_filters() {
		...
		remove_filter( 'woocommerce_bundles_update_price_meta', array( __CLASS__, 'filter_bundles_update_price_meta' ), 10, 2 );
		...
		remove_filter( 'woocommerce_bundled_item_discount', array( __CLASS__, 'filter_bundled_item_discount' ), 10, 3 );

		/**
		 * NYP compatibility.
		 */

		remove_filter( 'woocommerce_nyp_html', array( __CLASS__, 'filter_get_nyp_price_html' ), 15, 2 );
	}
	...
	*/
	...
	public static function schedule_price_calc_tasks() {
		...
		// Give background processing a chance to work before considering a manual run...
			...
			if ( self::$price_calc_task_runner->is_queued() && ! self::$price_calc_task_runner->is_running() ) {
	 ...
	 * @return boolean
	 */
	public static function filter_bundles_update_price_meta( $update, $bundle ) {
		return false;
	}
	...
	public static function filter_get_price_html( $price_html, $product ) {
		...
		if ( ! empty( self::$filtered_component_option ) ) {
			...
			if ( ! self::$filtered_component_option->is_priced_individually() ) {
				...
				if ( $add_suffix ) {
					$suffix     = '' === self::$filtered_component_option->get_quantity_max() || self::$filtered_component_option->get_quantity_max() > 1 ? ' ' . __( '/ pc.', 'woocommerce-composite-products' ) : '';
		...
		if ( self::$filtered_component_option ) {
 		...
 		if ( $component_option && ( $component_option instanceof WC_CP_Product ) ) {
			...
			if ( empty( $price ) ) {
				...
				if ( isset( $product->composited_cart_item ) ) {
 					$product->composited_cart_item = false;
 				} else {
 					self::$filtered_component_option = false;
 				}
				...
				if ( isset( $product->composited_cart_item ) ) {
 					$product->composited_cart_item = $component_option;
 				} else {
 					self::$filtered_component_option = $component_option;
 				}
			}
	 ...
	 * @return string
	 */
	public static function filter_get_sale_price( $price, $product ) {
		...
		if ( self::$filtered_component_option ) {
 		...
 		if ( $component_option && ( $component_option instanceof WC_CP_Product ) ) {
			...
			if ( '' === $price || false === $component_option->is_discount_allowed_on_sale_price() ) {
	...
	/**
 	...
 	public static function filter_get_price_cart( $price, $product ) {
 		return self::filter_get_price( $price, $product );
 	}
 	...
 	public static function filter_get_sale_price_cart( $price, $product ) {
 		return self::filter_get_sale_price( $price, $product );
 	}
	 ...
	 * @return mixed
	 */
	public static function filter_bundled_item_discount( $bundled_discount, $bundled_item, $context ) {
	 ...
	 * @return mixed
	 */
	public static function filter_bundled_item_discount_cart( $bundled_discount, $bundled_item, $context ) {
		return self::filter_bundled_item_discount( $bundled_discount, $bundled_item, $context );
	}
