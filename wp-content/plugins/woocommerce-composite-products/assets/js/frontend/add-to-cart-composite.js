...
( function( $, Backbone ) {
		...
		this.Component_Options_View = function( component, opts ) {
			...
			var View = Backbone.View.extend( {
				...
				render: function( dropdown_only ) {
					 ...
					 */
					if ( ( ! composite.is_finalized || selected_product === this.reference_option ) && 'relative' === self.get_price_display_format() && self.has_valid_selections( false ) ) {
						this.reference_price  = composite.data_model.calculate_component_subtotals( self, composite.data_model.price_data, 1 ).price;
						this.reference_option = selected_product;
					}
	...
	wc_cp_classes.WC_CP_Scenarios_Manager = function( composite ) {
		...
		this.is_scenario_action_calculation = function( type, modifier ) {
			...
			return _.contains( manager.get_scenario_action_settings( type ).calculation, modifier );
		};
