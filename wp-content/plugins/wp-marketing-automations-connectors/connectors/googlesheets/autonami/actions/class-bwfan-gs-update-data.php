...
class BWFAN_GS_Update_Data extends BWFAN_Action {

	private static $instance = null;

	public function __construct() {
		$this->action_name = __( 'Update Row', 'autonami-automations-connectors' );
	}
        ...
        <script type="text/html" id="tmpl-action-<?php echo $unique_slug; ?>">
            ...
            <div class="bwfan-input-form clearfix">
                <label for="" class="bwfan-label-title">
					...
					$message = __( 'Select work sheet where data needs to update.', 'autonami-automations-connectors' );
            ...
            <label for="" class="bwfan-label-title">
				<?php
				echo __( 'Row to update for a given column value', 'autonami-automations-connectors' );
            ...
            <div class="clearfix bwfan-input-search">
                ...
                #>
                <div class="bwfan-input-form clearfix gs-search-fields">
                    <div class="bwfan-col-sm-3 bwfan-pl-0">
                        <select data-element-type="bwfan-select" data-parent-groupid="{{data.action_id}}" data-parent-slug="<?php echo $unique_slug; ?>" required
                                class="bwfan-field-select bwfan-input-wrapper" name="bwfan[{{data.action_id}}][data][worksheet_search_data][column][0]">
                            <#
                            if(_.has(data.actionFieldsOptions, 'worksheet_column_options') && _.isObject(data.actionFieldsOptions.worksheet_column_options) ) {
	...
	public function execute_action( $action_data ) {
		...
		$call           = $load_connector->get_call( 'wfco_gs_update_data' );
		if ( is_null( $call ) ) {
			return array(
				'status'  => 4,
				'message' => __( 'No update call found', 'autonami-automations-connectors' ),
			);
		}
		...
		if ( true === $result ) {
			return array(
				'status'  => 3,
				'message' => __( 'Data successfully updated in the google spreadsheet.', 'autonami-automations-connectors' ),
			);
		}
