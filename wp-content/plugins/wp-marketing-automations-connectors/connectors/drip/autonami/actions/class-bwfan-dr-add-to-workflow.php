...
final class BWFAN_DR_Add_To_Workflow extends BWFAN_Action {

	private static $ins = null;

	private function __construct() {
		$this->action_name     = __( 'Add Subscriber to Workflow', 'autonami-automations-connectors' );
		$this->action_desc     = __( 'This action adds the subscriber to the selected workflow', 'autonami-automations-connectors' );
	...
	public function execute_action( $action_data ) {
		...
		if ( isset( $result['body']['subscribers'][0]['id'] ) && '' !== $result['body']['subscribers'][0]['id'] ) {
			return array(
				'status'  => 3,
				'message' => 'Subscriber added to workflow.',
			);
		}
 ...
 */
return 'BWFAN_DR_Add_To_Workflow';
