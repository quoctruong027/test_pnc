...
final class BWFAN_DR_Add_To_Campaign extends BWFAN_Action {

	private static $ins = null;

	private function __construct() {
		$this->action_name     = __( 'Add Subscriber to Campaign', 'autonami-automations-connectors' );
		$this->action_desc     = __( 'This action adds the subscriber to the selected campaign', 'autonami-automations-connectors' );
	...
	public function execute_action( $action_data ) {
		...
		// Email already exist send success status to autonami;
		if ( isset( $result['body']['errors'] ) && isset( $result['body']['errors'][0] ) && isset( $result['body']['errors'][0]['code'] ) && 'uniqueness_error' === $result['body']['errors'][0]['code'] ) {
			return array(
				'status'  => 3,
				'message' => ( isset( $result['body']['errors'][0]['message'] ) ) ? $result['body']['errors'][0]['message'] : '',
			);
		}
 ...
 */
return 'BWFAN_DR_Add_To_Campaign';
