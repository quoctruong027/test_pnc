...
/**
 * Class BWFAN_Drip_Webhook_Received
 */
final class BWFAN_Drip_Webhook_Received extends BWFAN_Event {
	private static $instance = null;
	private $drip_id = null;
	private $first_name = '';
	private $last_name = '';
	private $phone = '';
	private $email = '';
	...
	private function __construct() {
		...
		$this->event_merge_tag_groups = array( 'drip_contact' );
		$this->event_rule_groups      = array( 'bwf_contact' );
		$this->customer_email_tag     = '{{drip_contact_email}}';
	}

	public function load_hooks() {
		add_action( 'admin_enqueue_scripts', array( $this, 'bwfan_webhook_admin_enqueue_assets' ), 98 );
		add_action( "bwfan_drip_connector_sync_call", array( $this, 'before_process_webhook_contact' ), 10, 3 );

		add_action( 'bwfan_webhook_drip_contact', array( $this, 'process' ), 10, 7 );
	}

	public function before_process_webhook_contact( $automation_id, $automation_key, $request_data ) {
		$hook  = 'bwfan_webhook_drip_contact';
		$args  = array(
			'first_name'     => isset( $request_data['custom_fields']['first_name'] ) ? $request_data['custom_fields']['first_name'] : '',
			'last_name'      => isset( $request_data['custom_fields']['last_name'] ) ? $request_data['custom_fields']['last_name'] : '',
			'phone'          => isset( $request_data['custom_fields']['phone'] ) ? $request_data['custom_fields']['phone'] : '',
			'email'          => isset( $request_data['email'] ) ? $request_data['email'] : '',
			'contact_id'     => isset( $request_data['id'] ) ? $request_data['id'] : '',
	...
	public function bwfan_webhook_admin_enqueue_assets() {
		...
		$this->drip_id = isset( $_GET['edit'] ) ? sanitize_text_field( $_GET['edit'] ) : ''; //phpcs:ignore WordPress.Security.NonceVerification
		if ( ! empty( $this->drip_id ) ) {
			$meta = BWFAN_Model_Automationmeta::get_meta( $this->drip_id, 'event_meta' );
			if ( isset( $meta['bwfan_unique_key'] ) && ! empty( $meta['bwfan_unique_key'] ) ) {
				$this->localized_automation_key = $meta['bwfan_unique_key'];
			}
		}

		BWFAN_Core()->admin->set_events_js_data( $this->get_slug(), 'automation_id', $this->drip_id );
	...
	public function get_task_view( $global_data ) {
		...
		if ( isset( $global_data['contact_id'] ) && ! empty( $global_data['contact_id'] ) ) { ?>
            <li>
                <strong><?php esc_html_e( 'Contact:', 'autonami-automations' ); ?> </strong>
	...
	public function get_view( $db_eventmeta_saved_value ) {
        ...
        <script type="text/html" id="tmpl-event-<?php esc_attr_e( $this->get_slug() ); ?>">
            ...
            var webhook_url = '<?php esc_attr_e( home_url( '/' ) ); ?>wp-json/autonami/v1/drip/webhook?drip_id='+eventData.automation_id+'&drip_key='+eventData.localized_automation_key;
            #>
	...
	public function capture_async_data() {
		$this->drip_id        = BWFAN_Common::$events_async_data['drip_id'];
	...
	public function run_automations() {
		...
		foreach ( $this->automations_arr as $automation_id => $automation_data ) {
			...
			$automation_id_matched = ( absint( $automation_id ) === absint( $this->drip_id ) );
	...
	public function get_event_data() {
		$data_to_send                             = [];
		$data_to_send['global']['drip_id']        = $this->drip_id;
...
if ( array_key_exists( 'bwfco_drip', $saved_connectors ) ) {
	return 'BWFAN_Drip_Webhook_Received';
}