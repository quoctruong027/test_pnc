...
final class BWFAN_Ontraport_Add_To_Campaign extends BWFAN_Action {

	private static $ins = null;

	private function __construct() {
		$this->action_name     = __( 'Add Contact to Campaign', 'autonami-automations-connectors' );
		$this->action_desc     = __( 'This action adds the contact to the selected campaign', 'autonami-automations-connectors' );
	...
	public function get_view() {
		...
		<script type="text/html" id="tmpl-action-<?php echo esc_attr__( $unique_slug ); ?>">
			...
			<label for="" class="bwfan-label-title">
	            ...
	            $message = __( 'Select campaign to add contact to and if unable to locate then sync the connector.', 'autonami-automations-connectors' );
	...
	protected function handle_response( $result, $call_object = null ) {
		...
		if (200 === absint($result['response']) &&  isset( $result['body']['data'] ) && $result['body']['data'] ) {
			return array(
				'status'  => 3,
				'message' => __( 'Contact added to Campaign successfully!', 'autonami-automations-connectors' ),
			);
		}
 ...
 */
return 'BWFAN_Ontraport_Add_To_Campaign';
