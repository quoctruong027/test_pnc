...
/**
 * Class BWFAN_Webhook_Event
 */
final class BWFAN_Ontraport_Webhook_Received extends BWFAN_Event {
	...
	private function __construct() {
		...
		$this->event_merge_tag_groups = array( 'ontraport_contact' );
		$this->event_rule_groups      = array( 'bwf_contact' );
		$this->customer_email_tag     = '{{ontraport_contact_email}}';
	}

	public function load_hooks() {
		add_action( 'admin_enqueue_scripts', array( $this, 'bwfan_webhook_admin_enqueue_assets' ), 98 );
		add_action( "bwfan_ontraport_connector_sync_call", array( $this, 'before_process_webhook_contact' ), 10, 3 );
		add_action( 'bwfan_webhook_ontraport_contact', array( $this, 'process' ), 10, 7 );
	}

	public function before_process_webhook_contact( $automation_id, $automation_key, $request_data ) {
		$hook  = 'bwfan_webhook_ontraport_contact';
		$args  = array(
			'first_name'     => isset( $request_data['first_name'] ) ? $request_data['first_name'] : '',
			'last_name'      => isset( $request_data['last_name'] ) ? $request_data['last_name'] : '',
	...
	public function bwfan_webhook_admin_enqueue_assets() {
		$this->on_id = isset( $_GET['edit'] ) ? sanitize_text_field( $_GET['edit'] ) : ''; //phpcs:ignore WordPress.Security.NonceVerification
		if ( ! empty( $this->on_id ) ) {
			$meta = BWFAN_Model_Automationmeta::get_meta( $this->on_id, 'event_meta' );
			if ( isset( $meta['bwfan_unique_key'] ) && ! empty( $meta['bwfan_unique_key'] ) ) {
				$this->localized_automation_key = $meta['bwfan_unique_key'];
			}
		}

		BWFAN_Core()->admin->set_events_js_data( $this->get_slug(), 'automation_id', $this->on_id );
	...
	public function get_view( $db_eventmeta_saved_value ) {
        ...
        <script type="text/html" id="tmpl-event-<?php esc_attr_e( $this->get_slug() ); ?>">
            ...
            var webhook_url = '<?php esc_attr_e( home_url( '/' ) ); ?>wp-json/autonami/v1/ontraport/webhook?bwfan_ontraport_id='+eventData.automation_id+'&bwfan_ontraport_key='+eventData.localized_automation_key+'&bwfan_contact_id=[Contact ID]';
            #>
	 ...
	 * @param $order_id
	 */
	public function process( $first_name, $last_name, $email, $phone, $contact_id, $automation_key, $automation_id ) {
		$data                   = $this->get_default_data();
		$data['on_id']          = $automation_id;
	...
	public function capture_async_data() {
		$this->on_id          = BWFAN_Common::$events_async_data['on_id'];
	...
	public function run_automations() {
		...
		foreach ( $this->automations_arr as $automation_id => $automation_data ) {
			...
			$automation_id_matched = ( absint( $automation_id ) === absint( $this->on_id ) );
	...
	public function get_event_data() {
		$data_to_send                             = [];
		$data_to_send['global']['on_id']          = $this->on_id;
...
if ( array_key_exists( 'bwfco_ontraport', $saved_connectors ) ) {
	return 'BWFAN_Ontraport_Webhook_Received';
}
