...
final class BWFAN_AC_Rmv_From_Automation extends BWFAN_Action {

	private static $instance = null;

	private function __construct() {
		$this->action_name     = __( 'End Automation', 'autonami-automations-connectors' );
		$this->action_desc     = __( 'This action removes a contact from the selected automation', 'autonami-automations-connectors' );
	...
	public function get_view() {
        ...
        <script type="text/html" id="tmpl-action-<?php echo esc_attr__( $unique_slug ); ?>">
            ...
            <label for="" class="bwfan-label-title">
				...
				$message = __( 'Select an automation to remove contact from and if unable to locate then sync the connector.', 'autonami-automations-connectors' );
	...
	protected function handle_response( $result, $call_object = null ) {
		...
		if ( isset( $result['body']['result_code'] ) && 1 === intval( $result['body']['result_code'] ) ) {
			return array(
				'status'  => 3,
				'message' => isset( $result['body']['result_message'] ) ? $result['body']['result_message'] : __( 'Contact successfully removed from automation', 'autonami-automations-connectors' ),
			);
		}
...
return 'BWFAN_AC_Rmv_From_Automation';
