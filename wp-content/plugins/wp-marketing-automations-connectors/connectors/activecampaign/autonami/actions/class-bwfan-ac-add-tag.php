...
final class BWFAN_AC_Add_Tag extends BWFAN_Action {
	...
	public function make_new_tags( $new_tags_to_make, $action_data ) {
		...
		$call_class     = $load_connector->get_call( 'wfco_ac_create_tag' );
	...
	protected function handle_response( $result, $call_object = null ) {
		...
		if ( isset( $result['body']['contactTag'] ) && is_array( $result['body']['contactTag'] ) && count( $result['body']['contactTag'] ) > 0 ) {
			return array(
				'status'  => 3,
				'message' => isset( $result['body']['result_message'] ) ? $result['body']['result_message'] : __( 'Tag successfully added', 'autonami-automations-connectors' ),
			);
		}
