 ...
 */
final class BWFAN_AC_Webhook_Received extends BWFAN_Event {
	...
	private function __construct() {
		...
		$this->event_merge_tag_groups = array( 'ac_contact' );
		$this->event_rule_groups      = array( 'bwf_contact' );
		$this->customer_email_tag     = '{{ac_contact_email}}';
	}

	public function load_hooks() {
		add_action( 'admin_enqueue_scripts', array( $this, 'bwfan_webhook_admin_enqueue_assets' ), 98 );
		add_action( "bwfan_connector_sync_call", array( $this, 'before_process_webhook_contact' ), 10, 3 );
		add_action( 'bwfan_webhook_ac_contact', array( $this, 'process' ), 10, 7 );
	}

	public function before_process_webhook_contact( $automation_id, $automation_key, $request_data ) {
		$hook  = 'bwfan_webhook_ac_contact';
		$args  = array(
	...
	public function bwfan_webhook_admin_enqueue_assets() {
		...
		$this->a_id = isset( $_GET['edit'] ) ? sanitize_text_field( $_GET['edit'] ) : ''; //phpcs:ignore WordPress.Security.NonceVerification
		if ( ! empty( $this->a_id ) ) {
			$meta = BWFAN_Model_Automationmeta::get_meta( $this->a_id, 'event_meta' );
			if ( isset( $meta['bwfan_unique_key'] ) && ! empty( $meta['bwfan_unique_key'] ) ) {
				$this->localized_automation_key = $meta['bwfan_unique_key'];
			}
		}

		BWFAN_Core()->admin->set_events_js_data( $this->get_slug(), 'automation_id', $this->a_id );
	...
	public function get_view( $db_eventmeta_saved_value ) {
        ...
        <script type="text/html" id="tmpl-event-<?php esc_attr_e( $this->get_slug() ); ?>">
            ...
            var webhook_url = '<?php esc_attr_e( home_url( '/' ) ); ?>wp-json/autonami/v1/ac/webhook?ac_id='+eventData.automation_id+'&ac_key='+eventData.localized_automation_key;
            #>
	...
	public function capture_async_data() {
		$this->a_id           = BWFAN_Common::$events_async_data['a_id'];
	...
	public function run_automations() {
		...
		foreach ( $this->automations_arr as $automation_id => $automation_data ) {
			...
			$automation_id_matched = ( absint( $automation_id ) === absint( $this->a_id ) );
	...
	public function get_event_data() {
		$data_to_send                             = [];
		$data_to_send['global']['a_id']           = $this->a_id;
...
if ( array_key_exists( 'bwfco_activecampaign', $saved_connectors ) ) {
	return 'BWFAN_AC_Webhook_Received';
}
