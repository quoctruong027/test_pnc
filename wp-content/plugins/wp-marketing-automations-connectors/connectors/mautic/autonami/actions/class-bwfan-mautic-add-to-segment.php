...
final class BWFAN_Mautic_Add_To_Segment extends BWFAN_Action {

	private static $ins = null;

	private function __construct() {
		$this->action_name     = __( 'Add Contact to Segment / List', 'autonami-automations-connectors' );
		$this->action_desc     = __( 'This action adds the contact to the selected segment / list', 'autonami-automations-connectors' );
	...
	public function get_view() {
		...
		<script type="text/html" id="tmpl-action-<?php echo esc_attr__( $unique_slug ); ?>">
            ...
            <label for="" class="bwfan-label-title">
				...
				$message = __( 'Select segment to add contact to and if unable to locate then sync the connector.', 'autonami-automations-connectors' );
	...
	protected function handle_response( $result, $call_object = null ) {
		...
		if ( isset( $result['body']['success'] ) && $result['body']['success'] ) {
			return array(
				'status'  => 3,
				'message' => __( 'Contact added to Segment successfully!', 'autonami-automations-connectors' ),
			);
		}
 ...
 */
return 'BWFAN_Mautic_Add_To_Segment';
