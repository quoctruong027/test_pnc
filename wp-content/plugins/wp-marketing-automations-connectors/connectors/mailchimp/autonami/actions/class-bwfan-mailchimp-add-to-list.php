...
final class BWFAN_Mailchimp_Add_To_List extends BWFAN_Action {

	private static $ins = null;

	private function __construct() {
		$this->action_name     = __( 'Add Contact to List', 'autonami-automations-connectors' );
		$this->action_desc     = __( 'This action adds / create the contact to the selected list', 'autonami-automations-connectors' );
	...
	protected function handle_response( $result, $call_object = null ) {
		...
		if ( isset( $result['response'] ) && 200 === absint( $result['response'] ) ) {
			return array(
				'status'  => 3,
				'message' => __( 'Added / Created to list successfully!', 'autonami-automations-connectors' ),
			);
		}
 ...
 */
return 'BWFAN_Mailchimp_Add_To_List';
