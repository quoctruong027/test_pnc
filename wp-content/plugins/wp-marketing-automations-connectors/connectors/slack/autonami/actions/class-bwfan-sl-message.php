...
final class BWFAN_SL_Message extends BWFAN_Action {

	private static $ins = null;

	private function __construct() {
		$this->action_name     = __( 'Sends a message to a channel', 'autonami-automations-connectors' );
		$this->action_desc     = __( 'This action sends a Slack message on a selected channel', 'autonami-automations-connectors' );
	...
	public function admin_enqueue_assets() {
		if ( BWFAN_Common::is_load_admin_assets( 'automation' ) ) {
			...
			BWFAN_Core()->admin->set_actions_js_data( $this->get_class_slug(), 'slack_channel_options', $data );
		}
	}

	public function get_view_data() {
		$channels = WFCO_Common::get_single_connector_data( $this->connector, 'channels' );
	...
	public function get_view() {
        ...
        <script type="text/html" id="tmpl-action-<?php echo esc_attr__( $unique_slug ); ?>">
            <#
            selected_slack_channel = (_.has(data.actionSavedData, 'data') && _.has(data.actionSavedData.data, 'slack_channel')) ? data.actionSavedData.data.slack_channel : '';
            body = (_.has(data.actionSavedData, 'data') && _.has(data.actionSavedData.data, 'body')) ? data.actionSavedData.data.body : '';
            #>
            <label for="" class="bwfan-label-title">
				<?php
				echo esc_html__( 'Select Channel', 'autonami-automations-connectors' );
				$message = __( 'Select a channel and if unable to locate then sync the connector.', 'autonami-automations-connectors' );
				echo $this->add_description( $message, '2xl', 'right' ); //phpcs:ignore WordPress.Security.EscapeOutput
				?>
            </label>
            <select required id="" class="bwfan-single-select bwfan-input-wrapper" name="bwfan[{{data.action_id}}][data][slack_channel]">
                <option value=""><?php echo esc_html__( 'Choose Channel', 'autonami-automations-connectors' ); ?></option>
                <#
                if(_.has(data.actionFieldsOptions, 'slack_channel_options') && _.isObject(data.actionFieldsOptions.slack_channel_options) ) {
                _.each( data.actionFieldsOptions.slack_channel_options, function( value, key ){
                selected = (key == selected_slack_channel) ? 'selected' : '';
                #>
            ...
            <label for="" class="bwfan-label-title">
				...
				$message = __( 'Message to be sent in the channel selected above.', 'autonami-automations-connectors' );
				echo $this->add_description( $message, '2xl', 'right' ); //phpcs:ignore WordPress.Security.EscapeOutput
	...
	public function make_data( $integration_object, $task_meta ) {
		...
		$data_to_set['channel']      = $task_meta['data']['slack_channel'];
