...
if (!class_exists('A2W_Migrate')) {
    ...
    class A2W_Migrate {
        ...
        public function migrate(){
            ...
            if(version_compare($cur_version, "1.11.8", '<')) {
                $this->migrate_to_1_11_8();
            }
        ...
        private function migrate_to_170(){
            error_log('migrate to 1.7.0');

            a2w_set_setting('api_endpoint', 'https://api.ali2woo.com/v1/');
        }

        private function migrate_to_1814(){
            error_log('migrate to 1.8.14');

            a2w_set_setting('pricing_rules_type', 'sale_price_and_discount');
        }

        private function migrate_to_11100(){
            error_log('migrate to 1.11.00');

            a2w_set_setting('chrome_ext_import', false);
        }

        private function migrate_to_1_11_8(){
            error_log('migrate to 1.11.8');

            global $wpdb;

            $wpdb->query("UPDATE {$wpdb->comments} c INNER JOIN {$wpdb->commentmeta} cm ON (cm.comment_id=c.comment_ID) SET c.comment_type='review' WHERE (c.comment_type='' OR c.comment_type IS NULL) AND cm.meta_key='a2w_cash'");
