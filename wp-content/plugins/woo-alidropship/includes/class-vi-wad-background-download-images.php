...
class Vi_WAD_Background_Download_Images extends WP_Background_Process {
		...
		try {
			...
			if ( $post && $post->post_type === 'product' && $src ) {
				...
				if ( $thumb_id && ! is_wp_error( $thumb_id ) ) {
					...
					error_log( 'Dropshipping and Fulfillment for AliExpress and WooCommerce error: ' . $thumb_id->get_error_code() . ' - ' . $thumb_id->get_error_message() );
				}
			}

		} catch ( Exception $e ) {
			VI_WOO_ALIDROPSHIP_Error_Images_Table::insert( $product_id, implode( ',', $product_ids ), $src, intval( $set_gallery ) );
			error_log( 'Dropshipping and Fulfillment for AliExpress and WooCommerce error: ' . $e->getMessage() );
	...
	protected function complete() {
		if ( ! $this->is_downloading() && ! $this->is_queue_empty() ) {
