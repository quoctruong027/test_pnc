...
class WCUF_Email  
{
	...
	public function trigger( $links_to_notify_via_mail, $order , $attachment = array()) 
	{
		...
		foreach($links_to_notify_via_mail as $recipients => $meta)
		{
			...
			$subject = apply_filters('wcuf_email_notification_subject', __('User submitted new upload for order number: ', 'woocommerce-files-upload').$order_id, $order);
			...
			if($wcuf_upload_field_model->can_be_zip_file_created_upload_field_content($meta['order_meta']))
				$content .= '<br/><a href="'.get_site_url().'?wcuf_create_zip_for_field='.$meta['order_meta']['id'].'&wcuf_order_id='.$order_id.'" a>'.__('Download as zip', 'woocommerce-files-upload').'</a>';
