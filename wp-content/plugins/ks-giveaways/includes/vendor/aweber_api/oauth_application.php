<?php
if (!class_exists('KS_Giveaways_Vendor_CurlObject')) require_once('curl_object.php');
if (!class_exists('KS_Giveaways_Vendor_CurlResponse')) require_once('curl_response.php');
 ...
 * @version $id$
 */
interface KS_Giveaways_Vendor_OAuthServiceProvider {
 ...
 * @version $id$
 */
class KS_Giveaways_Vendor_OAuthApplication implements KS_Giveaways_Vendor_AWeberOAuthAdapter {
    public $debug = false;
    ...
    public function __construct($parentApp = false) {
        if ($parentApp) {
            if (!is_a($parentApp, 'KS_Giveaways_Vendor_OAuthServiceProvider')) {
                throw new Exception('Parent App must be a valid KS_Giveaways_Vendor_OAuthServiceProvider!');
            }
            $this->app = $parentApp;
        }
        $this->user = new KS_Giveaways_Vendor_OAuthUser();
        $this->curl = new KS_Giveaways_Vendor_CurlObject();
    }
    ...
    public function request($method, $uri, $data = array(), $options = array()) {
        ...
        if (empty($options['allow_empty']) && !isset($data)) {
            throw new KS_Giveaways_Vendor_AWeberResponseError($uri);
        }
        return $data;
    }
    ...
    public function getAccessToken() {
        ...
        if (empty($data['oauth_token'])) {
            throw new KS_Giveaways_Vendor_AWeberOAuthDataMissing('oauth_token');
        }
    ...
    public function parseAsError($response) {
        if (!empty($response['error'])) {
            throw new KS_Giveaways_Vendor_AWeberOAuthException($response['error']['type'],
    ...
    protected function requiredFromResponse($data, $requiredFields) {
        foreach ($requiredFields as $field) {
            if (empty($data[$field])) {
                throw new KS_Giveaways_Vendor_AWeberOAuthDataMissing($field);
            }
        }
    }
    ...
    public function makeRequest($method, $url, $data=array()) {
        ...
        if (!$resp) {
            ...
            throw new KS_Giveaways_Vendor_AWeberAPIException($error, $url);
        }

        if($resp->headers['Status-Code'] >= 400) {
            $data = json_decode($resp->body, true);
            throw new KS_Giveaways_Vendor_AWeberAPIException($data['error'], $url);
        }
    ...
    private function _sendRequest($handle, $headers = array('Expect:')) {
        ...
        if ($resp) {
            return new KS_Giveaways_Vendor_CurlResponse($resp);
        }
 ...
 * @version $id$
 */
class KS_Giveaways_Vendor_OAuthUser {
