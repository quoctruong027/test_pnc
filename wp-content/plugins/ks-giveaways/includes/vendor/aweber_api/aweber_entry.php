...
class KS_Giveaways_Vendor_AWeberEntry extends KS_Giveaways_Vendor_AWeberResponse {
    ...
    public function attrs() {
        ...
        if (!empty(KS_Giveaways_Vendor_AWeberAPI::$_collectionMap[$this->type])) {
            foreach (KS_Giveaways_Vendor_AWeberAPI::$_collectionMap[$this->type] as $child) {
     ...
     *
     * @access public
     * @param KS_Giveaways_Vendor_AWeberEntry(List)   List to move Resource (this) too.
    ...
    public function move($list, $last_followup_message_number_sent=NULL) {
        ...
        return new KS_Giveaways_Vendor_AWeberEntry($resource_data, $url, $this->adapter);
    }
     ...
     * @access public
     * @throws KS_Giveaways_Vendor_AWeberResourceNotImplemented
     * @return mixed
     */
    public function __get($value) {
        ...
        if (!empty($this->data) && array_key_exists($value, $this->data)) {
            if (is_array($this->data[$value])) {
                $array = new KS_Giveaways_Vendor_AWeberEntryDataArray($this->data[$value], $value, $this);
        ...
        throw new KS_Giveaways_Vendor_AWeberResourceNotImplemented($this, $value);
    }
     ...
     * @access public
     * @return KS_Giveaways_Vendor_AWeberCollection
     */
    public function findSubscribers($search_data) {
        ...
        return new KS_Giveaways_Vendor_AWeberCollection($data, $url, $this->adapter);
    }
    ...
    public function getParentEntry(){
        ...
        try {
            ...
            return new KS_Giveaways_Vendor_AWeberEntry($data, $url, $this->adapter);
        } catch (Exception $e) {
            return NULL;
        }
    }
    ...
    protected function _parseNamedOperation($data) {
        ...
        foreach($data as $entryData) {
            $results[] = new KS_Giveaways_Vendor_AWeberEntry($entryData, str_replace($this->adapter->app->getBaseUri(), '',
    ...
    protected function _methodFor($entryTypes) {
        ...
        throw new KS_Giveaways_Vendor_AWeberMethodNotImplemented($this);
    }
    ...
    protected function _getCollection($value) {
        if (empty($this->_collections[$value])) {
            $url = "{$this->url}/{$value}";
            $data = $this->adapter->request('GET', $url);
            $this->_collections[$value] = new KS_Giveaways_Vendor_AWeberCollection($data, $url, $this->adapter);
        }
    ...
    protected function _isChildCollection($value) {
        $this->_type();
        if (!empty(KS_Giveaways_Vendor_AWeberAPI::$_collectionMap[$this->type]) &&
            in_array($value, KS_Giveaways_Vendor_AWeberAPI::$_collectionMap[$this->type])) return true;
        return false;
    }
