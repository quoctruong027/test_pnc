 ...
 * @uses KS_Giveaways_Vendor_OAuthServiceProvider
 * @package
 * @version $id$
 */
class KS_Giveaways_Vendor_AWeberServiceProvider implements KS_Giveaways_Vendor_OAuthServiceProvider {
 ...
 * @version $id$
 */
class KS_Giveaways_Vendor_AWeberAPIBase {
     ...
     * @access public
     * @return KS_Giveaways_Vendor_AWeberEntry or KS_Giveaways_Vendor_AWeberCollection
     */
    ...
    protected function readResponse($response, $url) {
        ...
        if (!empty($response['id'])) {
            return new KS_Giveaways_Vendor_AWeberEntry($response, $url, $this->adapter);
        } else if (array_key_exists('entries', $response)) {
            return new KS_Giveaways_Vendor_AWeberCollection($response, $url, $this->adapter);
        }
        return false;
    }
}
 ...
 * @version $id$
 */
class KS_Giveaways_Vendor_AWeberAPI extends KS_Giveaways_Vendor_AWeberAPIBase {
     ...
     *
     * @param String KS_Giveaways_Vendor_Authorization code from authorize app page
     */
    public static function getDataFromAweberID($string) {
        list($consumerKey, $consumerSecret, $requestToken, $tokenSecret, $verifier) = KS_Giveaways_Vendor_AWeberAPI::_parseAweberID($string);

        if (!$verifier) {
            return null;
        }
        $aweber = new KS_Giveaways_Vendor_AweberAPI($consumerKey, $consumerSecret);
     ...
     *
     * @param String KS_Giveaways_Vendor_Consumer Key
     * @param String KS_Giveaways_Vendor_Consumer Secret
     * @return null
     */
    ...
    public function setAdapter($adapter=null) {
        if (empty($adapter)) {
            $serviceProvider = new KS_Giveaways_Vendor_AWeberServiceProvider();
            $adapter = new KS_Giveaways_Vendor_OAuthApplication($serviceProvider);
     ...
     *
     * @param String KS_Giveaways_Vendor_Access Token (Only optional/cached if you called getAccessToken() earlier
     *      on the same page)
     * @param String KS_Giveaways_Vendor_Access Token Secret (Only optional/cached if you called getAccessToken() earlier
     *      on the same page)
    ...
    public function getAccount($token=false, $secret=false) {
        if ($token && $secret) {
            $user = new KS_Giveaways_Vendor_OAuthUser();
