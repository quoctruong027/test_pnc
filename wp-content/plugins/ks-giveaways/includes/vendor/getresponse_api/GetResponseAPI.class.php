 ...
 * @package GetResponsePHP
 */
class KS_Giveaways_Vendor_GetResponse
{	
	/**
	 * GetResponse API key
	 ...
	 * @param string $cycle_day Custom Modification - Cycle Day for Auto-responders
	 * @param string $name Name
	 * @return object
	 */
	public function addContact($campaign, $email, $cycle_day = "0", $name = null)
	{
		$params = array();
		...
		// Add a cycle_day parameter of 0 - otherwise the contact won't get auto registered
		// to an auto responder. Issue 3318.
		$params['cycle_day'] = (string) $cycle_day;

		$request  = $this->prepRequest('add_contact', $params);

		$response = $this->execute2($request);
		return $response;
	}
	 ...
	 * Executes an API call.
	 *
	 * Modified from the original version because the original lacks detailed error response.
	 *
	 * @param string $request JSON encoded array
	 * @return object
	 * @access private
	 */
	private function execute2($request)
	{
		$handle = curl_init($this->apiURL);
		curl_setopt($handle, CURLOPT_POST, 1);
		curl_setopt($handle, CURLOPT_POSTFIELDS, $request);
