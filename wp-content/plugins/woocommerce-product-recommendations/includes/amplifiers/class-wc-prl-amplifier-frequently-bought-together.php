...
class WC_PRL_Amplifier_Frequently_Bought_Together extends WC_PRL_Amplifier {
	...
	public function run_step( $step_index, $deployment, $args = array() ) {
		...
		$time_period_in_weeks = (int) apply_filters( 'woocommerce_prl_amp_fgt_time_weeks', 0, $deployment );
		...
		switch ( $step_index ) {
			...
			case 1:
				...
				$sql = "SELECT count( distinct( `order_id` ) )
						...
						;
				$total_orders_containing_source = (int) $wpdb->get_var( $sql );
				...
				if ( empty( $total_orders_containing_source ) || $total_orders_containing_source < apply_filters( 'woocommerce_prl_amp_fgt_foreground_order_count_min', 10, $deployment ) ) {
					return null;
				}
			...
			case 2:
				...
				$having_support_percentage      = (float) apply_filters( 'woocommerce_prl_amp_fgt_foreground_support_pct_min', 10, $deployment );
				$support_number                 = max( (int) $total_orders_containing_source * $having_support_percentage / 100, apply_filters( 'woocommerce_prl_amp_fgt_foreground_count_min', 10, $deployment ) );
				$results                        = array();
			...
			case 3:
				...
				$sql = "SELECT count( * )
						...
						;
				$total_orders = (int) $wpdb->get_var( $sql );
			...
			case 4:
				...
				$background_support_min_count = (int) apply_filters( 'woocommerce_prl_amp_fgt_background_count_min', 10, $deployment );
			...
			case 5:
				...
				if ( ! empty( $products_to_calc ) && 0 != $total_orders_containing_source ) {
					 ...
					 */
					$significance_score = apply_filters( 'woocommerce_prl_fgt_significance_score', 'jlh', $deployment );

					if ( 'jlh' === $significance_score ) {
						$jlh_threshold = (float) apply_filters( 'woocommerce_prl_amp_fgt_jlh_threshold', 0.2, $deployment );
					} elseif ( 'gnd' === $significance_score ) {
						$gnd_threshold = (float) apply_filters( 'woocommerce_prl_amp_fgt_gnd_threshold', 0.8, $deployment );
					}
