<?php
/**
 * WC_PRL_Background_Updater class
 *
 ...
 *
 * @class    WC_PRL_Background_Updater
 * @version  1.0.0
 */
class WC_PRL_Background_Updater extends WP_Background_Process {
	...
	public function dispatch() {
		...
		if ( is_wp_error( $dispatched ) ) {
			WC_PRL()->log( sprintf( 'Unable to dispatch WooCommerce Product Recommendations updater: %s', $dispatched->get_error_message() ), 'error', 'wc_prl_db_updates' );
		}
	}
	...
	protected function task( $callback ) {
		...
		include_once( WC_PRL_ABSPATH . 'includes/wc-prl-update-functions.php' );

		if ( is_callable( $callback ) ) {

			WC_PRL()->log( sprintf( '- Running %s callback...', $callback ), 'info', 'wc_prl_db_updates' );
			...
			WC_PRL()->log( $message, 'info', 'wc_prl_db_updates' );

		} else {

			$result = false;

			WC_PRL()->log( sprintf( '- Could not find %s callback.', $callback ), 'notice', 'wc_prl_db_updates' );
		}
	...
	protected function complete() {
		WC_PRL_Install::update_complete();
		parent::complete();
	}
}
