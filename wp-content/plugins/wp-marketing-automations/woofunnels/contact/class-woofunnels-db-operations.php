...
class WooFunnels_DB_Operations {
	...
	public function insert_contact( $contact ) {
		...
		$inserted     = $this->wp_db->insert( $this->contact_tbl, $contact_data );
	...
	public function update_contact( $contact ) {
		...
		if ( $this->wp_db->last_error !== '' ) {
			WooFunnels_Dashboard::$classes['BWF_Logger']->log( "Get last error in update_custoemr for cid: {$contact['cid']} " . print_r( $this->wp_db->last_error, true ), 'woofunnels_indexing' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
		}
	}
	...
	public function get_contacts( $args ) {
		...
		if ( ! empty( $args['min_creation_date'] ) ) {
			$query['where'] .= "
				AND 	contact.creation_date >= '" . date( 'Y-m-d H:i:s', $args['min_creation_date'] ) . "'
			";
		}

		if ( ! empty( $args['max_creation_date'] ) ) {
			$query['where'] .= "
				AND 	contact.creation_date < '" . date( 'Y-m-d H:i:s', $args['max_creation_date'] ) . "'
			";
		}
	...
	public function update_customer( $customer ) {
		...
		if ( $this->wp_db->last_error !== '' ) {
			WooFunnels_Dashboard::$classes['BWF_Logger']->log( "Get last error in update_custoemr for cid: {$customer['cid']} " . print_r( $this->wp_db->last_error, true ), 'woofunnels_indexing' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
		}
	}
	...
	public function get_customers( $args ) {
		...
		if ( ! empty( $args['min_last_order_date'] ) ) {
			$query['where'] .= "
				AND 	customer.l_order_date >= '" . date( 'Y-m-d H:i:s', $args['min_last_order_date'] ) . "'
			";
		}

		if ( ! empty( $args['max_last_order_date'] ) ) {
			$query['where'] .= "
				AND 	customer.l_order_date < '" . date( 'Y-m-d H:i:s', $args['max_last_order_date'] ) . "'
			";
		}

		if ( ! empty( $args['min_creation_date'] ) ) {
			$query['where'] .= "
				AND 	customer.creation_date >= '" . date( 'Y-m-d H:i:s', $args['min_creation_date'] ) . "'
			";
		}

		if ( ! empty( $args['max_creation_date'] ) ) {
			$query['where'] .= "
				AND 	customer.creation_date < '" . date( 'Y-m-d H:i:s', $args['max_creation_date'] ) . "'
			";
		}
