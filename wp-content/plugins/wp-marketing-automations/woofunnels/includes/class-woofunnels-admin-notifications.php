...
class WooFunnels_Admin_Notifications {
	...
	public static function hide_notices() {
		if ( isset( $_GET['woofunnels-hide-notice'] ) && isset( $_GET['_woofunnels_notice_nonce'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			if ( ! wp_verify_nonce( sanitize_text_field( $_GET['_woofunnels_notice_nonce'] ), 'woofunnels_hide_notices_nonce' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			...
			$hide_notice = sanitize_text_field( $_GET['woofunnels-hide-notice'] ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public static function remove_notice( $name ) {
		...
		if ( $has_notice_removed && ! empty( $has_notice_removed ) ) {
			...
			$array_to_push = array_push( get_option( 'woofunnels_admin_notices' ), $name );
