...
class WooFunnels_Deactivate {
	...
	public static function _submit_uninstall_reason_action() {
		if ( ! isset( $_POST['reason_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			exit;
		}

		$reason_info = isset( $_REQUEST['reason_info'] ) ? trim( stripslashes( sanitize_text_field( $_REQUEST['reason_info'] ) ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification

		$reason = array(
			'id'   => sanitize_text_field( $_POST['reason_id'] ), // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			'info' => substr( $reason_info, 0, 128 ),
		);

		$licenses        = WooFunnels_addons::get_installed_plugins();
		$version         = 'NA';
		$plugin_basename = isset( $_POST['plugin_basename'] ) ? sanitize_text_field( $_POST['plugin_basename'] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		...
		$license_info       = isset( $_REQUEST['licenses'] ) ? json_decode( stripslashes( sanitize_text_field( $_REQUEST['licenses'] ) ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
