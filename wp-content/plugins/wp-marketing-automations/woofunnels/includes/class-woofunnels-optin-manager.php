...
class WooFunnels_OptIn_Manager {
	...
	public static function maybe_clear_optin() {
		if ( isset( $_GET['woofunnels_tracking'] ) && ( 'reset' === sanitize_text_field( $_GET['woofunnels_tracking'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			self::reset_optin();
	...
	public static function woofunnelso_optin_call() {
		if ( is_array( $_POST ) && count( $_POST ) > 0 ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$_POST['domain'] = home_url();
			$_POST['ip']     = isset( $_SERVER['REMOTE_ADDR'] ) ? sanitize_text_field( $_SERVER['REMOTE_ADDR'] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			WooFunnels_API::post_optin_data( $_POST ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification

			/** scheduling track call when success */
			if ( isset( $_POST['status'] ) && 'yes' === sanitize_text_field( $_POST['status'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
