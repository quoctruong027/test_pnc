...
final class WooFunnels_AS_DS {
	...
	public function as_pre_init_cb() {
		...
		if ( isset( $_GET['rest_route'] ) && false !== strpos( $_GET['rest_route'], '/woofunnels/v1/worker' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$is_worker_request = true;
		} else if ( isset( $_SERVER['REQUEST_URI'] ) && false !== strpos( $_SERVER['REQUEST_URI'], '/woofunnels/v1/worker' ) ) {
			$is_worker_request = true;
		}
