...
class WFCO_AJAX_Controller {
	...
	public static function update_connector() {
		...
		if ( empty( $_REQUEST ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			wp_send_json( $resp );
		}

		/** Connector slug and connector saved id doesn't exist */
		if ( ! isset( $_REQUEST['wfco_connector'] ) || empty( $_REQUEST['wfco_connector'] ) || ! isset( $_REQUEST['id'] ) || empty( $_REQUEST['id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$resp['message'] = 'Connector saved data missing, kindly disconnect and connect again.';
			wp_send_json( $resp );
		}

		/** Couldn't able to verify the nonce */
		if ( isset( $_REQUEST['edit_nonce'] ) && false === wp_verify_nonce( sanitize_text_field( $_REQUEST['edit_nonce'] ), 'wfco-connector-edit' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$resp['message'] = 'Something is wrong, couldn\'t verify the call.';
			wp_send_json( $resp );
		}

		$active_connectors = WFCO_Load_Connectors::get_active_connectors();
		if ( ! $active_connectors[ sanitize_text_field( $_REQUEST['wfco_connector'] ) ] instanceof BWF_CO ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$resp['message'] = 'Something is wrong, connector isn\'t available.';
			wp_send_json( $resp );
		}

		/** @var $connector_ins BWF_CO */
		$connector_ins   = $active_connectors[ sanitize_text_field( $_REQUEST['wfco_connector'] ) ]; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		$response        = $connector_ins->handle_settings_form( $_REQUEST, 'update' ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public static function save_connector() {
		...
		/** Connector slug and connector saved id doesn't exist */
		if ( ! isset( $_REQUEST['wfco_connector'] ) || empty( $_REQUEST['wfco_connector'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$resp['message'] = 'Something is wrong, couldn\'t find the connector.';
			wp_send_json( $resp );
		}

		/** Couldn't able to verify the nonce */
		if ( isset( $_REQUEST['_wpnonce'] ) && false === wp_verify_nonce( sanitize_text_field( $_REQUEST['_wpnonce'] ), 'wfco-connector' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$resp['message'] = 'Something is wrong, couldn\'t verify the call.';
			wp_send_json( $resp );
		}
		...
		$response        = $active_connectors[ sanitize_text_field( $_REQUEST['wfco_connector'] ) ]->handle_settings_form( $_REQUEST, 'save' ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		$resp['status']  = ( 'success' === $response['status'] ) ? true : false;
		$resp['message'] = $response['message'];
	...
	public static function sync_connector() {
		...
		/** Connector slug and connector saved id doesn't exist */
		if ( ! isset( $_REQUEST['slug'] ) || empty( $_REQUEST['slug'] ) || ! isset( $_REQUEST['id'] ) || empty( $_REQUEST['id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$resp['message'] = 'Connector saved data missing, kindly disconnect and connect again.';
			wp_send_json( $resp );
		}

		/** Couldn't able to verify the nonce */
		if ( isset( $_REQUEST['sync_nonce'] ) && false === wp_verify_nonce( sanitize_text_field( $_REQUEST['sync_nonce'] ), 'wfco-connector-sync' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$resp['message'] = 'Something is wrong, couldn\'t verify the call.';
			wp_send_json( $resp );
		}

		$slug              = trim( sanitize_text_field( $_REQUEST['slug'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		...
		try {
			$response = $current_connector->handle_settings_form( $_REQUEST, 'sync' ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public static function delete_connector() {
		...
		if ( isset( $_REQUEST['id'] ) && '' !== sanitize_text_field( $_REQUEST['id'] ) && isset( $_REQUEST['delete_nonce'] ) && wp_verify_nonce( sanitize_text_field( $_REQUEST['delete_nonce'] ), 'wfco-connector-delete' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification

			$connector_id      = sanitize_text_field( $_REQUEST['id'] ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public static function connector_install() {
		...
		if ( isset( $_REQUEST['connector_slug'] ) && '' !== sanitize_text_field( $_REQUEST['connector_slug'] ) && isset( $_REQUEST['install_nonce'] ) && wp_verify_nonce( sanitize_text_field( $_REQUEST['install_nonce'] ), 'wfco-connector-install' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification

			$encode_slug       = isset( $_REQUEST['slug'] ) ? trim( sanitize_text_field( $_REQUEST['slug'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$type              = isset( $_REQUEST['type'] ) ? trim( sanitize_text_field( $_REQUEST['type'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public static function create_connector_license() {
		...
		$resp['status'] = false;
		if ( isset( $_REQUEST['connector_slug'] ) && '' !== sanitize_text_field( $_REQUEST['connector_slug'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$connector_slug  = trim( sanitize_text_field( $_REQUEST['connector_slug'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$connector_slugs = isset( $_REQUEST['slug'] ) ? trim( sanitize_text_field( $_REQUEST['slug'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$type            = isset( $_REQUEST['type'] ) ? trim( sanitize_text_field( $_REQUEST['type'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
