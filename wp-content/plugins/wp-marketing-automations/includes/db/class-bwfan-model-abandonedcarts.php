...
class BWFAN_Model_Abandonedcarts extends BWFAN_Model {
	...
	public static function get_abandoned_data( $where = '', $offset = '', $per_page = '', $order_by = 'ID' ) {
		global $wpdb;
		...
		$results = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}bwfan_abandonedcarts {$where} ORDER BY {$order_by} DESC {$limit_string}" ); // WPCS: unprepared SQL OK
