...
class BWFAN_Tasks {
	...
	public function bwfan_ac_execute_task( $task_id ) {
		try {
			...
			if ( false === $validate ) {
				...
				/** Delete task if required data is missing */
	...
	public function execute_tasks( $processed_actions_data, $task_details ) {
		...
		$action_result['message'] = isset( $action_result['message'] ) ? $action_result['message'] : '';
	...
	public function insert_task( $automation_id, $task_data, $event_object ) {
		...
		if ( 'after_delay' === $task_data['time']['delay_type'] ) {
		...
		if ( 'fixed' === $task_data['time']['delay_type'] && isset( $task_data['time']['fixed_date'] ) && ! empty( $task_data['time']['fixed_date'] ) ) {
	...
	public function get_all_tasks( $no_limit = null ) {
		...
		$query_from   = 'FROM ' . $wpdb->prefix . 'bwfan_tasks';
		$query_where  = 'WHERE 1=1';
		$query_order  = 'ORDER BY e_date ASC, ID ASC';
		$query_limit  = '';
		$params       = [];
		...
		if ( isset( $_GET['s'] ) && ! empty( $_GET['s'] ) ) { // phpcs:ignore WordPress.VIP.SuperGlobalInputUsage.AccessDetected, WordPress.CSRF.NonceVerification.NoNonceVerification
			$search                          = true;
			$query_select                    = 'SELECT t.ID, t.integration_slug, t.integration_action, t.automation_id, t.status, t.e_date ';
			$query_from                      .= ' as t';
			$query_from                      .= ' LEFT JOIN ' . $wpdb->prefix . 'bwfan_taskmeta as m';
			$query_from                      .= ' ON t.ID = m.bwfan_task_id';
			...
			$query_order                     = ' ORDER BY t.e_date ASC, t.ID ASC';
			$this->show_filtered_tasks_count = true;
		}

		if ( ! is_null( $automation_id ) ) {
			$query_where                     .= ( true === $search ) ? ' AND t.automation_id = %d' : ' AND `automation_id` = %d';
			$params[]                        = $automation_id;
			$this->show_filtered_tasks_count = true;
		}
		if ( ! is_null( $task_action ) ) {
			$query_where                     .= ( true === $search ) ? ' AND t.integration_action = %s' : ' AND `integration_action` = %s';
			$params[]                        = $task_action;
			$this->show_filtered_tasks_count = true;
		}
		if ( ! is_null( $task_status ) ) {
			$query_where                     .= ( true === $search ) ? ' AND t.status = %d' : ' AND `status` = %d';
			$params[]                        = $task_status;
			$this->show_filtered_tasks_count = true;
		}
		...
		if ( $this->show_filtered_tasks_count ) {
			...
			if ( true === $search ) {
				$query_select = 'SELECT COUNT(t.ID) as `count`';
			}
	...
	public function fetch_tasks_count( $status ) {
		global $wpdb;
		$query         = 'SELECT count(ID) as tasks_count FROM {table_name} WHERE status = %d';
		...
		if ( ! is_null( $automation_id ) ) {
			$query = 'SELECT count(ID) as tasks_count FROM {table_name} WHERE status = %d and automation_id = %d';
