...
abstract class  BWFAN_Event {
	...
	public function check_language_support( $action_data, $data ) {
		if ( ! function_exists( 'icl_get_languages' ) || false === $this->support_lang || ! isset( $action_data['language'] ) || ! isset( $action_data['language']['enable_lang'] ) || 1 !== absint( $action_data['language']['enable_lang'] ) && ! isset( $action_data['language']['lang'] ) || empty( $action_data['language']['lang'] ) ) {
			return true;
		}
	...
	public function send_async_contact_call( $automation_id ) {
		...
		$url  = add_query_arg(
			array(
				'rest_route' => '/autonami/v1/update-contact-automation',
			), $url
		);
		$body_data = array(
