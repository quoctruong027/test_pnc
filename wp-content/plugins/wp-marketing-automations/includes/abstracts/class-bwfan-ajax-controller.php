...
abstract class BWFAN_AJAX_Controller {
	...
	public static function handle_automation_post_submit() {
		...
		if ( ! is_null( $t_to_delete ) ) {
			...
			$url       = add_query_arg( array(
				'rest_route' => '/autonami/v1/delete-tasks',
			), $url );
			$body_data = array(
	...
	public static function handle_bwfan_global_tools() {
		...
		switch ( $_POST['tool_type'] ) {
			...
			case 'delete_previous_logs_automation':
				...
				$data_inputs           = isset( $_POST['data_inputs'] ) ? json_decode( BWFAN_Common::remove_backslashes( $_POST['data_inputs'] ), true ) : array(); //phpcs:ignore WordPress.Security.ValidatedSanitizedInput
	...
	public static function test_email() {
		...
		$post['data']['subject']  = isset( $post['data']['subject'] ) ? stripslashes( $post['data']['subject'] ) : '';
		$post['data']['body']     = stripslashes( $post['data']['body'] );//phpcs:ignore WordPress.Security.ValidatedSanitizedInput
		$post['data']['body_raw'] = stripslashes( $post['data']['body_raw'] );//phpcs:ignore WordPress.Security.ValidatedSanitizedInput
	...
	public static function handle_single_automation_export() {
		...
		echo esc_url_raw( admin_url() . 'admin.php?page=autonami&action=export&data=1&automation=' . $automation_id );
	...
	public static function handle_duplicate_single_automation() {
		...
		wp_send_json( [ 'status' => 1, 'url' => admin_url( 'admin.php?page=autonami' ) ] );
	}
	...
	public static function handle_export_all_automation() {
		echo esc_url_raw( admin_url() . 'admin.php?page=autonami&action=export&data=1&automation=all' );
