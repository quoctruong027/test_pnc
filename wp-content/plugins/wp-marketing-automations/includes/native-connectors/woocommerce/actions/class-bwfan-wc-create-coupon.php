...
final class BWFAN_WC_Create_Coupon extends BWFAN_Action {
	...
	public function admin_enqueue_assets() {
		if ( BWFAN_Common::is_load_admin_assets( 'automation' ) ) {
			...
			BWFAN_Core()->admin->set_actions_js_data( $this->get_class_slug(), 'expiry_type_options', $time_types );
	...
	public function get_view() {
        ...
        <script type="text/html" id="tmpl-action-<?php esc_attr_e( $this->get_slug() ); ?>">
            ...
            selected_time_type = (_.has(data.actionSavedData, 'data') && _.has(data.actionSavedData.data, 'expiry_type')) ? data.actionSavedData.data.expiry_type : '';
            ...
            <div class="bwfan-input-form clearfix">
                <label for="" class="bwfan-label-title"><?php esc_html_e( 'Coupon Expiry', 'wp-marketing-automations' ); ?></label>
                ...
                <div class="bwfan-col-sm-5 bwfan-pl-0">
                    <select required id="" class="bwfan-input-wrapper" name="bwfan[{{data.action_id}}][data][expiry_type]">
                        <option value=""><?php esc_html_e( 'Select Type', 'wp-marketing-automations' ); ?></option>
                        <#
                        if(_.has(data.actionFieldsOptions, 'expiry_type_options') && _.isObject(data.actionFieldsOptions.expiry_type_options) ) {
                        _.each( data.actionFieldsOptions.expiry_type_options, function( value, key ){
                        selected = (key == selected_time_type) ? 'selected' : '';
                        #>
	...
	public function make_data( $integration_object, $task_meta ) {
		...
		$data_to_set['expiry_type'] = $task_meta['data']['expiry_type'];
	...
	public function process() {
		...
		if ( $get_wc_coupon->get_id() === 0 ) {
			...
			if ( ! empty( $data['expiry'] ) ) {
				$expiry                          = $this->get_expiry_dates( (int) $data['expiry'], $data['expiry_type'] );
	...
	public function get_expiry_dates( $no_of_days, $type ) {
		$dbj       = new DateTime();
		$timestamp = strtotime( '+1 ' . $type );
		...
		$dbj->setTimestamp( $timestamp );
		$exp_date_email   = $dbj->format( 'Y-m-d h:i:s' );
		$expiry_timestamp = strtotime( $exp_date_email );
		$no_of_days       = (int) $no_of_days;

		if ( $no_of_days > 0 ) {
			$no_of_days_pl = $no_of_days;
			$exptime       = strtotime( "+{$no_of_days_pl} " . $type );

			$dbj->setTimestamp( $exptime );
			$exp_date         = $dbj->format( 'Y-m-d' );
			$no_of_days       += 1;
			$exptime          = strtotime( "+{$no_of_days} " . $type );
