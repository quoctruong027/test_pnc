...
class BWFAN_Abandoned_Cart {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_checkout_order_processed', [ $this, 'save_marketing_status_for_user' ], 20, 3 );
		...
		add_action( 'init', [ $this, 'set_session_cookies' ] );
		...
		add_filter( 'wfacp_default_values', [ $this, 'prefill_embed_forms' ], 10, 2 );
		add_filter( 'wfacp_skip_add_to_cart', [ $this, 'check_aerocheckout_page' ], 10, 2 );
	 ...
	 */
	public function save_marketing_status_for_user( $order_id, $posted_data, $order ) {
		if ( ! isset( $_POST['bwfan_user_consent'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification
			return;
		}
		...
		$bwf_contact->meta->marketing_status = absint( $_POST['bwfan_user_consent'] ); //phpcs:ignore WordPress.Security.NonceVerification
	...
	public function process_guest_cart_details( $email = null, $checkout_fields_data = null ) {
		...
		if ( ! is_null( $email ) ) {
			...
			if ( false === $cart_details ) { // check cart by guest email id
				global $wpdb;
				$sql_where    = 'email = %s';
		...
		if ( false === $cart_details && false === $this->is_empty() ) { // There is no abandoned cart row for this user, create a new row and insert in table
	...
	public function update_reports_view() {
		...
		$url = home_url();
		$url = add_query_arg( array(
			'rest_route' => '/autonami/v1/update-generated-increment',
		), $url );
	...
	public function woocommerce_add_to_cart() {
		...
		$url       = home_url();
		$url       = add_query_arg( array(
			'rest_route' => '/autonami/v1/wc-add-to-cart',
		), $url );
		$body_data = array(
			'id'          => $user_id,
	...
	public function insert_abandoned_cart() {
		...
		if ( 0 === absint( $abandoned_cart_id ) ) {
			wp_send_json( array(
				'success' => false,
				'id'      => 0,
				'message' => esc_html__( 'The user email abandoned recently.', 'wp-marketing-automations' ),
			) );
		}
	...
	public static function get_woocommerce_default_checkout_nice_names() {
		return apply_filters( 'bwfan_ab_default_checkout_nice_names', array(
			...
			'current_step'         => __( 'AeroCheckout Form Current Step', 'wp-marketing-automations' ),
			'aerocheckout_page_id' => __( 'AeroCheckout Page ID', 'wp-marketing-automations' ),
			'current_page_id'      => __( 'WordPress Page ID', 'wp-marketing-automations' ),
			'wfacp_source'         => __( 'AeroCheckout Page Source', 'wp-marketing-automations' ),
