...
class BWFAN_Abandoned_Cart_Table extends WP_List_Table {
	...
	public function no_items() {
		echo wpautop( __( 'No recoverable cart available.', 'wp-marketing-automations' ) ); //phpcs:ignore WordPress.Security.EscapeOutput
	}

	/** Made the data for abandoned carts screen.
	 * @return array
	 */
	public function get_abandoned_cart_table_data() {
		global $wpdb;
	...
	public function process_bulk_action() {
		if ( isset( $_GET['bwfan_delete_recoverable_cart'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification
			$where = array(
				'ID' => sanitize_text_field( $_GET['bwfan_delete_recoverable_cart'] ), //phpcs:ignore WordPress.Security.NonceVerification
			);
		...
		/** Bulk Actions */
		if ( 'bwfan_delete_abandoned' === $_GET['action'] || 'bwfan_delete_abandoned' === sanitize_text_field( $_GET['action2'] ) ) {
			$this->delete_abandoned_cart( $ids );
		} elseif ( 'bwfan_retry_abandoned' === $_GET['action'] || 'bwfan_retry_abandoned' === sanitize_text_field( $_GET['action2'] ) ) {
			$this->retry_abandoned_cart( $ids );
		} else {
			return;
		}
	 ...
	 * @param $ids
	 */
	public function delete_abandoned_cart( $ids ) {
		if ( empty( $ids ) ) {
			return;
		}
		foreach ( $ids as $id ) {
	 ...
	 * @param $ids
	 */
	public function retry_abandoned_cart( $ids ) {
	...
	public function get_bulk_actions() {
		return array(
			'bwfan_delete_abandoned' => 'Delete',
			'bwfan_retry_abandoned'  => 'Retry',
		);
	}
	...
	public function get_status_tooltip() {
		...
		$this->tooltip['status'] = '<div class="bwfan_tooltip" data-size="5xl">
					<span class="bwfan_tooltip_text" data-position="top" style="text-align: left;">
						<b>Pending</b> - Carts where automation is scheduled to trigger. Automation will trigger after a delay of ' . $wait_time . ' min(s) if user does not complete the order.<br/><br/>
						<b>In-Progress</b> - Carts where automation has triggered and tasks are created.<br/><br/>
						<b>Pending (No Tasks Found)</b> - Carts where automation triggered but no tasks were created. This is a rare status that occurs due to misconfiguration. Check automation and Retry to create tasks.<br/><br/>
						<b>Re-Scheduled</b> - Carts where automations are re-scheduled manually from Bulk Actions.
					</span>
				</div>';
	...
	public function column_email( $item ) {
		...
		if ( ! empty( $item->user_id ) ) {
			...
			$return = apply_filters( 'bwfan_abandoned_cart_email_col', $return, $user, $item );
		}
		$return .= '</div>';
	...
	public function column_status( $item ) {
		...
		switch ( intval( $item->status ) ) {
			case 0:
				$result = '<span data-status="pending">' . __( 'Pending', 'wp-marketing-automations' ) . '</span>';
				break;
			case 1:
				$result = '<span data-status="progress">' . __( 'In-Progress', 'wp-marketing-automations' ) . '</span>';
				break;
			case 3:
				$result = '<span data-status="captured">' . __( 'Pending (No Tasks Found) ', 'wp-marketing-automations' );
				$title  = __( 'Retry cart', 'wp-marketing-automations' );
				$text   = __( 'Retry', 'wp-marketing-automations' );
				$result .= '<br/><a href="javascript:void(0)" class="bwfan_rerun_automations" title="' . $title . '" data-id="' . $item->ID . '">' . $text . '</a></span>';

				break;
			case 4:
				$result = '<span data-status="rescheduled">' . __( 'Re-Scheduled', 'wp-marketing-automations' ) . '</span>';
				break;
		}
	...
	public function column_actions( $item ) {
		$view_tasks_enable = in_array( absint( $item->status ), array( 1 ) );

		$delete_url = add_query_arg( [
			'page'                          => 'autonami',
			'tab'                           => 'carts',
			'ab_section'                    => 'recoverable',
			'bwfan_delete_recoverable_cart' => esc_attr( $item->ID ),
		], admin_url( 'admin.php' ) );
		...
		$title  = __( 'View tasks associated with the cart email', 'wp-marketing-automations' );
		$output .= $view_tasks_enable ? '<a href="' . $view_task_url . '" class="bwfan_view_abandoned_tasks" title="' . $title . '" data-id="' . $item->ID . '">' . __( 'View Tasks', 'wp-marketing-automations' ) . '</a> | ' : '';
