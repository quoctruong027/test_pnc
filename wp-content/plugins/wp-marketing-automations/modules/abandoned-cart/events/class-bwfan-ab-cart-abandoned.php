...
final class BWFAN_AB_Cart_Abandoned extends BWFAN_Event {
	...
	public function load_hooks() {
		add_action( 'bwfan_trigger_abandoned_event', [ $this, 'process' ], 10 );
	}
	...
	public function get_abandoned_automations( $active_abandoned_carts ) {
		foreach ( $active_abandoned_carts as $active_abandoned_cart ) {
			do_action( 'bwfan_trigger_abandoned_event', $active_abandoned_cart );
		}
	}
	...
	public function process( $abandoned_cart ) {
		...
		$data['ID']      = $abandoned_cart['ID'];
		$data['token']   = $abandoned_cart['token'];
		$data['user_id'] = $abandoned_cart['user_id'];
	...
	public function capture_async_data() {
		...
		$this->abandoned_id = BWFAN_Common::$events_async_data['ID'];
		...
		$this->token           = BWFAN_Common::$events_async_data['token'];
	...
	public function get_task_view( $global_data ) {
        ...
        <li>
			...
			<?php echo "<a href='" . site_url( 'wp-admin/admin.php?page=autonami&tab=carts&ab_section=recoverable&bwfan_cart_id=' . $global_data['cart_abandoned_id'] . '&bwfan_cart_email=' . $global_data['email'] . '' ) . "' target='_blank'>" . esc_html( $global_data['email'] ) . '</a>'; //phpcs:ignore WordPress.Security.EscapeOutput ?>
        </li>
		<?php
		return ob_get_clean();
	}
