...
"use strict";
	...
	AdminPage.prototype.eventsFilters = (function () {
		...
		jQuery(document).on('change wpf-dependent', '.wpfFiltersBlock input.toggle', function() {
			var filter   = jQuery(this).closest('.wpfFilter'),
				name     = jQuery(this).attr('name'),
				isActive = jQuery(this).is(':checked'),
				elem     = filter.find('[data-parent-switch~="'+name+'"]');
			if (elem.length) {
				if (isActive) {
	...
	AdminPage.prototype.fListChanged = (function(_this){
		...
		if (attrSlug == 'custom_meta_field_check') {
			...
			frontendType.find('option[value="slider"]').addClass('wpfHidden');
		} else {
			frontendType.find('option[value="colors"]').removeClass('wpfHidden');
			frontendType.find('option[value="slider"]').removeClass('wpfHidden');
		}
	});
