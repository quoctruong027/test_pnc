...
class WoofiltersViewWpf extends ViewWpf {
	...
	public function setFilterExistsItems( $order, $prodCatId = false, $querySettings = array() ) {
		if (is_null(self::$filterExistsTerms)) {
			$module                  = $this->getModule();
			$taxonomies              = $module->getFilterTaxonomies($order);
			$itemList                = $module->getFilterExistsItems(null, $taxonomies, null, $prodCatId, $order);
...
//		}
		...
		if ('yes_open' === $showTitle) {
			$iconClass = DispatcherWpf::applyFilters('getIconHtml', 'fa-minus', $filterSettings, 'title_icons');
			$icon      = '<i class="fa ' . $iconClass . ' wpfTitleToggle"></i>';
		} elseif ('yes_close' === $showTitle) {
			$iconClass = DispatcherWpf::applyFilters('getIconHtml', 'fa-plus', $filterSettings, 'title_icons');
			$icon      = '<i class="fa ' . $iconClass . ' wpfTitleToggle"></i>';
		} else {
			$icon = '';
		}
	...
	public function generateSortByFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		$type           = $this->getFilterSetting($settings, 'f_frontend_type', 'dropdown');
		...
		$htmlOpt        = $this->generateTaxonomyOptionsHtml($productSortBy, $filter, $sortBySelected);
		switch ($type) {
			case 'radio':
	...
	public function generateAuthorFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		if ( is_numeric($selectDefaultId) ) {
			$userSelectedSlugs[]    = $selectDefaultId;
		}
	...
	public function generateRatingFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		$htmlOpt = $this->generateTaxonomyOptionsHtml($rating, $filter, $ratingSelected, false, '', $layout);
	...
	public function generateAttributeFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		list($productAttr, $filterNameSlug, $filterName) = DispatcherWpf::applyFilters( 'getCustomMetaField', [$productAttr, $filterNameSlug, $filterName], $settings );
		...
		$noActive  = $defSelected ? '' : 'wpfNotActive';
		$noActive  = !$defSelected && $hidden_atts ? 'wpfHidden' : $noActive;
		$showCount = $filter['settings']['f_show_count'] ? ' wpfShowCount' : '';
	...
	public function wpfGetFilteredPriceFromProductList( $productList, $convert = true, $dec = 0 ) {
		global $wpdb;

		if (empty($dec)) {
			$sql = 'SELECT min( FLOOR( meta_value ) ) as wpfMinPrice, max( CEILING( meta_value ) ) as wpfMaxPrice ';
		} else {
			$sql = 'SELECT min( ROUND( meta_value, ' . $dec . ' ) ) as wpfMinPrice, max( ROUND( meta_value, ' . $dec . ' ) ) as wpfMaxPrice';
		}
	...
	private function generateTaxonomyOptionsHtml( $filterItemList, $filter = false, $selectedElem, $excludeIds = false, $pre = '', $layout = 0, $includeIds = false, $showedTerms = false, $countsTerms = false, $leer = true, $itemLevel = 0 ) {
		...
		$isSlug = ( 'product_tag' == $filterName || 'product_brand' == $filterName || ( strpos($filterName, 'filter_') === 0 && $this->getFilterSetting($filter, 'id', '') == 'wpfAttribute' ) );

		$iconCollapsible = '<span class="wpfCollapsible">' . DispatcherWpf::applyFilters('getIconHtml', '+', $settings, 'categories_icon') . '</span>';
		...
		foreach ($filterItemList as $filterItem) {
			...
			if ( 'dropdown' === $type || 'mul_dropdown' === $type ) {
				...
				if ($hasChildren) {
					...
					$html .= $this->generateTaxonomyOptionsHtml($filterItem->children, $filter, $selectedElem, $excludeIds, $tmpPre, false, $includeIds, $showedTerms, $countsTerms, $leer, $childrenLevel);
					if ( $isHierarchical && !$hideParent ) {
						$html .= '</ul>';
					} elseif ( $isHierarchical && $hideParent && 0 != $filterItem->parent ) {
						$html .= '</ul>';
