...
class WoofiltersControllerWpf extends ControllerWpf {
	...
	public function filtersFrontend() {
		...
		$taxonomies = $module->getFilterTaxonomies($generalSettings, !is_null($calcParentCategory), $filterSettings);
		if (!$recount) {
			$taxonomies['count'] = array();
		}
		$args = DispatcherWpf::applyFilters('beforeFilterExistsTerms', $args, $filterSettings);
		$filterItems = $module->getFilterExistsItems($args, $taxonomies, $calcParentCategory, false, $generalSettings, true);
