...
class XLWCTY_Coupon_Code extends xlwcty_component {
	...
	public $viewpath = '';
	public $coupun_data = array();
	public $campaign_data = array();
	public $show_type = 'immediate';
	public $coupon_data = array();
	public $fadeCouponText = '';
	...
	public function __construct( $order = false ) {
		...
		add_action( 'xlwcty_after_component_data_setup_xlwcty_coupon', array( $this, 'setup_style' ) );
		add_action( 'xlwcty_after_components_loaded', array( $this, 'setup_fields' ) );
		add_action( 'wp_ajax_xlwcty_generate_new_coupons', array( $this, 'get_locked_coupon' ) );
		add_action( 'wp_ajax_nopriv_xlwcty_generate_new_coupons', array( $this, 'get_locked_coupon' ) );
		add_filter( 'xlwcty_decode_coupon_merge_tags', array( $this, 'decode_coupon_expiry_tag' ), 10, 2 );
	}
	...
	public function setup_fields() {
		$this->fields = array(
			...
			'btn_txt'                   => $this->get_slug() . '_btn_text',
			'btn_link_immediate_after'  => $this->get_slug() . '_btn_link_immediate_click',
			...
			'display_coupon'            => $this->get_slug() . '_display',
			'desc_after'                => $this->get_slug() . '_desc_after_click',
			'btn_txt_after'             => $this->get_slug() . '_btn_text_after_click',
			'btn_link_after'            => $this->get_slug() . '_btn_link_after_click',
	...
	public function prepare_out_put_data() {
		...
		if ( $this->data->personalize == 'yes' ) {
			$this->data->fadeCouponText = $this->data->format;
		} else {
			if ( $this->data->selected_coupon > 0 ) {
				$this->data->fadeCouponText = get_the_title( $this->data->selected_coupon );
			}
		}
	...
	public function setup_style( $slug ) {
		if ( $this->is_enable() ) {
			if ( $this->data->heading_font_size != '' ) {
				$style['.xlwcty_wrap .xlwcty_coupon .xlwcty_title']['font-size']   = $this->data->heading_font_size . 'px';
				$style['.xlwcty_wrap .xlwcty_coupon .xlwcty_title']['line-height'] = ( $this->data->heading_font_size + 4 ) . 'px';
			}
			if ( $this->data->heading_color != '' ) {
				$style['.xlwcty_wrap .xlwcty_coupon .xlwcty_title']['color'] = $this->data->heading_color;
			}
			if ( $this->data->heading_alignment != '' ) {
				$style['.xlwcty_wrap .xlwcty_coupon .xlwcty_title']['text-align'] = $this->data->heading_alignment;
			}
			if ( $this->data->format_font != '' ) {
				$style['.xlwcty_wrap .xlwcty_coupon .xlwcty_coupon_code']['font-size'] = $this->data->format_font . 'px';
			}
			if ( $this->data->format_color != '' ) {
				$style['.xlwcty_wrap .xlwcty_coupon .xlwcty_coupon_code']['color']        = $this->data->format_color;
				$style['.xlwcty_wrap .xlwcty_coupon .xlwcty_coupon_code']['border-color'] = $this->data->format_color;
				//                $style['.xlwcty_wrap .xlwcty_coupon .xlwcty_coupon_area .xlwcty_coupon_inner']['border-color'] = $this->data->format_color;
			}
			if ( $this->data->btn_font_size != '' ) {
				$style['.xlwcty_wrap .xlwcty_coupon .xlwcty_btn']['font-size'] = $this->data->btn_font_size . 'px';
			}
			if ( $this->data->btn_color != '' ) {
				$style['.xlwcty_wrap .xlwcty_coupon .xlwcty_btn']['color'] = $this->data->btn_color;
			}
			if ( $this->data->btn_bg_color != '' ) {
				$style['.xlwcty_wrap .xlwcty_coupon .xlwcty_btn']['background'] = $this->data->btn_bg_color;
				$rgba                                                           = XLWCTY_Common::hex2rgb( $this->data->btn_bg_color, true );
				if ( $rgba != '' ) {
					$style['.xlwcty_wrap .xlwcty_coupon .xlwcty_btn:hover']['background'] = "rgba({$rgba},0.70)";
				}
			}
			if ( $this->data->border_style != '' ) {
				$style['.xlwcty_wrap .xlwcty_Box.xlwcty_coupon']['border-style'] = $this->data->border_style;
			}
			if ( (int) $this->data->border_width >= 0 ) {
				$style['.xlwcty_wrap .xlwcty_Box.xlwcty_coupon']['border-width'] = (int) $this->data->border_width . 'px';
			}
			if ( $this->data->border_color != '' ) {
				$style['.xlwcty_wrap .xlwcty_Box.xlwcty_coupon']['border-color'] = $this->data->border_color;
			}
			if ( $this->data->component_bg_color != '' ) {
				$style['.xlwcty_wrap .xlwcty_Box.xlwcty_coupon']['background-color'] = $this->data->component_bg_color;
			}
	...
	public function generate_new_coupons( $or_id = 0 ) {
		...
		if ( isset( $or_id ) && $or_id > 0 ) {
			try {
				$cp_data = $this->get_formated_coupon( true );

				return array(
					'coupon_code' => $cp_data['coupon_code'],
					'exp'         => $cp_data['expiry_date'],
					'type'        => $this->data->display_coupon,
				);
			} catch ( Exception $e ) {
				echo $e->getMessage();
			}
	...
	public function get_formated_coupon( $is_ajax = false ) {
		$coupon_code      = $this->save_coupon_data( $is_ajax );
			...
			if ( $this->data->personalize == 'yes' && $this->data->format != '' ) {
				...
				if ( $formated_coupon != '' && is_array( $coupon_meta ) && count( $coupon_meta ) > 0 ) {
					...
					if ( ! empty( $this->data->exp_days ) && $this->data->exp_days > 0 ) {
						...
						$exp_date                           = isset( $coupon_meta['expiry_date'] ) ? $coupon_meta['expiry_date'] : '';
						$coupon_meta['_xlwcty_expiry_date'] = $exp_date;
						$coupon_meta['expiry_date']         = $exp_date;
	...
	public function decode_coupon_expiry_tag( $content, $obj = false ) {
		if ( is_object( $obj ) ) {
