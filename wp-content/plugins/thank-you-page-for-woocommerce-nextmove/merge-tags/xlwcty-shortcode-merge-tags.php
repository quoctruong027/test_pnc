...
class XLWCTY_ShortCode_Merge_Tags {
	 ...
	 * @param bool $class_obj mostly $this class object
	 *
	 * @return string|string[]|void
	 */
	public static function maybe_parse_merge_tags( $content = '', $class_obj = false ) {
			...
			foreach ( $get_all as $tag ) {
				...
				if ( $matches && is_array( $matches ) && count( $matches ) > 0 ) {
					...
					foreach ( $matches[0] as $exact_match ) {
						...
						$exact_match = str_replace( '{{' . $tag, '[xlwcty_' . $tag, $exact_match );
	...
	public static function init() {
		...
		add_shortcode( 'xlwcty_coupon_name', array( __CLASS__, 'xlwcty_coupon_name' ) );
	...
	public static function process_today( $shortcode_attrs ) {
		...
		if ( $date_obj->getTimestamp() > $date_obj_cutoff->getTimestamp() ) {
			...
			if ( $is_excluded ) {
				...
				return __( 'tomorrow', 'thank-you-page-for-woocommerce-nextmove' );
			}
		} else {
			$is_excluded = false;
			/**
			...
			if ( $is_excluded ) {
				...
				return __( 'today', 'thank-you-page-for-woocommerce-nextmove' );
			}
		}
	}
	...
	public static function countdown_timer_admin( $shortcode_attrs ) {
		return __( '<div class="xlwcty_countdown_timer_admin" data-timer="3600"></div>' );
	}
	...
	public static function xlwcty_coupon_name( $shortcode_attrs ) {

		$atts     = shortcode_atts( array(
			'order_id' => '', //has to be user friendly , user will not understand 12:45 PM (g:i A) (https://codex.wordpress.org/Formatting_Date_and_Time)
			'label'    => '',
		), $shortcode_attrs );
		...
		$nextmove_dynamic_coupon_id = get_post_meta( $order_id, '_xlwcty_coupon', true );
		if ( empty( $nextmove_dynamic_coupon_id ) ) {
			return __return_empty_string();
		}

		return get_the_title( $nextmove_dynamic_coupon_id );
