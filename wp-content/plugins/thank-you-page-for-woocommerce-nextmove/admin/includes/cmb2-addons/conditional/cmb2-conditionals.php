 ...
 */
if ( ! class_exists( 'xlwcty_CMB2_Conditionals' ) ) {

	/**
	 * CMB2_Conditionals Plugin.
	 */
	class xlwcty_CMB2_Conditionals {
		...
		public function admin_footer() {
			...
			if ( XLWCTY_Common::is_load_admin_assets( 'all' ) ) {
				wp_enqueue_script( 'xlwcty-cmb2-conditionals', plugins_url( '/cmb2-conditionals.js', __FILE__ ), array( 'jquery', 'cmb2-scripts' ), self::VERSION, true );
			}
		}
...
//				);
	...
	if ( ( function_exists( 'wp_installing' ) && wp_installing() === false ) || ( ! function_exists( 'wp_installing' ) && ( ! defined( 'WP_INSTALLING' ) || WP_INSTALLING === false ) ) ) {
		add_action( 'plugins_loaded', 'xlwctycmb2_conditionals_init' );
	}

	if ( ! function_exists( 'xlwctycmb2_conditionals_init' ) ) {

		/**
		 * Initialize the class.
		 */
		function xlwctycmb2_conditionals_init() {
			$cmb2_conditionals = new xlwcty_CMB2_Conditionals();
		}
