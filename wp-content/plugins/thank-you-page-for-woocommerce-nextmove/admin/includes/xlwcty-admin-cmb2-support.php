...
class XLWCTY_Admin_CMB2_Support {
	...
	public static function cmb_before_row_cb( $field_args, $field ) {
		$default         = array(
			'xlwcty_accordion_title'     => __( 'Untitled', 'thank-you-page-for-woocommerce-nextmove' ),
	...
	public static function render_trigger_nav() {
		...
		$html_inside                 = array();
		$html_inside[]               = sprintf( '<li><a href="%s" class="%s">%s</a></li>', admin_url( 'admin.php?page=wc-settings&tab=' . XLWCTY_Common::get_wc_settings_tab_slug() ), self::active_class( 'all' ), __( 'All', 'thank-you-page-for-woocommerce-nextmove' ) );
	...
	public static function get_orders_cmb2( $field ) {
		$data = array();
		...
		if ( $value !== '' ) {
			$selected_order = wc_get_order( $value );
			...
			$data[ XLWCTY_Compatibility::get_order_id( $selected_order ) ] = $label;
		}

		if ( is_array( $orders ) && count( $orders ) > 0 ) {
			foreach ( $orders as $order ) {
				if ( XLWCTY_Compatibility::get_order_id( $order ) == $value ) {
					continue;
				}
