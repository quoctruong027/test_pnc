...
;(function ($, window, document) {
    ...
    window.equalheight = function (container) {
        ...
        jQuery(container).each(function () {
            var topPostion;
            $el = jQuery(this);
            jQuery($el).find('.xlwcty_pro_inner').css('minHeight', 'inherit');
            topPostion = $el.position().top;
            if (currentRowStart != topPostion) {
                for (var currentDiv = 0; currentDiv < rowDivs.length; currentDiv++) {
                    rowDivs[currentDiv].find('.xlwcty_pro_inner').css('minHeight', currentTallest);
                }
                rowDivs.length = 0;
            ...
            setTimeout(function () {
                for (var currentDiv1 = 0; currentDiv1 < rowDivs.length; currentDiv1++) {
                    rowDivs[currentDiv1].find('.xlwcty_pro_inner').css('minHeight', currentTallest);
                }
            }, 800);
    ...
    $(document).ready(function () {
        ...
        if (typeof xlwcty_fab_ecom !== 'undefined' && xlwcty_fab_ecom) {
            ...
            if (xlwcty_fab_ecom.google_analytics_id !== '' && xlwcty_fab_ecom.google_analytics_id !== undefined) {

                ga('create', xlwcty_fab_ecom.google_analytics_id, 'auto',{'name':'xlwcty'});
                ga('send', 'pageview');

                ga('xlwcty.require', 'ecommerce');
                ga('xlwcty.ecommerce:addTransaction', {
                    'id': xlwcty_fab_ecom.order_id,
                    'affiliation': xlwcty_fab_ecom.affiliation,
                    'revenue': xlwcty_fab_ecom.order_total,
                    'shipping': xlwcty_fab_ecom.shipping_total,
                    'tax': xlwcty_fab_ecom.order_tax
                });
                if (xlwcty_fab_ecom.gproducts.length > 0 && xlwcty_fab_ecom.gproducts !== 'undefined') {
                    var gproducts = $.parseJSON(xlwcty_fab_ecom.gproducts);
                    $.each(gproducts, function (index, gprodObject) {
                        ga('xlwcty.ecommerce:addItem', {
                            'id': gprodObject.id,
                            'name': gprodObject.name,
                            'sku': gprodObject.sku,
                            'category': gprodObject.category,
                            'price': gprodObject.price,
                            'quantity': gprodObject.quantity,
                        });
                    });

                }
                ga('xlwcty.ecommerce:send');
