...
if ( ! $xl_ultimate_latest_core_all || ( is_array( $xl_ultimate_latest_core_all ) && count( $xl_ultimate_latest_core_all ) <= 0 ) ) {
	$xl_ultimate_latest_core_all                                                     = array();
	$xl_ultimate_latest_core_all['plugins']                                          = array();
	$xl_ultimate_latest_core_all['plugins'][ plugin_basename( XLWCTY_PLUGIN_FILE ) ] = array(
		'version'     => $this_current_version,
		'plugin_path' => dirname( XLWCTY_PLUGIN_FILE ),
	);
} else {
	//if not registered yet
	if ( ! isset( $xl_ultimate_latest_core_all['plugins'][ plugin_basename( XLWCTY_PLUGIN_FILE ) ] ) ) {
		$xl_ultimate_latest_core_all['plugins'][ plugin_basename( XLWCTY_PLUGIN_FILE ) ] = array(
			'version'     => $this_current_version,
			'plugin_path' => dirname( XLWCTY_PLUGIN_FILE ),
		);
...
if ( ! isset( $xl_ultimate_latest_core_all['latest'] ) ) {
	$xl_ultimate_latest_core_all['latest'] = array(
		'basename'    => plugin_basename( XLWCTY_PLUGIN_FILE ),
		'version'     => $this_current_version,
		'plugin_path' => dirname( XLWCTY_PLUGIN_FILE ),
	);
