	...
	class Alg_WC_PIF_Main {
		...
		public function __construct( $scope ) {
			...
			if ( 'yes' === get_wc_pif_option( $this->scope . '_enabled', 'yes' ) ) {
				...
				if ( 'disable' !== $position ) {
                    ...
                    add_action( 'add_input_field', array( $this, 'add_product_input_fields_to_frontend_custom' ), get_wc_pif_option( 'frontend_position_priority', 10 ) );
				}
		...
		public function validate_product_input_fields_on_add_to_cart( $passed, $product_id ) {
			...
			for ( $i = 1; $i <= $total_number; $i++ ) {
				...
				$input=$_POST["form_data"];
				parse_str( $input, $out_put );
				$_POST = array_merge($_POST, $out_put);
				// Validate required.
