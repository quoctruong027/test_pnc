...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        window.GrapejsEditor.init({
            ...
            loadUrl: "<?php echo admin_url('admin-ajax.php?action=wecp_get_template&template_id=' . $template->id); ?>",
            storeUrl: "<?php echo admin_url('admin-ajax.php?action=wecp_auto_save_edited_template&template_id=' . $template->id); ?>",
            ...
            preview_data_uri: '<?php echo admin_url('admin-ajax.php?action=wecp_get_sample_data'); ?>',
            handleTestMail: async function ({email, template_id, test_order_id}) {
                let response = {"success": false, "message": "Unknown Issue"};
                jQuery.ajax({
                    type: "POST",
                    url: "<?php echo admin_url('admin-ajax.php?action=wecp_send_test_email'); ?>",
                    data: {
                        "email": email,
            ...
            handlePreview: async function ({template_id, test_order_id}) {
                ...
                jQuery.ajax({
                    type: "POST",
                    url: "<?php echo admin_url('admin-ajax.php?action=wecp_get_template_preview'); ?>",
                    data: {
                        "template_id": template_id,
                        "sample_order": test_order_id
                    },
