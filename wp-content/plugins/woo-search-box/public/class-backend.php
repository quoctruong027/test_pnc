...
class Guaven_woo_search_backend
{
    ...
    private function purengine_find_posts($search_query_local, $extra_where_query = ''){
        global $wpdb;
        $maxcount=get_option('guaven_woos_maxres') != '' ? (int) get_option('guaven_woos_maxres') : 10;
        $guaven_woo_search_admin = new Guaven_woo_search_admin();
        $search_query_local_tr_name = $guaven_woo_search_admin->translitter($search_query_local);
        $search_query_local_tr_name = $guaven_woo_search_admin->lowercase(urldecode(substr($search_query_local_tr_name, 0, 166)));

        $cache_results = $this->purengine_get_from_cache($search_query_local_tr_name);
        ...
        $sql = "SELECT displayed_html_data, product_id FROM {$wpdb->prefix}woos_search_data WHERE ";
        ...
        foreach($results as $i => $result)
            $product_ids .= ($i == 0 ? '':',') . $result->product_id;
        
        if(!empty($results)){
            $this->purengine_insert_to_results_cache($search_query_local_tr_name, $product_ids);
        }
    ...
    public function purengine_ajax_callback(){
        ...
        //&& isset($_POST['price'][1]) && floatval($_POST['price'][1])
        ){
            if($_POST['price_segment']==0){
                $xbeg=0;
                $xend=(floatval($_POST['price'])+0.0001);
            }
            elseif($_POST['price_segment']==1){
                $xbeg=floatval($_POST['price'])*0.8;
                $xend=floatval($_POST['price'])*1.2;
            }
            else{
                $xbeg=floatval($_POST['price']);
                $xend=999999999;
            }
