...
guaven_woos_data = {
  ...
  <?php
    ...
    echo 'guaven_woos_dttrr=0;';
}
if (is_singular('product') and intval(get_option('guaven_woos_data_trend_num')) > 0) {
    global $post;
    if (!empty($post->ID)){
    ?>
setTimeout(function(){guaven_woos_send_trend(<?php
    echo $post->ID; ?>,gws_get_unid());  },1000);
<?php
  }
}
...
if (get_option('guaven_woos_mobilesearch') != '') {
 ...
 <div class="guaven_woos_mobilesearch">
  <p><a autocomplete="off"  onclick='guaven_woos_mobclose()' href="javascript://" class="guaven_woos_mobclose"><img src="<?php
