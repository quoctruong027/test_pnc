...
class ListTable extends WP_List_Table {
	 ...
	 * @param WebhookTopics   $topic_lists WebhookTopics instance.
	...
	public function column_date_time( $task ) {
		...
		return esc_html( $date_time->date_i18n( sprintf( __( '%1$s %2$s', 'woocommerce-zapier' ), get_option( 'date_format' ), get_option( 'time_format' ) ) ) );
	}
	...
	public function column_resource( $task ) {
		...
		if ( false !== $resource ) {
			...
			if ( $resource->is_enabled() ) {
				...
				$resource_url         = $resource->get_url( $id );
			}
		}
...
.wp-list-table.task-history tfoot { display: none; }
	...
	protected function get_webhook_topic_name( $topic_key ) {
		$topics = $this->topic_lists->get_topics();
