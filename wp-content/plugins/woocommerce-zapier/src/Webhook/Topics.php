...
/**
 * Accesses and extends WooCommerce's standard webhook Topics/Events,
 * adding WooCommerce Zapier specific trigger events.
 *
 * @since 2.0.0
 */
class Topics {
	...
	public function woocommerce_webhook_topics( $topics ) {
		foreach ( $this->resource_manager->get_enabled() as $resource ) {
			foreach ( $resource->get_triggers() as $trigger ) {
