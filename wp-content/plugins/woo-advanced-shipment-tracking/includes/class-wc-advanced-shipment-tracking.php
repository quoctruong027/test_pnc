...
class WC_Advanced_Shipment_Tracking_Actions {
	...
	*/
	public function add_column_my_account_orders_ast_track_column( $order ) {
		...
		if( count($tracking_items) == 0 )return;
		if( count($tracking_items) > 1 && ( !$wc_ast_api_key || !$use_tracking_page) )return;
		echo '<a href="'.$tracking_items[0]['formatted_tracking_link'].'" class="woocommerce-button button" target="_blank">'.__( 'Track', 'woo-advanced-shipment-tracking' ).'</a>';
	}
		...
		$tracking_item['tracking_provider'] = $tracking_provider;
			...
			if( $wc_ast_api_key && $use_tracking_page && $trackship_supported == 1 && $status != 'carrier_unsupported'){		
				...
				if($tracking_page == 'other'){
					...
					$formatted['formatted_tracking_link'] = $trackship_other_page.'?order_id='.$order_id.'&order_key='.$order_key;
				} else{
...
//		}
	...
	public function insert_tracking_item( $order_id, $args ) {
		...
		if( empty($shippment_provider) ){
			$shippment_provider = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $woo_shippment_table_name WHERE api_provider_name=%s",$tracking_provider ) );
		}				
