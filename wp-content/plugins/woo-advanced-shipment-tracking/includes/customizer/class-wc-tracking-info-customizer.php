...
class wcast_initialise_customizer_settings {
	...
	*/
	...
	public static function is_own_customizer_request() {
		return isset( $_REQUEST['email'] ) && $_REQUEST['email'] === 'ast_tracking_display_panel';
	}
	...
	public function wcast_register_sample_default_controls( $wp_customize ) {		
		...
		*/
		...
		if ( is_plugin_active( 'ast-tracking-per-order-items/ast-tracking-per-order-items.php' ) ) {			
			...
			// Show track label
			$wp_customize->add_setting( 'tracking_info_settings[tracking_per_item_hide_tracking_order_line_items]',
				array(
					'default' => '',
					'transport' => 'refresh',
					'sanitize_callback' => '',
			...
			$wp_customize->add_control( 'tracking_info_settings[tracking_per_item_hide_tracking_order_line_items]',
				array(
					'label' => __( 'Hide tracking information from order line items', 'woo-advanced-shipment-tracking' ),
					'description' => esc_html__( '', 'woo-advanced-shipment-tracking' ),
					'section' => 'ast_tracking_per_item',
					'type' => 'checkbox'
				)
			);
		}
		...
		$wp_customize->add_control( new WP_Customize_Heading_Control( $wp_customize, 'tracking_info_settings[simple_content_header]',
			array(
				...
				'section' => 'ast_tracking_simple_section'
			)
		) );
		...
		$wp_customize->add_control( new WP_Customize_Heading_Control( $wp_customize, 'tracking_info_settings[table_content_header]',
			array(
				...
				'section' => 'ast_tracking_table_section'
			)
		) );
