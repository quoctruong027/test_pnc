...
class WC_Advanced_Shipment_Tracking_Front {
	...
	*/
	public function woo_track_order_function(){
		...
		if(!isset($order_id)){
			ob_start();		
			include 'views/front/track_order_form.php';		
			$form = ob_get_clean();	
			return $form;
		} else{
		ob_start();												
				...
				<div class="tracking-detail col">			
					...
					<div class="shipment-header">
						<?php if($total_trackings > 1 ){ ?>
							...
							<span><?php _e( 'Order', 'woocommerce' ); ?> <a href="<?php echo $order_url; ?>" target="_blank"><strong>#<?php echo apply_filters( 'ast_order_number_filter', $order_id); ?></strong></a></span>
						<?php } else{ ?>
