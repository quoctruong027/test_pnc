...
class WC_Advanced_Shipment_Tracking_Admin {
	...
	*/
	function get_settings_data(){
		...
		$ast_add_tracking_options = array( "wc_ast_default_mark_shipped" => __( 'Set the "mark as shipped" option checked  when adding tracking info to orders', 'woo-advanced-shipment-tracking' ));
		...
		$form_data = array(		
			'ast_completed_order_status' => array(
				...
				'title'		=> __( 'Completed Order Status', 'woo-advanced-shipment-tracking' ),
			...
			'wc_ast_unclude_tracking_info' => array(
				...
				'title'		=> __( 'On which order emails to include the shipment tracking info?', 'woo-advanced-shipment-tracking' ),
			...
			'wc_ast_show_orders_actions' => array(
				'type'		=> 'multiple_checkbox',
				'title'		=> __( 'For which Order status to display Add Tracking action button?', 'woo-advanced-shipment-tracking' ),
	...
	*/	
	function footer_function(){
		if ( !is_plugin_active( 'woocommerce-order-status-manager/woocommerce-order-status-manager.php' ) ) {
			...
			$ps_bg_color = get_option('wc_ast_status_partial_shipped_label_color','#c9d7e1');
			$ps_color = get_option('wc_ast_status_partial_shipped_label_font_color','#000');
			
			$ut_bg_color = get_option('wc_ast_status_updated_tracking_label_color','#c9d7e1');
	...
	*/
	function upload_tracking_csv_fun(){	
        ...
        $status_shipped = $_POST['status_shipped'];
		...
		if( $shippment_provider == 0 ){
			$sql = $wpdb->prepare( "SELECT COUNT(*) FROM $woo_shippment_table_name WHERE api_provider_name = %s", $tracking_provider );
			$shippment_provider = $wpdb->get_var( $sql );
		}				
		...
		if($tracking_provider && $tracking_number && $date_shipped){
			...
			if(!$tracking_provider){
				$sql = $wpdb->prepare( "SELECT ts_slug FROM $woo_shippment_table_name WHERE api_provider_name = %s", $_POST['tracking_provider'] );		
				$tracking_provider = $wpdb->get_var( $sql );					
			}
								...
								if($item['tracking_number'] == $_POST['tracking_number']){
									...
									if(isset($item['products_list']) && !empty($item['products_list'])){
										...
										foreach (array_keys($product_data_array + $product_list_array) as $product) {
											$mearge_array[$product] = (isset($product_data_array[$product]) ? $product_data_array[$product] : 0) + (isset($product_list_array) ? $product_list_array[$product] : 0);
										}																								
	...
	*/
	public function get_provider_html($default_shippment_providers,$status){
		...
		<div class="provider_list">
			...
			if($status == 'custom'){
				?>					
				<h3 class="add_custom_provider_h3"><?php _e( 'Add Custom Provider', 'woo-advanced-shipment-tracking' ); ?> <a href="javaScript:void(0);" class="add_custom_provider" id="add-custom"><span class="dashicons dashicons-plus-alt"></span></a></h3>	
			<?php } ?>
			...
			<?php } else{ 
				if($status == 'custom'){ ?>					
				<h3><?php echo sprintf(__("You did not create any %s shipping providers yet.", 'woo-advanced-shipment-tracking'), $status); ?></h3>
				<h3 class="add_custom_provider_h3"><?php _e( 'Add Custom Provider', 'woo-advanced-shipment-tracking' ); ?> <a href="javaScript:void(0);" class="add_custom_provider" id="add-custom"><span class="dashicons dashicons-plus-alt"></span></a></h3>	
			<?php } else{ ?>
	...
	*/
	public function update_custom_shipment_provider_fun(){
		...
		if($provider_type == 'default_provider'){
			$data_array = array(				
				'custom_provider_name' => sanitize_text_field($_POST['shipping_display_name']),
				'api_provider_name' => sanitize_text_field($_POST['api_provider_name']),				
				'custom_thumb_id' => sanitize_text_field($_POST['thumb_id']),				
			);				
		} else{
			$data_array = array(
