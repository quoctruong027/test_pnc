...
if ( ! function_exists( 'bwf_create_update_contact' ) ) {
	function bwf_create_update_contact( $order_id, $products, $total, $force = false ) {
		...
		if ( absint( $bwf_contact->get_id() ) > 0 && true !== apply_filters( 'bwf_is_contact_updatable', true, $bwf_contact ) ) {
			return $bwf_contact->get_id();
		}
 ...
 */
if ( ! function_exists( 'bwf_update_contact_changes_to_customer' ) ) {
