...
class WFOCU_Gateway_Integration_Braintree_CC extends WFOCU_Gateway {
	...
	protected function set_payment_method( $order, $package ) {
		...
		} elseif ( ! empty( $order->payment->token ) && empty( $order->payment->use_3ds_nonce ) ) {
	...
	public function maybe_render_in_offer_transaction_scripts() {
		...
		<script src="https://js.braintreegateway.com/web/3.48.0/js/three-d-secure.min.js?ver=2.2.6"></script> <?php //phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript ?>
		<script src="https://js.braintreegateway.com/web/3.48.0/js/client.min.js?ver=2.2.6"></script> <?php //phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript ?>
	...
	public function maybe_add_3ds_nonce_in_package( $package ) {
		$token = filter_input( INPUT_POST, '_wc_3ds_nonce', FILTER_SANITIZE_STRING );
		if ( ! is_null( $token ) ) {
