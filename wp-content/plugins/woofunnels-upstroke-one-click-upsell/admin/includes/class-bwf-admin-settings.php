...
if ( ! class_exists( 'BWF_Admin_Settings' ) ) {
	...
	class BWF_Admin_Settings {
		...
		public function maybe_register_admin_menu() {
			...
			if ( empty( $found ) ) {
				add_submenu_page( 'woofunnels', __( 'Settings', 'woofunnels' ), __( 'Settings', 'woofunnels' ), 'manage_woocommerce', 'woofunnels_settings', [ $this, '_callback' ] );
			}
		}
		...
		public function maybe_open_correct_settings() {
			if ( is_admin() && 'woofunnels_settings' === filter_input( INPUT_GET, 'page', FILTER_SANITIZE_STRING ) ) {
