...
Using WFOCUKirki theme developers can create rich experiences for the WordPress Customizer using best coding practices.
...
We advise you to familiarise yourself with the Customizer API before you start writing your theme using WFOCUKirki. An excellent handbook for the WordPress Customizer can be found on the [developer.wordpress.org](https://developer.wordpress.org/themes/customize-api/) website.

You can find detailed documentation on how to use WFOCUKirki on [https://aristath.github.io/kirki/](https://aristath.github.io/kirki/)
...
If you want to integrate WFOCUKirki in your theme or plugin, please read the instructions on [our documentation site](https://aristath.github.io/kirki/docs/integration).
...
* Fix: CSS output on numeric values. [issue link](https://wordpress.org/support/topic/all-css-output-from-wfocukirki-stop-working-on-version-3-0-26/).
...
* Fix: `WFOCUKirki_Helper::compare_values` vs. `field-dependencies.js` boolean issue (props @CaptJiggly) [#1825](https://github.com/aristath/kirki/issues/1825).
...
* Fix: `WFOCUKirki_Helper::compare_values` contains/in PHP warning if value doesn't exist in array [#1828](https://github.com/aristath/kirki/issues/1828).
...
* New: Added new `WFOCUKirki::remove_control()`, `WFOCUKirki::remove_section()` and `WFOCUKirki::remove_panel()` methods.
* New: Added 2 new filters: `wfocukirki/{$config_id}/webfonts/skip_hidden` and `wfocukirki/{$config_id}/css/skip_hidden` [#1678](https://github.com/aristath/kirki/issues/1678)
...
* Fix: Deprecated call to non-existing `WFOCUKirki_Styles_Frontend`, props @FrankM1 [#1644](https://github.com/aristath/kirki/issues/1644)
...
* Fix: PHP Warning in the `WFOCUKirki_Modules_Webfonts_Link` class [#1626](https://github.com/aristath/kirki/issues/1626)
...
* Fix: Add WFOCUKirki version number when enqueueing scripts & styles (cache-busting) [#1623](https://github.com/aristath/kirki/issues/1623)
...
It also changes the file structure and paves the way for a 3.1 rewrite which will be a significant improvement, making WFOCUKirki a mostly JS-based app fully integrated in WordPress's JS API and moving away from the PHP API.
...
* Fix: Overriding WFOCUKirki translations from a theme when WFOCUKirki is embedded.
* New: Replaced `select2` with `selectWoo`.
* New: Added a `WFOCUKirki_Control_Base` class and abstracted controls.
...
* Fix: `$subsets` not defined in the `WFOCUKirki_Modules_Webfonts_Link` class.
* Fix: Coding improvements in the `WFOCUKirki_Field` class.
...
* Fix: Undefined index notice in the `WFOCUKirki_Output` class.
...
* Fix: Removed the `WFOCUKirki_Custom_Build` class.
* Fix: Plugin does not exist error when WFOCUKirki is embedded in a theme [#1448](https://github.com/aristath/kirki/issues/1448)
...
* Fix: Incorrect logic when `WFOCUKirki::add_field()` only has 1 argument defined [#1429](https://github.com/aristath/kirki/issues/1429)
...
* Fix: `WFOCUKirki_Fonts_Google::$force_load_all_variants` was not working in version 3.0
...
* Fix: Added extra checks to avoid PHP Warning in the `WFOCUKirki_Fonts_Google` class [#1402](https://github.com/aristath/kirki/issues/1402).
...
* Fix: Improvements when embedding WFOCUKirki in a theme.
...
* Fix: Disabled the "loading" module by default. Use the `wfocukirki/modules` filter to enable.
...
* New: Added code to automatically handle translations when WFOCUKirki is embedded in a theme [#1381](https://github.com/aristath/kirki/issues/1381)
...
* New: Added 2 new methods for enqueueing google fonts. See the [`wfocukirki/googlefonts_load_method`](https://github.com/aristath/kirki/blob/9e3e4a6928339bdcd0f7520d305c145a80a06c8a/modules/webfonts/class-kirki-modules-webfonts.php#L100) filter.
...
* Fix: Disabled the custom wfocukirki-preview loader. This will have to be built more modular in future versions.
...
* Fix: `WFOCUKirki::get_variables()` method should be static ([#1050](https://github.com/aristath/kirki/pull/1050)).
...
* FIX: Better implementation when embedding WFOCUKirki in a theme ([#1025](https://github.com/aristath/kirki/issues/1025)).
...
* NEW: Added a new `wfocukirki/{$config_id}/styles` filter ([#908](https://github.com/aristath/kirki/issues/908)).
* NEW: Added a `customize-control-wfocukirki` class to all WFOCUKirki controls.
...
WFOCUKirki is now 100% WordPress Coding Standards compliant.
...
* FIX: Javascript Console Errors: "wp.customize" object undefined when WFOCUKirki fields were added in `customize_register`
...
* FIX: Issue with URLs when using WFOCUKirki embedded in a theme and not installed as a plugin.
...
* NEW: Started refactoring the `WFOCUKirki_Field` class. Now each field can have its own sub-class extending the main WFOCUKirki_Field object.
* NEW: `multicolor` control.
* NEW: Added `cropped_image` support in `repeater`. Props @guillaumemolter
* TWEAK: Renamed `WFOCUKirki_Customizer_Scripts_Loading` to `WFOCUKirki_Scripts_Loading`.
* TWEAK: Renamed `WFOCUKirki_Customizer_Scripts_Tooltips` to `WFOCUKirki_Scripts_Tooltips`.
* TWEAK: Renamed `WFOCUKirki_Customizer_Scripts_Icons` to `WFOCUKirki_Scripts_Icons`.
* TWEAK: More inline comments, docs & coding-standards improvements.
* DEPRECATED: Removed the `WFOCUKirki_Colourlovers` class.
...
* FIX: Styles were being applied to the customizer even if they were not defined in the `wfocukirki/config` filter.
...
* NEW: Added Material design palettes in the `WFOCUKirki_Helper` class.
...
* NEW: Allow disabling the custom loader using the `disable_loader` argument in the `wfocukirki/config` filter.
...
* FIX: Typo in the textdomain for some strings (some strings were using "WFOCUKirki" instead of "wfocukirki").
...
* NEW: Deprecated the `WFOCUKirki_Field_Sanitize` class in favor of a more simplified & robust implementation.
* NEW: Completely refactored the `WFOCUKirki_Field` class, we're migrating to a more OOP model.
* NEW: Added a new `wfocukirki-generic` control.
* NEW: Deprecated the custom text control and used the new `wfocukirki-generic` control instead.
* NEW: Deprecated the custom textarea control and used the new `wfocukirki-generic` control instead.
...
* NEW: Intruducing WFOCU_KIRKI_NO_OUTPUT constant that disables CSS output completely when set to true.

= 2.0.8 =

February 10, 2016, dev time: 2 hours

* FIX: Only load WFOCUKirki styles when in the customizer
* FIX: Performance issue with Google Fonts
* NEW: Added radio-image controls to repeaters
* TWEAK: Better color handling in the WFOCUKirki_Color class
...
* FIX: Minor bugfixes to the WFOCUKirki_Color class.
* FIX: wfocukirki_get_option now uses WFOCUKirki::get_option().
...
* NEW: Introducing the WFOCUKirki::get_option() method to get values.
...
* FIX: `WFOCUKirki::get_option()` method now works for all fields, including background fields.
...
* TWEAK: Improve the WFOCUKirki_Styles_Customizer class
* TWEAK: Code cleanups
...
* TWEAK: Better color manipulation in the WFOCUKirki_Color class
* TWEAK: Move secondary classes instantiation to the WFOCUKirki() function.
* TWEAK: set a $wfocukirki global
* TWEAK: deprecate getOrThrow method in the WFOCUKirki_Config class.
* TWEAK: Move sanitisation functions to a WFOCUKirki_Sanitize class.
* TWEAK: Rename WFOCUKirki_Framework to WFOCUKirki_Toolkit.
* TWEAK: Move variables to the new API
* TWEAK: simplify WFOCUKirki_Controls class
* TWEAK: move the wfocukirki/fields & wfocukirki/controls filters to the new API
...
* FIX: Issues with the URL when WFOCUKirki is embedded in a theme
...
* NEW: Added a WFOCUKirki_Colourlovers class to use palettes from the colourlovers API
...
* NEW: added a functional wfocukirki_get_option() function
* TWEAK: Simplified configuration options.
* NEW: Turn WFOCUKirki into a singleton and a facade (props @vpratfr)
...
* REMOVED: Remove the `wfocukirki_get_option` function that was introduced in 0.7 as it's not working properly yet.
...
* NEW: Added `description` argument in the wfocukirki configuration (replaces the theme description)
...
* NEW: Added `wfocukirki_get_option( $setting );` function that also gets default values
...
* NEW: Introducing WFOCUKirki_Customize_Control class
* FIX: CSS tweaks
...
* NEW: Change the WFOCUKirki theme based on which admin theme is selected.
