...
class WFOCUKirki_Util {
	...
	public static function is_plugin() {
		...
		foreach ( $plugins as $plugin => $args ) {
			if ( ! $is_plugin && isset( $args['Name'] ) && ( 'WFOCUKirki' === $args['Name'] || 'WFOCUKirki Toolkit' === $args['Name'] ) ) {
				$is_plugin = true;
				$_plugin   = $plugin;
			}
		}

		// No need to proceed any further if WFOCUKirki wasn't found in the list of plugins.
		if ( ! $is_plugin ) {
			return false;
		}
			...
			if ( isset( $field['variables'] ) && $field['variables'] && ! empty( $field['variables'] ) ) {
				...
				foreach ( $field['variables'] as $field_variable ) {
					...
					if ( isset( $field_variable['name'] ) ) {
						...
						$variables[ $variable_name ] = WFOCUKirki_Values::get_value( $field['settings'] );
						if ( $variable_callback ) {
							$variables[ $variable_name ] = call_user_func( $field_variable['callback'], WFOCUKirki_Values::get_value( $field['settings'] ) );
						}
					}
				}
			}
	...
	public function http_request( $request = array(), $url = '' ) {
		...
		foreach ( $plugins['plugins'] as $plugin ) {
			if ( isset( $plugin['Name'] ) && 'WFOCUKirki Toolkit' === $plugin['Name'] ) {
				$exists = true;
			}
		}
		// Inject data.
		if ( ! $exists && defined( 'WFOCU_KIRKI_PLUGIN_FILE' ) ) {
			$plugins['plugins']['wfocukirki/kirki.php'] = get_plugin_data( WFOCU_KIRKI_PLUGIN_FILE );
		}
