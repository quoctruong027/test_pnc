...
class WFOCUKirki_Field {
	...
	public function __construct( $config_id = 'global', $args = array() ) {
		...
		if ( isset( $args['setting'] ) && ! empty( $args['setting'] ) && ( ! isset( $args['settings'] ) || empty( $args['settings'] ) ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_attr__( 'Typo found in field %s - setting instead of settings.', 'wfocukirki' ), esc_attr( $args['settings'] ) ), '3.0.10' );
		...
		if ( is_array( $config_id ) && empty( $args ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_attr__( 'Config not defined for field %1$s - See %2$s for details on how to properly add fields.', 'wfocukirki' ), esc_attr( $args['settings'] ), 'https://aristath.github.io/kirki/docs/getting-started/fields.html' ), '3.0.10' );
			$args      = $config_id;
			$config_id = 'global';
		}

		$args['wfocukirki_config'] = $config_id;

		$this->wfocukirki_config = trim( esc_attr( $config_id ) );
		if ( '' === $config_id ) {
			/* translators: %1$s represents the field ID where the error occurs. %2$s is the URL in the documentation site. */
			_doing_it_wrong( __METHOD__, sprintf( esc_attr__( 'Config not defined for field %1$s - See %2$s for details on how to properly add fields.', 'wfocukirki' ), esc_attr( $args['settings'] ), 'https://aristath.github.io/kirki/docs/getting-started/fields.html' ), '3.0.10' );
			$this->wfocukirki_config = 'global';
		}
		...
		$config_defaults = ( isset( WFOCUKirki::$config['global'] ) ) ? WFOCUKirki::$config['global'] : array();
		if ( 'global' !== $this->wfocukirki_config && isset( WFOCUKirki::$config[ $this->wfocukirki_config ] ) ) {
			$config_defaults = WFOCUKirki::$config[ $this->wfocukirki_config ];
		}
	...
	protected function set_field() {
		...
		WFOCUKirki::$fields[ $this->settings ] = $args;
	...
	protected function set_wfocukirki_config() {
		...
		$this->wfocukirki_config = esc_attr( $this->wfocukirki_config );
	}
	...
	protected function set_option_type() {
		...
		if ( 'theme_mods' === $this->option_type ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_attr__( 'Typo found in field %s - "theme_mods" vs "theme_mod"', 'wfocukirki' ), esc_attr( $this->settings ) ), '3.0.10' );
	...
	protected function set_partial_refresh() {
		...
		foreach ( $this->partial_refresh as $id => $args ) {
			if ( ! is_array( $args ) || ! isset( $args['selector'] ) || ! isset( $args['render_callback'] ) || ! is_callable( $args['render_callback'] ) ) {
				/* translators: %1$s represents the field ID where the error occurs. */
				_doing_it_wrong( __METHOD__, sprintf( esc_attr__( '"partial_refresh" invalid entry in field %s', 'wfocukirki' ), esc_attr( $this->settings ) ), '3.0.10' );
	...
	protected function set_settings() {
		...
		if ( ! is_array( $this->settings ) ) {
			$this->settings = array(
				'wfocukirki_placeholder_setting' => $this->settings,
			);
		}
		$settings = array();
		...
		if ( isset( $this->settings['wfocukirki_placeholder_setting'] ) ) {
			$this->settings = $this->settings['wfocukirki_placeholder_setting'];
		}
	}
	...
	protected function set_output() {
		...
		if ( ! is_array( $this->output ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_attr__( '"output" invalid format in field %s. The "output" argument should be defined as an array of arrays.', 'wfocukirki' ), esc_attr( $this->settings ) ), '3.0.10' );
			$this->output = array(
				array(
					'element' => $this->output,
				),
