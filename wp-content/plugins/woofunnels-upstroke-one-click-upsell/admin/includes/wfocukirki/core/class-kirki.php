...
class WFOCUKirki extends WFOCUKirki_Init {
	...
	public static function get_option( $config_id = '', $field_id = '' ) {
		...
		return WFOCUKirki_Values::get_value( $config_id, $field_id );
	}
	...
	public static function add_config( $config_id, $args = array() ) {
		...
		$config                             = WFOCUKirki_Config::get_instance( $config_id, $args );
	...
	public static function add_panel( $id = '', $args = array() ) {
		...
		$args['type']        = 'wfocukirki-' . $args['type'];
	...
	public static function add_field( $config_id, $args ) {
		...
		if ( doing_action( 'customize_register' ) ) {
			_doing_it_wrong( __METHOD__, esc_attr__( 'WFOCUKirki fields should not be added on customize_register. Please add them directly, or on init.', 'wfocukirki' ), '3.0.10' );
		}
		...
		if ( class_exists( 'WFOCUKirki_Modules_CSS' ) && ( 'fontawesome' === $args['type'] || 'wfocukirki-fontawesome' === $args['type'] ) ) {
			WFOCUKirki_Modules_CSS::add_fontawesome_script();
		}

		$str       = str_replace( array( '-', '_' ), ' ', $args['type'] );
		$classname = 'WFOCUKirki_Field_' . str_replace( ' ', '_', ucwords( $str ) );
		if ( class_exists( $classname ) ) {
			new $classname( $config_id, $args );
			return;
		}
		if ( false !== strpos( $classname, 'WFOCUKirki_Field_WFOCUKirki_' ) ) {
			$classname = str_replace( 'WFOCUKirki_Field_WFOCUKirki_', 'WFOCUKirki_Field_', $classname );
			if ( class_exists( $classname ) ) {
				new $classname( $config_id, $args );
				return;
			}
