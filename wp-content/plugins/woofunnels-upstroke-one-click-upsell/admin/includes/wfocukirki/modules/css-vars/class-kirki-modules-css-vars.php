...
class WFOCUKirki_Modules_CSS_Vars {
	...
	public function the_style() {
		...
		echo '<style id="wfocukirki-css-vars">';
		echo ':root{';
		foreach ( $fields as $id => $args ) {
			if ( ! isset( $args['css_vars'] ) || empty( $args['css_vars'] ) ) {
				continue;
			}
			$val = WFOCUKirki_Values::get_value( $args['wfocukirki_config'], $id );
	...
	public function postmessage() {
		wp_enqueue_script( 'wfocukirki_auto_css_vars', trailingslashit( WFOCUKirki::$url ) . 'modules/css-vars/script.js', array( 'jquery', 'customize-preview' ), WFOCU_KIRKI_VERSION, true );
		$fields = WFOCUKirki::$fields;
		$data   = array();
		...
		wp_localize_script( 'wfocukirki_auto_css_vars', 'wfocukirkiCssVarFields', $data );
	}
}
