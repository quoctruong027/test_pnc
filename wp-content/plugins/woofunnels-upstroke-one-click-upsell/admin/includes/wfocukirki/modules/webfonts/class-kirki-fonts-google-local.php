...
final class WFOCUKirki_Fonts_Google_Local {
	...
	private function download_font_file( $url ) {
		...
		if ( ! file_exists( $this->folder_path ) ) {
			WFOCUKirki_Helper::init_filesystem()->mkdir( $this->folder_path, FS_CHMOD_DIR );
		}
		...
		// Write file.
		return WFOCUKirki_Helper::init_filesystem()->put_contents( $path, $contents, FS_CHMOD_FILE );
	}
	...
	public function get_root_path() {
		...
		// If the folder doesn't exist, create it.
		if ( ! file_exists( $path ) ) {
			WFOCUKirki_Helper::init_filesystem()->mkdir( $path, FS_CHMOD_DIR );
		}

		// Return the path.
		return apply_filters( 'wfocukirki_googlefonts_root_path', $path );
	}
	...
	public function get_root_url() {
		...
		return apply_filters( 'wfocukirki_googlefonts_root_url', untrailingslashit( esc_url_raw( $url ) ) . '/webfonts' );
	}
