...
class WFOCUKirki_Modules_Webfonts {
	 ...
	 */
	public function run() {
		$this->fonts_google = WFOCUKirki_Fonts_Google::get_instance();
		$this->init();
	}
	...
	protected function init() {
		...
		foreach ( array_keys( WFOCUKirki::$config ) as $config_id ) {
			$method    = $this->get_method( $config_id );
			$classname = 'WFOCUKirki_Modules_Webfonts_' . ucfirst( $method );
	...
	public function get_method() {
		...
		$method = apply_filters( 'wfocukirki_googlefonts_load_method', 'async' );
	...
	public function loop_fields( $config_id ) {
		foreach ( WFOCUKirki::$fields as $field ) {
			if ( isset( $field['wfocukirki_config'] ) && $config_id !== $field['wfocukirki_config'] ) {
				continue;
			}
			if ( true === apply_filters( "wfocukirki_{$config_id}_webfonts_skip_hidden", true ) ) {
