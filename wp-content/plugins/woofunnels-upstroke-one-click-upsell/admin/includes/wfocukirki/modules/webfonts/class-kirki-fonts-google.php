...
final class WFOCUKirki_Fonts_Google {
	...
	/**
	 * The WFOCUKirki_Fonts_Google instance.
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_wfocukirki_fonts_google_all_get', array( $this, 'get_googlefonts_json' ) );
		add_action( 'wp_ajax_nopriv_wfocukirki_fonts_google_all_get', array( $this, 'get_googlefonts_json' ) );
		add_action( 'wp_ajax_wfocukirki_fonts_standard_all_get', array( $this, 'get_standardfonts_json' ) );
		add_action( 'wp_ajax_nopriv_wfocukirki_fonts_standard_all_get', array( $this, 'get_standardfonts_json' ) );

		// Populate the array of google fonts.
		$this->google_fonts = WFOCUKirki_Fonts::get_google_fonts();
	}
	...
	public static function get_instance() {
		if ( null === self::$instance ) {
			self::$instance = new WFOCUKirki_Fonts_Google();
		}
	...
	public function generate_google_font( $args ) {
		...
		if ( isset( $args['type'] ) && 'wfocukirki-typography' === $args['type'] ) {

			// Get the value.
			$value = WFOCUKirki_Values::get_sanitized_field_value( $args );
			...
			if ( ! WFOCUKirki_Fonts::is_google_font( $value['font-family'] ) ) {
				return;
			}
	...
	public function get_standardfonts_json() {
		echo wp_json_encode( WFOCUKirki_Fonts::get_standard_fonts() ); // WPCS: XSS ok.
		wp_die();
	}
