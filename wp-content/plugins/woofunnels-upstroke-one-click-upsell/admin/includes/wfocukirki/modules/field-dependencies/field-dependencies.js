var wfocukirkiDependencies = {
	...
	init: function() {
		...
		wp.customize.control.each( function( control ) {
			self.showWFOCUKirkiControl( control );
		} );
				...
				wp.customize( master, function( setting ) {
					var setupControl = function( control ) {
						...
						isDisplayed = function() {
							return self.showWFOCUKirkiControl( wp.customize.control( slave ) );
						};
	 ...
	 * @returns {bool}
	 */
	showWFOCUKirkiControl: function( control ) {
		var self     = this,
			show     = true,
			isOption = (
				control.params && // Check if control.params exists.
				control.params.wfocukirkiOptionType &&  // Check if option_type exists.
				'option' === control.params.wfocukirkiOptionType &&  // We're using options.
				control.params.wfocukirkiOptionName && // Check if option_name exists.
				! _.isEmpty( control.params.wfocukirkiOptionName ) // Check if option_name is not empty.
			),
			i;
	 ...
	 * Check a condition.
	 *
	 * @param {Object} requirement - The requirement, inherited from showWFOCUKirkiControl.
	...
	checkCondition: function( requirement, control, isOption, relation ) {
		...
		if ( isOption && requirement.setting ) {
			...
			if ( -1 === requirement.setting.indexOf( control.params.wfocukirkiOptionName + '[' ) ) {
				requirement.setting = control.params.wfocukirkiOptionName + '[' + requirement.setting + ']';
			}
		}
