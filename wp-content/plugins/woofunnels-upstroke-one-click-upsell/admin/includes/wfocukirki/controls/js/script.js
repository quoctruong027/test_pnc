/* jshint -W079 */
/* jshint unused:false */
if ( _.isUndefined( window.wfocukirkiSetSettingValue ) ) {
	var wfocukirkiSetSettingValue = { // eslint-disable-line vars-on-top
		...
		set: function( setting, value ) {
			...
			switch ( subControl.type ) {
				...
				case 'wfocukirki-background':
					if ( ! _.isUndefined( value['background-color'] ) ) {
						$this.setColorPicker( $this.findElement( setting, '.wfocukirki-color-control' ), value['background-color'] );
					}
				...
				case 'wfocukirki-color':
					$this.setColorPicker( $this.findElement( setting, '.wfocukirki-color-control' ), value );
					break;

				case 'wfocukirki-multicheck':
				...
				case 'wfocukirki-multicolor':
					_.each( value, function( subVal, index ) {
						$this.setColorPicker( $this.findElement( setting, '.multicolor-index-' + index ), subVal );
					} );
					break;

				case 'wfocukirki-radio-buttonset':
				case 'wfocukirki-radio-image':
				case 'wfocukirki-radio':
				case 'wfocukirki-dashicons':
				case 'wfocukirki-color-palette':
				case 'wfocukirki-palette':
					jQuery( $this.findElement( setting, 'input[value="' + value + '"]' ) ).prop( 'checked', true );
					break;

				case 'wfocukirki-typography':
...
var wfocukirki = {
	...
	initialize: function() {
		...
		/*setTimeout( function() {
			wfocukirki.util.webfonts.standard.initialize();
			wfocukirki.util.webfonts.google.initialize();
		}, 150 );*/
...
// Initialize the wfocukirki object.
wfocukirki.initialize();
var wfocukirki = wfocukirki || {};
wfocukirki = jQuery.extend( wfocukirki, {
	...
	control: {
		...
		'wfocukirki-radio': {
			...
			init: function( control ) {
				...
				// Init the control.
				wfocukirki.input.radio.init( control );
			...
			template: function( control ) {
				var template = wp.template( 'wfocukirki-input-radio' );
				control.container.html( template( {
					...
					value: wfocukirki.setting.get( control.id ),
		...
		'wfocukirki-color': {
			...
			init: function( control ) {
				...
				// Init the control.
				wfocukirki.input.color.init( control );
			 ...
			 */
			template: function( control ) {
				var template = wp.template( 'wfocukirki-input-color' );
				control.container.html( template( {
					label: control.params.label,
					description: control.params.description,
					'data-id': control.id,
		...
		'wfocukirki-generic': {
			...
			init: function( control ) {
				...
				if ( ! _.isUndefined( control.params ) && ! _.isUndefined( control.params.choices ) && ! _.isUndefined( control.params.choices.element ) && 'textarea' === control.params.choices.element ) {
					wfocukirki.input.textarea.init( control );
					return;
				}
				wfocukirki.input.genericInput.init( control );
			},
			...
			template: function( control ) {
				...
				if ( ! _.isUndefined( control.params ) && ! _.isUndefined( control.params.choices ) && ! _.isUndefined( control.params.choices.element ) && 'textarea' === control.params.choices.element ) {
					template = wp.template( 'wfocukirki-input-textarea' );
					control.container.html( template( args ) );
					return;
				}
				template = wp.template( 'wfocukirki-input-generic' );
				control.container.html( template( args ) );
			}
		},
		...
		'wfocukirki-number': {
			...
			init: function( control ) {
				...
				// Init the control.
				wfocukirki.input.number.init( control );
			},
			 ...
			 */
			template: function( control ) {
				var template = wp.template( 'wfocukirki-input-number' );
		...
		'wfocukirki-select': {
			...
			init: function( control ) {
				...
				// Init the control.
				wfocukirki.input.select.init( control );
			},
			 ...
			 */
			template: function( control ) {
				var template = wp.template( 'wfocukirki-input-select' );
...
wfocukirki = jQuery.extend( wfocukirki, {
	...
	input: {
		...
		radio: {
			...
			init: function( control ) {
				...
				input.on( 'change keyup paste click', function() {
					wfocukirki.setting.set( control.id, jQuery( this ).val() );
				} );
			}
		},
		...
		color: {
			 ...
			 */
			init: function( control ) {
				var picker = jQuery( '.wfocukirki-color-control[data-id="' + control.id + '"]' ),
					clear;
				...
				setTimeout( function() {
					clear = jQuery( '.wfocukirki-input-container[data-id="' + control.id + '"] .wp-picker-clear' );
					if ( clear.length ) {
						clear.click( function() {
							wfocukirki.setting.set( control.id, '' );
						} );
					}
				}, 200 );
				...
				picker.wpColorPicker( {
					change: function() {
						...
						setTimeout( function() {
							wfocukirki.setting.set( control.id, picker.val() );
						}, 20 );
					}
				} );
			}
		...
		select: {
			...
			init: function( control ) {
				...
				jQuery( element ).selectWoo( selectWooOptions ).on( 'change', function() {
					...
					wfocukirki.setting.set( control.id, selectValue );
				} );
			}
		},
		...
		number: {
			...
			init: function( control ) {
				...
				up   = jQuery( '.wfocukirki-input-container[data-id="' + control.id + '"] .plus' );
				down = jQuery( '.wfocukirki-input-container[data-id="' + control.id + '"] .minus' );
		...
		image: {
			...
			getTemplate: function( data ) {
				...
				if ( data.value.url || '' !== url ) {
					...
					html += '<div class="placeholder">' + wfocukirkiL10n.noFileSelected + '</div>';
				}
				html += '<div class="actions">';
				html += '<button class="button image-upload-remove-button' + ( '' === url ? ' hidden' : '' ) + '">' + wfocukirkiL10n.remove + '</button>';
				if ( data.default && '' !== data.default ) {
					...
					html += '>' + wfocukirkiL10n.default + '</button>';
				}
				html += '<button type="button" class="button image-upload-button">' + wfocukirkiL10n.selectFile + '</button>';
				html += '</div></div>';

				return '<div class="wfocukirki-input-container" data-id="' + data.id + '">' + html + '</div>';
			},
		...
		webfonts: {
			...
			google: {
				...
				setFonts: function() {
					...
					jQuery.post( ajaxurl, { 'action': 'wfocukirki_fonts_google_all_get' }, function( response ) {
			...
			standard: {
				...
				setFonts: function() {
					...
					jQuery.post( ajaxurl, { 'action': 'wfocukirki_fonts_standard_all_get' }, function( response ) {
...
( function() {
	 ...
	 */
	wp.customize.wfocukirkiDynamicControl = wp.customize.Control.extend( {
		...
		initialize: function( id, options ) {
			...
			if ( ! args.params.type ) {
				args.params.type = 'wfocukirki-generic';
			}
		...
		ready: function() {
			...
			control.deferred.embedded.done( function() {
				control.initWFOCUKirkiControl( control );
			} );
		},
		...
		embed: function() {
			...
			wp.customize.section( sectionId, function( section ) {
				if ( 'wfocukirki-expanded' === section.params.type || section.expanded() || wp.customize.settings.autofocus.control === control.id ) {
		 ...
		 * @returns {null}
		 */
		initWFOCUKirkiControl: function( control ) {
			if ( 'undefined' !== typeof wfocukirki.control[ control.params.type ] ) {
				wfocukirki.control[ control.params.type ].init( control );
				return;
			}
...
_.each( wfocukirki.control, function( obj, type ) {
	wp.customize.controlConstructor[ type ] = wp.customize.wfocukirkiDynamicControl.extend( {} );
} );
/* global wfocukirkiControlLoader */
wp.customize.controlConstructor['wfocukirki-background'] = wp.customize.Control.extend( {
	...
	ready: function() {
		...
		// Init the control.
		if ( ! _.isUndefined( window.wfocukirkiControlLoader ) && _.isFunction( wfocukirkiControlLoader ) ) {
			wfocukirkiControlLoader( control );
		} else {
			control.initWFOCUKirkiControl();
		}
	},

	initWFOCUKirkiControl: function() {

		var control = this,
			value   = control.setting._value,
			picker  = control.container.find( '.wfocukirki-color-control' );
...
wp.customize.controlConstructor['wfocukirki-color-palette'] = wp.customize.wfocukirkiDynamicControl.extend( {} );
wp.customize.controlConstructor['wfocukirki-dashicons'] = wp.customize.wfocukirkiDynamicControl.extend( {} );
wp.customize.controlConstructor['wfocukirki-date'] = wp.customize.wfocukirkiDynamicControl.extend( {
	...
	initWFOCUKirkiControl: function() {
		...
		control.container.find( '.wfocukirki-controls-loading-spinner' ).hide();
...
/* global dimensionwfocukirkiL10n */
wp.customize.controlConstructor['wfocukirki-dimension'] = wp.customize.wfocukirkiDynamicControl.extend( {
	...
	initWFOCUKirkiControl: function() {
		...
		// Notifications.
		control.wfocukirkiNotifications();
	 ...
	 */
	wfocukirkiNotifications: function() {
		...
		wp.customize( control.id, function( setting ) {
			setting.bind( function( value ) {
				...
				if ( false === wfocukirki.util.validate.cssValue( value ) ) {
					setting.notifications.add( code, new wp.customize.Notification(
						code,
						{
							type: 'warning',
							message: dimensionwfocukirkiL10n['invalid-value']
						}
					) );
				} else {
...
/* global dimensionswfocukirkiL10n */
wp.customize.controlConstructor['wfocukirki-dimensions'] = wp.customize.wfocukirkiDynamicControl.extend( {
		...
		wp.customize( control.id, function( setting ) {
			setting.bind( function( value ) {
				...
				_.each( value, function( val, direction ) {
					if ( false === wfocukirki.util.validate.cssValue( val ) ) {
						subs[ direction ] = val;
					} else {
				...
				if ( ! _.isEmpty( subs ) ) {
					message = dimensionswfocukirkiL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
...
/* global tinyMCE */
wp.customize.controlConstructor['wfocukirki-editor'] = wp.customize.wfocukirkiDynamicControl.extend( {

	initWFOCUKirkiControl: function() {

		var control = this,
			element = control.container.find( 'textarea' ),
			id      = 'wfocukirki-editor-' + control.id.replace( '[', '' ).replace( ']', '' ),
			editor;
...
wp.customize.controlConstructor['wfocukirki-fontawesome'] = wp.customize.wfocukirkiDynamicControl.extend( {
...
/* global wfocukirkiControlLoader */
wp.customize.controlConstructor['wfocukirki-image'] = wp.customize.Control.extend( {
...
wp.customize.controlConstructor['wfocukirki-multicheck'] = wp.customize.wfocukirkiDynamicControl.extend( {
...
/* global wfocukirkiControlLoader */
wp.customize.controlConstructor['wfocukirki-multicolor'] = wp.customize.Control.extend( {
	...
	initWFOCUKirkiControl: function() {
		...
		function wfocukirkiMulticolorChangeHandler( control, value, subSetting ) {
		...
		while ( i < Object.keys( colors ).length ) {
			wfocukirkiMulticolorChangeHandler( this, value, keys[ i ] );
			i++;
		}
	},
...
wp.customize.controlConstructor['wfocukirki-palette'] = wp.customize.wfocukirkiDynamicControl.extend( {} );
wp.customize.controlConstructor['wfocukirki-radio-buttonset'] = wp.customize.wfocukirkiDynamicControl.extend( {} );
wp.customize.controlConstructor['wfocukirki-radio-image'] = wp.customize.wfocukirkiDynamicControl.extend( {} );
/* global wfocukirkiControlLoader */
...
wp.customize.controlConstructor.repeater = wp.customize.Control.extend( {
	...
	initWFOCUKirkiControl: function() {
		...
		// The current value set in Control Class (set in WFOCUKirki_Customize_Repeater_Control::to_json() function)
	...
	setImageInRepeaterField: function( attachment ) {
		...
		$targetDiv.find( '.wfocukirki-image-attachment' ).html( '<img src="' + attachment.url + '">' ).hide().slideDown( 'slow' );
	...
	setFileInRepeaterField: function( attachment ) {
		...
		$targetDiv.find( '.wfocukirki-file-attachment' ).html( '<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>' ).hide().slideDown( 'slow' );
	...
	removeImage: function( event ) {
		...
		$targetDiv.find( '.wfocukirki-image-attachment' ).slideUp( 'fast', function() {
	...
	removeFile: function( event ) {
		...
		$targetDiv.find( '.wfocukirki-file-attachment' ).slideUp( 'fast', function() {
			jQuery( this ).show().html( jQuery( this ).data( 'placeholder' ) );
		} );
		$targetDiv.find( '.hidden-field' ).val( '' );
		$uploadButton.text( $uploadButton.data( 'label' ) );
	...
	addRow: function( data ) {
		...
		var control       = this,
			template      = control.repeaterTemplate(), // The template for the new row (defined on WFOCUKirki_Customize_Repeater_Control::render_content() ).
...
wp.customize.controlConstructor['wfocukirki-slider'] = wp.customize.wfocukirkiDynamicControl.extend( {
...
/* global wfocukirkiControlLoader */
wp.customize.controlConstructor['wfocukirki-sortable'] = wp.customize.Control.extend( {
...
wp.customize.controlConstructor['wfocukirki-switch'] = wp.customize.wfocukirkiDynamicControl.extend( {
...
wp.customize.controlConstructor['wfocukirki-toggle'] = wp.customize.wfocukirkiDynamicControl.extend( {
...
/* global wfocukirkiL10n, wfocukirki */
wp.customize.controlConstructor['wfocukirki-typography'] = wp.customize.wfocukirkiDynamicControl.extend( {
	...
	initWFOCUKirkiControl: function() {
		...
		if ( ! _.isUndefined( control.params.default.color ) ) {
			picker = this.container.find( '.wfocukirki-color-control' );
			picker.wpColorPicker( {
				change: function() {
					setTimeout( function() {
	...
	renderFontSelector: function() {
		...
		data = jQuery.extend( {}, controlFontFamilies, {
			default: {
				text: wfocukirkiL10n.defaultCSSValues,
				children: [
					{ id: '', text: wfocukirkiL10n.defaultBrowserFamily },
					{ id: 'initial', text: 'initial' },
					{ id: 'inherit', text: 'inherit' }
				]
			},
			standard: {
				text: wfocukirkiL10n.standardFonts,
				children: standardFonts
			},
			google: {
				text: wfocukirkiL10n.googleFonts,
				children: googleFonts
			}
		} );

		if ( wfocukirkiL10n.isScriptDebug ) {
			console.info( 'WFOCUKirki Debug: Font families for control "' + control.id + '":' );
			console.info( data );
		}
	...
	renderBackupFontSelector: function() {
		...
		if ( 'inherit' === fontFamily || 'initial' === fontFamily || 'google' !== wfocukirki.util.webfonts.getFontType( fontFamily ) ) {
	...
	renderVariantSelector: function() {
		...
		var control    = this,
			...
			data       = [],
			isValid    = false,
			fontType   = wfocukirki.util.webfonts.getFontType( fontFamily ),
		...
		if ( 'google' === fontType ) {
			variants = wfocukirki.util.webfonts.google.getVariants( fontFamily );
		}
		...
		if ( wfocukirkiL10n.isScriptDebug ) {
			console.info( 'WFOCUKirki Debug: Font variants for font-family "' + fontFamily + '":' );
	...
	getFonts: function() {
		var control            = this,
			initialGoogleFonts = wfocukirki.util.webfonts.google.getFonts(),
			googleFonts        = {},
		...
		if ( ! _.isEmpty( control.params.choices.fonts.google ) ) {
			if ( 'alpha' === control.params.choices.fonts.google[0] || 'popularity' === control.params.choices.fonts.google[0] || 'trending' === control.params.choices.fonts.google[0] ) {
				...
				googleFonts = wfocukirki.util.webfonts.google.getFonts( googleFontsSort, '', googleFontsNumber );
		...
		if ( ! _.isEmpty( control.params.choices.fonts.standard ) ) {
			_.each( control.params.choices.fonts.standard, function( fontName ) {
				if ( 'undefined' !== typeof wfocukirki.util.webfonts.standard.fonts[ fontName ] && ! _.isEmpty( wfocukirki.util.webfonts.standard.fonts[ fontName ] ) ) {
					standardFonts[ fontName ] = {};
					if ( 'undefined' !== wfocukirki.util.webfonts.standard.fonts[ fontName ].stack && ! _.isEmpty( wfocukirki.util.webfonts.standard.fonts[ fontName ].stack ) ) {
						standardFonts[ fontName ].family = wfocukirki.util.webfonts.standard.fonts[ fontName ].stack;
					} else {
						standardFonts[ fontName ].family = googleFonts[ fontName ];
					}
					if ( 'undefined' !== wfocukirki.util.webfonts.standard.fonts[ fontName ].label && ! _.isEmpty( wfocukirki.util.webfonts.standard.fonts[ fontName ].label ) ) {
						standardFonts[ fontName ].label = wfocukirki.util.webfonts.standard.fonts[ fontName ].label;
			...
			_.each( wfocukirki.util.webfonts.standard.fonts, function( font, id ) {
				standardFonts[ id ] = {
					family: font.stack,
					label: font.label
				};
	...
	localFontsCheckbox: function() {
		var control           = this,
			checkboxContainer = control.container.find( '.wfocukirki-host-font-locally' ),
			checkbox          = control.container.find( '.wfocukirki-host-font-locally input' ),
