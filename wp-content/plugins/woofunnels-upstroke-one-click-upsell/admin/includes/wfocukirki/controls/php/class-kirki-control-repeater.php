...
class WFOCUKirki_Control_Repeater extends WFOCUKirki_Control_Base {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->row_label = array(
			'type'  => 'text',
			'value' => esc_attr__( 'row', 'wfocukirki' ),
			'field' => false,
		);
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'wfocukirki' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							array(
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_attr__( 'Select a Page', 'wfocukirki' ),
								'option_none_value' => '0',
								'selected'          => '',
							)
						);
	...
	public function to_json() {
		...
		$this->json['value'] = apply_filters( "wfocukirki_controls_repeater_value_{$this->id}", $this->json['value'] );
	}
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'wfocukirki' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
				...
				<div class="repeater-row-content">
					<# _.each( data, function( field, i ) { #>
						...
						<div class="repeater-field repeater-field-{{{ field.type }}} repeater-field-{{ field.id }}">
							...
							<# } else if ( 'radio-icon' === field.type ) { #>

							<label class="wfocu_icons_wrapper">
									<# if ( field.label ) { #><span class="customize-control-title">{{{ field.label }}}</span><# } #>
									<# if ( field.description ) { #><span class="description customize-control-description">{{{ field.description }}}</span><# } #>

									<# _.each( field.choices, function( choice, i ) { #>
										<input type="radio" class="screen-reader-text" id="{{{ field.id }}}_{{ index }}_{{{ i }}}" name="{{{ field.id }}}{{ index }}" data-field="{{{ field.id }}}" value="{{{ i }}}" <# if ( field.default == i ) { #> checked="checked" <# } #>>
											<label for="{{{ field.id }}}_{{ index }}_{{{ i }}}" class="abc"><span class="dashicons {{ choice }}"></span></label>
										</input>
									<# }); #>
								</label>
							<# } else if ( 'color' === field.type ) { #>
								...
								<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'wfocukirki' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="wfocukirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'wfocukirki' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_attr_e( 'No Image Selected', 'wfocukirki' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'wfocukirki' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'wfocukirki' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'wfocukirki' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change Image', 'wfocukirki' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add Image', 'wfocukirki' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="wfocukirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'wfocukirki' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_attr_e( 'No File Selected', 'wfocukirki' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'wfocukirki' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'wfocukirki' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change File', 'wfocukirki' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add File', 'wfocukirki' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'wfocukirki' ); ?></button>
				</div>
			</li>
		</script>
		<?php
