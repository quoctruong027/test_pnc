		...
		foreach ( $layout as $single ) {
			switch ( $single ) {
				...
				case 'products':
					foreach ( $data->products as $hash_key => $product_data ) {
						if ( isset( $product_data->id ) && $product_data->id > 0 ) {
							$style       = WFOCU_Common::get_option( 'wfocu_product_product_' . $hash_key . '_layout' );
							$product_raw = array(
								'key'     => $hash_key,
								'product' => $product_data,
							);
							switch ( $style ) {
								...
								case 'style7':
								case 'style8':
									WFOCU_Core()->template_loader->get_template_part( 'product-layouts/' . $style, $product_raw );
									break;
							}
						}
					}
