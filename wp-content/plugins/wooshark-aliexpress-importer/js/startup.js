                    ...
                    try {
                        ...
                        if (
                            (t.forEach(function(e) {
                                    e && e.packageType ?
                                        jQuery(
                                            '<div class="card text-center" style="flex: 1 1 20%; margin:30px;border-radius: 10px; padding:50px">  <div class="card-body"><h5 class="card-title"> ' +
                                            ...
                                            e.productId +
                                            '</p><div><div><a  style="width:80%" id="importToShop" class="importToS btn btn-primary">Import to shop</a></div><div><a  style="width:80%; margin-top:5px"" id="addToWaitingList" class=" btn btn-primary">Add to waiting list</a></div><div><a target="_blank" style="width:80%; margin-top:5px" href="' +
                                            e.productUrl +
...
function getAllProducts(e) {
    jQuery.ajax({
        ...
        type: "POST",
        dataType: "JSON",
        data: { action: "get_products", paged: e },
        success: function(e) {
                ...
                t.find("tbody tr").remove(),
                    e.forEach(function(e, r) {
                        ...
                        t.append(
                            ...
                            e.productUrl +
                            "  target='_blank'> Original product url </a></button></td><td><button class='btn btn-primary' style='width:100%' id='sync-product-stock-and-price' disabled><a style='color:white'  target='_blank' > Update product stock and price <small style='color:red'> PREMUIM</small> </a></button></td><td><button class='btn btn-default' id='insert-product-reviews' style='width:100%'><a style='color:white'  data-toggle='modal' data-target='#myModalReviews'> Insert Reviews </a></button></td></tr>"
                        );
                    });
            }
        },
        ...
        tagsProduct && tagsProduct.length && (g = tagsProduct),
            jQuery.ajax({
                ...
                data: {
                    ...
                    quantity: 33,
                    attributes: c && c.length ? c : [],
                    variations: a.variations && a.variations.length ? a.variations : [],
                    isFeatured: p,
...
function displayAlreadyImportedIcon(e) {
    if (e && e.length) {
        ...
        for (var t = 0; t < a.length; t++) {
            ...
            if (r.indexOf(e) > -1) {
                jQuery(
                    '<div><a  style="width:80%" id="alreadyImported" class=" btn btn-default">Already imported</a></div>'
                ).appendTo(jQuery(a[t]));
            }
        }
    }
