...
class WCS_WFOCU_PayPal_Standard_IPN_Handler extends WCS_PayPal_Standard_IPN_Handler {
	...
	protected function process_ipn_request( $transaction_details ) {
		...
		WFOCU_Core()->log->log( '$subscription_id_and_key' . $subscription_id_and_key );
		WFOCU_Core()->log->log( '$subscription_key' . $subscription_key );
		...
		if ( empty( $subscription ) ) {
			...
			if ( in_array( $transaction_details['txn_type'], array( 'recurring_payment_profile_cancel', 'recurring_payment_expired' ) ) ) {
				...
				if ( ! empty( $subscription_id_and_key['order_id'] ) ) {
					WC_Gateway_Paypal::log( 'IPNN subscription cancellation request ignored - new PayPal Profile ID linked to this subscription, for subscription ' . $subscription_id_and_key['order_id'] );
		...
		if ( isset( $transaction_details['txn_id'] ) ) {
			...
			$ipn_lock_transient_name = 'wcs_pp_' . md5( $ipn_transaction_id ); // transient names need to be less than 45 characters and the $ipn_id will be long, e.g. 34292625HU746553V_recurring_payment_completed_5ab4c38e1f39d, so md5
		...
		if ( $is_renewal_sign_up_after_failure || $is_payment_change ) {
			...
			if ( empty( $existing_profile_id ) || $existing_profile_id !== $transaction_details['recurring_payment_id'] ) {
		...
		if ( isset( $transaction_details['recurring_payment_id'] ) && ! in_array( $transaction_details['txn_type'], array( 'recurring_payment_profile_cancel', 'recurring_payment_expired' ) ) ) {
			wcs_set_paypal_id( $subscription, $transaction_details['recurring_payment_id'] );

			if ( wcs_is_paypal_profile_a( $transaction_details['recurring_payment_id'], 'out_of_date_id' ) && 'disabled' != get_option( 'wcs_paypal_invalid_profile_id' ) ) {
		...
		switch ( $transaction_details['txn_type'] ) {
			case 'recurring_payment_profile_created':
				...
				if ( ! $is_payment_change && ! $is_renewal_sign_up_after_failure && 0 == $subscription->get_parent()->get_total() ) {
					// Safe to assume the subscription has an order here because otherwise we wouldn't get a 'recurring_payment_profile_created' IPN
					$subscription->get_parent()->payment_complete(); // No 'txn_id' value for 'recurring_payment_profile_created' IPN messages
				...
				if ( $is_payment_change ) {
					...
					$this->add_order_note( _x( 'IPN subscription payment method changed to PayPal.', 'when it is a payment change, and there is a recurring_payment_profile_created message, this will be a confirmation message that PayPal accepted it being the new payment method', 'woocommerce-subscriptions' ), $subscription, $transaction_details );
				...
				if ( 'completed' == strtolower( $transaction_details['payment_status'] ) ) {
					...
					if ( $is_first_payment ) {
						...
						if ( true === $is_renewal_sign_up_after_failure && is_object( $renewal_order ) ) {
							...
							if ( 'paypal' == get_post_meta( $subscription->get_id(), '_old_payment_method', true ) ) {
								...
								if ( $profile_id !== $transaction_details['recurring_payment_id'] ) {
						...
						wcs_set_paypal_id( $renewal_order, $transaction_details['recurring_payment_id'] );
					}
					...
					if ( ! $is_first_payment ) {
						...
						if ( 'failed' == strtolower( $transaction_details['payment_status'] ) ) {
							...
							// translators: placeholder is payment status (e.g. "completed")
							$this->add_order_note( sprintf( _x( 'IPN subscription payment %1$s for reason: %2$s.', 'used in order note', 'woocommerce-subscriptions' ), $transaction_details['payment_status'], $transaction_details['pending_reason'] ), $renewal_order, $transaction_details );
						}
					}
			...
			case 'recurring_payment_profile_cancel':
				...
				if ( wcs_get_paypal_id( $subscription ) != $transaction_details['recurring_payment_id'] ) {
			...
			case 'recurring_payment_failed': // Subscription sign up failed
		...
		if ( isset( $ipn_id ) && ! empty( $ipn_id ) ) { //phpcs:ignore WordPressVIPMinimum.Variables.VariableAnalysis.UndefinedVariable
			$log_message .= sprintf( ' (%s)', $ipn_id );//phpcs:ignore WordPressVIPMinimum.Variables.VariableAnalysis.UndefinedVariable
		}
	...
	public static function get_order_id_and_key( $args, $order_type = 'shop_order', $meta_key = '_paypal_subscription_id' ) {
		...
		if ( isset( $args['recurring_payment_id'] ) ) { // PayPal Standard IPN message
		...
		// Couldn't find the order ID by recurring_payment_id, so it's either not set on the order yet or the $args doesn't have a recurring_payment_id (?!), either way, let's get it from the args
	...
	protected function is_woocommerce_payload( $payload ) {
		return is_numeric( $payload ) || (bool) preg_match( '/(wc_)?order_[a-f0-9]{5,20}/', $payload );
	}
