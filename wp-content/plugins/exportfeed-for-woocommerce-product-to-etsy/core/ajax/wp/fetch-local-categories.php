...
foreach ($categoryList->categories as $this_category) {
    if (!isset($this_category->parent_category)) {
        etcpf_process_category($result->children, $this_category);
    }
}

echo wp_json_encode($result);
function etcpf_process_category(&$target_list, $this_category)
{
    ...
    foreach ($this_category->children as $child) {
        etcpf_process_category($new_category->children, $child);
    }
}