...
class ETCPF_FeedOverride extends ETCPF_BaseFeedOverride {
	...
	function __construct($providerName, $parent, $saved_feed) {
        ...
        // if($etcore->feedType){
		//        $this->loadCustomAdvancedCommands($providerName);
		//        }else{
		if (($saved_feed == null) || (!property_exists($saved_feed,'own_overrides'))) {
			$this->loadAdvancedCommands($providerName);
		} else {
		    if($saved_feed->feed_overrides==NULL){
		...
		if (!empty($this->advancedCommands)) {
			foreach ($this->advancedCommands as $this_option) {
				...
				//is a comment
				if (($this_option[0] == '/') && ($this_option[1] == '/')) {
					continue;
				}
				//is a comment
				...
				case 'set':
					...
					if (count($params) > 3) {
						if (strtolower($params[3]) == 'false') {
							$params[3] = false;
						}

						if (strtolower($params[3]) == 'true') {
							$params[3] = true;
						}

						if (strtolower($params[3]) == 'no') {
							$params[3] = false;
						}

						if (strtolower($params[3]) == 'yes') {
							$params[3] = true;
						}

						if (strtolower($params[3]) == 'off') {
							$params[3] = false;
						}

						if (strtolower($params[3]) == 'on') {
							$params[3] = true;
						}
				...
				case 'setattribute':
					...
					if (strtolower($params[2]) == 'default') {
						if (isset($params[4])) {
							...
							$defaultClass = 'ETCPF_AttributeDefault';
						}
				...
				case 'setattributedefault':
					if (isset($params[4])) {
						$defaultClass = 'ETCPF_ActionAfterHarmonize';
					} else {
						$defaultClass = 'ETCPF_AttributeDefault';
					}
				...
				case 'setrapidcarttoken':
					global $etcore;
					$etcore->settingSet('cp_rapidcarttoken', $params[1]);
					break;
				case 'addtags':
					$parent->tags = $params[1];
					if (isset($params[2])) {
		...
		if (!empty($this->advancedCustomCommands) > 0) {
			$limit = isset($GLOBALS['_POST']['feedLimit']) ? $GLOBALS['_POST']['feedLimit'] : 0;
			$command = '';
			if (!empty($limit)) {
				$limit_arr = explode('-', $limit);
				$parent->has_product_range = true;
				$parent->product_limit_low = $limit_arr[0];
				$parent->product_limit_high = $limit_arr[1];
			}

			foreach ($this->advancedCustomCommands as $key => $this_option) {
				if ($this_option['cpf_feed_type'] == 0) {
					$command = 'mapattribute';
				}
				if ($this_option['cpf_feed_type'] == 1) {
					$command = 'setattributedefault';
				}
				//Apply command
				switch ($command) {
				case '$': //Mapping 2.0 setting
				...
				case 'mapattribute':
					$usesCData = false;
					if (!isset($this_option['cpf_feed_value_default'])) {
						$this_option['cpf_feed_value_default'] = $this_option['merchant_attr'];
					}

					$mapattribute = $this_option['cpf_feed_value_default'];

					/* if(isset($this_option['cpf_prefix'])){
						                             $mapattribute = $this_option['cpf_prefix'].$mapattribute;
						                         }

						                         if(isset($this_option['cpf_suffix'])){
						                             $mapattribute =$mapattribute.$this_option['cpf_suffix'];
					*/
					$defaultClass = 'ETCPF_AttributeDefault';

					$recent_attribute = $parent->addAttributeMapping($mapattribute, $this_option['merchant_attr'], $usesCData, false, true);
//                         $recent_attribute = $parent->addAttributeMapping($params[1], $params[3], $usesCData, false, true);
					break;
				case 'rule':
				case 'addrule':
				...
				case 'setattribute':
					...
					break;
				case 'setattributedefault':
					if (strpos($this_option['merchant_attr'], ':') == true) {
						$mapAttr = substr($this_option['merchant_attr'], strpos($data, ":") + 2);
					} else {
						$mapAttr = $this_option['merchant_attr'];
					}
					$mapData = $this_option['cpf_feed_value_custom'];
					if (isset($this_option['cpf_prefix'])) {
						$mapData = $this_option['cpf_prefix'] . $mapData;
					}

					if (isset($this_option['cpf_suffix'])) {
						$mapData = $mapData . $this_option['cpf_suffix'];
					}
					$defaultClass = 'ETCPF_AttributeDefault';
					/* if (isset($params[4]))
						                             $defaultClass = $params[4];
					...
					*/
					// $parent->addAttributeDefault($params[1], $params[3], $defaultClass);
					$parent->addAttributeDefault($mapAttr, $mapData, $defaultClass);
					break;
				case 'setattributeparam':
					//For example, allows AmazonSC to set the localized name after mapAttribute [This equals setParam but allows you to specify which mapping]
					//eg: setAttributeParam description usesCData true
	...
	*/
	...
	function interpretSingleSetting($this_option, $parent) {
        ...
        if($this_option=='$getProductOfParticularCategory'){
            $parent->getProductOfParticularCategory = true;
        }
		...
		if ($this_option == '$currency') {
			$parent->currency = $value;
		} //Deprecated. Use $currency_format
		if ($this_option == '$currency_shipping') {
			$parent->currency_shipping = $value;
		}
		//if ($this_option == '$bing_force_google_category') {$parent->bingForceGoogleCategory = true;} //Not IMPL
		if ($this_option == '$bing_force_price_discount') {
			$parent->bingForcePriceDiscount = true;
		} //Debug for beta testers
		if ($this_option == '$currency_format') {
			$parent->currency_format = $value;
		}
		if ($this_option == '$default_brand') {
			$parent->addErrorMessage(11003, 'Use of deprecated command: $default_brand', true);
		}
		if ($this_option == '$exclude_variable_attributes') {
			$parent->productList->exclude_variable_attributes = true;
		}
		if ($this_option == '$field_delimiter') {
			$parent->fieldDelimiter = $value;
		}
		if ($this_option == '$get_wc_shipping_attributes') {
			$parent->get_wc_shipping_attributes = true;
		}
		if ($this_option == '$hide_out_of_stock') {
			$etcore->hide_outofstock = true;
		}
		...
		if ($this_option == '$strip_html_markup') {
			$parent->stripHTML = true;
		}
		if ($this_option == '$system_wide_shipping_type') {
			$parent->addErrorMessage(11001, 'Use of deprecated command: $system_wide_shipping_type', true);
		}
		if ($this_option == '$timeout') {
			$parent->timeout = $value;
		}
		if ($this_option == '$weight_unit') {
			$parent->weight_unit = $value;
		}
