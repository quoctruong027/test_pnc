...
class ETCPF_ProductW extends ETCPF_PAProduct
{

}

class ETCPF_MasterAttributeList
{
    protected $children;
...
class ETCPF_ProductList
{
    ...
    public $woocommerce_manage_stock;

    public $woocommerce_notify_no_stock_amount;
    ...
    */
    ...
    public function loadProducts($parent)
    {
        ...
        if (!isset($this->relatedData['brand']) && $parent->allowRelatedData) {
            $this->relatedData['brand'] = new ETCPF_ProductSupplementalData('product_brand');
        }
        if (!isset($this->relatedData['tag']) && $parent->allowRelatedData) {
            $this->relatedData['tag'] = new ETCPF_ProductSupplementalData('product_tag');
        }
    ...
    {
        $this->masterAttributeList = new ETCPF_MasterAttributeList();
        global $wpdb;
    ...
    public function getProductByCategory($parent, $remotecat)
    {
        ...
        foreach ($Products as $index => $productId) {
            ...
            $item = new ETCPF_ProductList(); //extends ETCPF_ProductList (basicfeed.php)
            ...
            if ($brandarray = wp_get_post_terms($item->id, 'product_brand', array("fields" => "names"))) {
                if (is_array($brandarray) && count($brandarray) > 0) {
                    $item->attributes['brand'] = $brandarray[0];
                } else {
            ...
            */
            ...
            $item->taxonomies = new ETCPF_TaxonomyList();
    ...
    public function getProductListDefault($parent, $remote_category)
    {
        global $woocommerce;
        ...
        foreach ($this->products as $index => $prod) {
            ...
            //Prepare the item
            $item = new ETCPF_ProductW(); //extends PAProduct (basicfeed.php)
            ...
            //tags
            $tags = get_the_terms($prod->ID, 'product_tag');
            if (is_array($tags) && count($tags) > 0) {
                ...
                $t = implode(",", $tag);
                $item->attributes['tags'] = $t;
            }

            if (isset($parent->tags)) {
                if (isset($parent->allow_tags) && isset($item->attributes['tags']) && (strlen($item->attributes['tags']) > 0))
                    $item->attributes['tags'] = $item->attributes['tags'] . ',' . $parent->tags;
                else
                    $item->attributes['tags'] = $parent->tags;
            }
            // end tags
            ...
            */
            ...
            if ($item->attributes['isVariable']) {
                ...
                $this->expandProduct($item, $parent, $available_variations);
            } else {
                $item->parent_manage_stock = 'no';
                $this->applyWCAttributes($item);
                $this->hideStockValid($item);
    ...
    public function fetchAndwriteProducts($parent, $args)
    {
        if ($args) {
            ...
            foreach ($args['products'] as $key => $product) {
                ...
                $productData = $this->getWooProductData($key);
                $localCategoriesArray = get_the_terms($key, 'product_cat');
                $localcatString = '';
    ...
    private function ManageVariationForCustom($variationIds, $actualVariation)
    {
        $variations = array();
        foreach ($actualVariation as $key => $variation) {
    ...
    function hideStockValid($item)
    {
        global $etcore;
        //Hide out of stock
        /* if ( ($etcore->manage_stock) ) */
        $statusCheck = $this->CheckifProductInListings($item->id);
        if ($statusCheck && $statusCheck !== 'draft' && $statusCheck !== 'inactive') {
            $item->attributes['state'] = $statusCheck;
        } else {
            $item->attributes['state'] = get_etsy_settings('state') ? get_etsy_settings('state') : 'draft';
        }

        if (($etcore->hide_outofstock) && ($item->attributes['stock_status'] == 0)) {
            if ($statusCheck !== false) {
                $item->attributes['valid'] = true;
                $item->attributes['stock_quantity'] = 1;
                $item->attributes['state'] = 'inactive';
                /* if ($statusCheck === 'active' || $statusCheck === 'inactive') {
                     $item->attributes['state'] = 'inactive';
                 } else {
                     $item->attributes['state'] == $statusCheck;
                 }*/
        ...
        } elseif ($item->attributes['manage_stock'] == 'no' && $item->attributes['stock_status'] == true) {
            if (($default_stock = get_etsy_settings('default_stock_quantity')) === null || $default_stock < 1)
                $item->attributes['stock_quantity'] = 10;
            else
                $item->attributes['stock_quantity'] = $default_stock;
        } elseif ($item->attributes['stock_status'] == 0 && $statusCheck !== false) {
            $item->attributes['valid'] = true;
            $item->attributes['stock_quantity'] = 1;
            $item->attributes['state'] = 'inactive';
            /*if ($statusCheck === 'active' || $statusCheck === 'inactive') {
    ...
    function hideStockValidChild($item)
    {
        global $etcore;
        //Hide out of stock
        /* if ( ($etcore->manage_stock) ) */
        $statusCheck = $this->CheckifProductInListings($item->attributes['item_group_id']);

        if ($statusCheck && $statusCheck !== 'draft' && $statusCheck !== 'inactive') {
            $item->attributes['state'] = $statusCheck;
        } else {
            $item->attributes['state'] = get_etsy_settings('state');
        }
        if($item->attributes['is_in_stock'] == true && $item->attributes['stock_quantity'] === 0){
            if (($default_stock = get_etsy_settings('default_stock_quantity')) === null || $default_stock < 1)
                $item->attributes['stock_quantity'] = 10;
            else
                $item->attributes['stock_quantity'] = $default_stock;
    ...
    public function getstockofAproduct($product)
    {
        if ($product->get_stock_quantity() > 0 && ($product->get_stock_quantity() < 999)) {
            ...
            return 999;
        } elseif ($product->get_manage_stock() == 'no' && $product->get_stock_status()) {
            if (($default_stock = get_etsy_settings('default_stock_quantity')) === null || $default_stock < 1)
                return 10;
            else
    ...
    public function CheckifProductInListings($itemid)
    {
        global $wpdb;
        $table = $wpdb->prefix . 'etcpf_listings';
        $result = $wpdb->get_row($wpdb->prepare("SELECT etsy_state,listing_id, upload_result FROM {$table} WHERE item_id=%d", array($itemid)));
        return empty($result->listing_id) ? false : $result->etsy_state;
    }

    /**
     * @function: expandProduct
     * $Info: Function is used for handling variation Products
     * */
    function expandProduct($listitem, $parent, array $variations)
    {
        global $wpdb;
        global $etcore;
        ...
        ***************************************** Process the results **************************************/
        ...
        if ($variations && count($variations) > 0) {
            foreach ($variations as $key => $variation) {
                ...
                $childItem->attributes['image_link'] = $variation['image']['url'];
                ...
                $childItem->attributes['is_in_stock'] = $variation['is_in_stock'];
    ...
    public function manageVariation(array $products)
    {
...
//currently it triggers on piped values in the MasterAttributeTable (which comes from the central table). Soon it will come from the product's attribute value
    ...
    public
    function postmetaLookup($item, $parent)
    {
        global $wpdb;
        //********************************************************************
