...
class ETCPF_ActionAfterHarmonize extends ETCPF_AttributeDefault
{
...
class ETCPF_ActionAfterLoad extends ETCPF_AttributeDefault
{
...
class ETCPF_ActionBeforeFeed extends ETCPF_AttributeDefault
{
...
class ETCPF_ActionAfterFeed extends ETCPF_AttributeDefault
{
...
class ETCPF_CategoryLookUp extends ETCPF_AttributeDefault
{
...
class ETCPF_CategoryTree extends ETCPF_ActionBeforeFeed
{
...
class ETCPF_CatVar extends ETCPF_ActionAfterHarmonize
{
...
class ETCPF_ConvertSpecialCharacters extends ETCPF_ActionAfterFeed
{
...
class ETCPF_FirstFoundColor extends ETCPF_AttributeDefault
{
...
class ETCPF_GraziaBusinessRule extends ETCPF_ActionBeforeFeed
{
...
//create output for Etsy additional image links. You can include up to 10 additional images per item
class ETCPF_GoogleAdditionalImages extends ETCPF_ActionAfterFeed
{
    ...
    public function postProcess($product, &$output)
    {
        ...
        global $etcore;
        if ($etcore->callSuffix != 'W' && $etcore->callSuffix != 'We') {
            return;
        }
        ...
        if (!isset($product->attributes['feature_imgurl'])) {
            ...
            foreach ($product->imgurls as $imgurl) {
                if ($image_count == 0) {
                    $output .= $this->parent_feed->formatLine('image_link', $imgurl, false);
                    $image_count++;
                } elseif ($image_count < 10) {
                    $output .= $this->parent_feed->formatLine('additional_image_link', $imgurl, true);
                    $image_count++;
                } else {
                    break;
                }
        ...
        if ($this->parent_feed->allow_additional_images) {
            ...
            foreach ($product->imgurls as $imgurl) {
                // $output .= $this->parent_feed->formatLine('additional_image_link', $imgurl, true);
                $additional_images[] = $imgurl;
                $image_count++;
                if ($image_count > 9) {
                    break;
                }
            }
            $formated_images =  implode(',', $additional_images);
            $output .= $this->parent_feed->formatLine('additional_image_link', $formated_images, true);
        }
...
class ETCPF_GoogleShipping extends ETCPF_ActionAfterFeed
{
    ...
    public function postProcess($product, &$output)
    {
        ...
        $output .= '
        <shipping>' .
            $this->parent_feed->formatLine('service', $product->attributes['shipping_type'], false, '  ') .
            $this->parent_feed->formatLine('price', sprintf($this->parent_feed->currency_format, $product->attributes['shipping_amount']), false, '  ') . '
        </shipping>';
...
class ETCPF_GoogleTax extends ETCPF_ActionAfterFeed
{
    ...
    public function postProcess($product, &$output)
    {
        ...
        if (isset($product->attributes['tax'])) {
            $output .= '
        <tax>' .
                $this->parent_feed->formatLine('country', $product->attributes['tax_country'], false, '  ') .
                $this->parent_feed->formatLine('rate', sprintf($this->parent_feed->currency_format, $product->attributes['tax']), false, '  ') . '
        </tax>';
        }
 ...
 */
class ETCPF_MergeFields extends ETCPF_ActionBeforeFeed
{
...
class ETCPF_SalePriceIfDefined extends ETCPF_ActionBeforeFeed
{
...
class ETCPF_VaryOnId extends ETCPF_ActionBeforeFeed
{
...
class ETCPF_VaryOnSku extends ETCPF_ActionBeforeFeed
{
...
class ETCPF_RemoveZeroPricedItems extends ETCPF_ActionBeforeFeed
{
