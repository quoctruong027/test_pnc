...
class ETCPF_ProductList
{
    ...
    public function loadProducts($parent)
    {
        ...
        $sql = "
			SELECT 
				posts.ID, posts.post_title, posts.post_date, posts.post_content, posts.post_excerpt, posts.post_content as description,
    ...
    public function getProductList($parent, $remote_category)
    {
        ...
        foreach ($this->products as $index => $prod) {
            ...
            $item->attributes['link'] = $etcore->siteHost . '?wpsc-product=' . rawurlencode($item->attributes['title']);
