...
function wad_get_cart_products() {
    ...
    foreach ($woocommerce->cart->cart_contents as $cart_item_key => $cart_item_data) {
        if ( isset( $cart_item_data['wad_free_gift'] ) )
            continue;
 ...
 * @global type $wpdb
 * @param type $list List to check the products against
 * @return array
 */
        ...
        if ($limit) {
            ...
            if (
                ...
                if ($group_by_types) {
                    ...
                    else if (in_array($metas["action"], $shipping_based_actions))
...
{
    return array("percentage-off-shipping-fee", "fixed-amount-off-shipping-fee", "fixed-shipping-fee");
}
...
//       return 0;
   ...
   foreach( $items as $item => $values ) {
       if( isset( $values["wad_free_gift"] ) ) {
           $gift_total += floatval( $values['data']->get_price() ) * ( $values['quantity'] );
       }
   }
   $cart_total -= $gift_total;
...
function wad_load_xml_from_url($url) {
