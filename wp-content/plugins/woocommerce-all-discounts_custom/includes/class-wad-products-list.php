...
//        var_dump("ready for action");
        ...
        $cat_name = 'Angles'; // Product category name
        $attr_taxonomy = 'attribute_pa_picture-size'; // Product attribute
        $attribute_term_slugs = array('665'); // <== Need to be term SLUGs
        ...
        foreach($args["tax_query"] as $key=>$value){
            ...
            if($value['operator']=="IN"){
                foreach($value['terms'] as $item){
                    array_push($in,$item);
                }
            }
            if($value['operator']=="NOT IN"){
                foreach($value['terms'] as $item){
                    array_push($not_in,$item);
                }
            }
        }
        unset($args["tax_query"]);

        if($this->get_product_by_category($in,$not_in))$args=array_merge($args,['post_parent__in'=>$this->get_product_by_category($in,$not_in)]);
    ...
    public function get_product_by_category($cat,$not_cat){
        global $wpdb;
        $argss = array(
            'post_type' => 'product',
            'tax_query' => array(
        ...
        AND tr.term_taxonomy_id in ('$to_return')
    " );
    }
