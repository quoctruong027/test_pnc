...
class VI_WOOCOMMERCE_ORDERS_TRACKING_FRONTEND_ORDER_DETAILS {
	...
	public function wp_enqueue_scripts() {
		if ( is_account_page() || apply_filters( 'woocommerce_orders_tracking_display_tracking_for_order_details', false ) ) {
	...
	protected function print_tracking_number( $current_tracking_data, $item_id, $order_id, $order, &$tracking_list ) {
		...
		if ( $tracking_number && ! in_array( $tracking_number, $tracking_list ) ) {
			...
			if ( $tracking_status ) {
				...
				$title             = sprintf( esc_attr__( 'Shipment status: %s', 'woocommerce-orders-tracking' ), VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::get_status_text( $tracking_status ) );
			}
			?>
