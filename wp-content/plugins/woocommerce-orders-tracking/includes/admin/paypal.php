...
class VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_PAYPAL {
	...
	public static function get_access_token( $client, $secret, $url, $new_token = false ) {
		...
		if ( ! $new_token && $transient_token ) {
			...
			if ( empty( $result ) ) {
				$token = array(
					'status' => 'error',
					'data'   => esc_html__( 'Your client ID or client secret is wrong', 'woocommerce-orders-tracking' )
				);
			} else {
	...
	private static function add_trackinfo( $data, $token, $url ) {
		...
		$request        = VI_WOOCOMMERCE_ORDERS_TRACKING_DATA::wp_remote_post( $url, $arg );
