...
class VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_WOO_ORDER_EMAIL {
	...
	public function include_tracking_info( $order ) {
		if ( $order ) {
			...
			$email_woo_html               = $this->settings->get_params( 'email_woo_html', '', $language );
			$email_woo_tracking_list_html = $this->settings->get_params( 'email_woo_tracking_list_html', '', $language );
	...
	protected function get_tracking_list( $current_tracking_data, $order, $email_woo_tracking_list_html, &$tracking_info, &$tracking_list ) {
		...
		$tracking_url = $this->settings->get_url_tracking( $carrier_url, $tracking_code, $carrier_id, $order->get_shipping_postcode(), false, false );
	...
	public static function include_tracking_info_after_order_item( $item_id, $order, $plain_text = false, $add_nonce = false, $language = '' ) {
		if ( ! $plain_text ) {
			...
			$email_woo_tracking_number_html  = $settings->get_params( 'email_woo_tracking_number_html', '', $language );
			$email_woo_tracking_carrier_html = $settings->get_params( 'email_woo_tracking_carrier_html', '', $language );
	...
	protected static function print_tracking_info( $current_tracking_data, $settings, $order, $email_woo_tracking_number_html, $email_woo_tracking_carrier_html, $add_nonce ) {
		...
		$tracking_url = $settings->get_url_tracking( $carrier_url, $tracking_code, $carrier_id, $order->get_shipping_postcode(), false, $add_nonce );
