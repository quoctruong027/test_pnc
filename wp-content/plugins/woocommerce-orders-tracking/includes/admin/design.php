...
class VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_DESIGN {
	...
	protected function add_section_design_tracking_form( $wp_customize ) {
		...
		$wp_customize->add_control( 'woo_orders_tracking_settings[tracking_form_button_track_title]', array(
			...
			'priority' => 10,
			'section'  => 'vi_wot_orders_tracking_design_tracking_form',
			'label'    => esc_html__( 'Button Track title', 'woocommerce-orders-tracking' ),
		) );
		if ( count( $this->languages ) ) {
			foreach ( $this->languages as $key => $value ) {
				...
				$label = esc_html__( 'Button Track title', 'woocommerce-orders-tracking' ) . "({$value})";
				if ( isset( $this->languages_data[ $value ]['translated_name'] ) ) {
					$label = esc_html__( 'Button Track title', 'woocommerce-orders-tracking' ) . "({$value}-{$this->languages_data[ $value ]['translated_name']})";
				}
		...
		$wp_customize->add_control(
			new WP_Customize_Color_Control(
				...
				array(
					'label'   => esc_html__( 'Button Track text color', 'woocommerce-orders-tracking' ),
					'section' => 'vi_wot_orders_tracking_design_tracking_form',
				) )
		);
		...
		$wp_customize->add_control(
			new WP_Customize_Color_Control(
				...
				array(
					'label'   => esc_html__( 'Button Track background color', 'woocommerce-orders-tracking' ),
					'section' => 'vi_wot_orders_tracking_design_tracking_form',
				) )
		);
	}
	...
	protected function add_section_design_general( $wp_customize ) {
		...
		$wp_customize->add_control( 'woo_orders_tracking_settings[timeline_track_info_date_format]', array(
			...
			'choices'  => array(
				'j F, Y' => esc_html__( date( 'j F, Y' ) . ' ( j F, Y )', 'woocommerce-orders-tracking' ),
				'Y-m-d'  => esc_html__( date( 'Y-m-d' ) . ' ( Y-m-d )', 'woocommerce-orders-tracking' ),
				'm/d/Y'  => esc_html__( date( 'm/d/Y' ) . ' ( m/d/Y )', 'woocommerce-orders-tracking' ),
				'd/m/Y'  => esc_html__( date( 'd/m/Y' ) . ' ( d/m/Y)', 'woocommerce-orders-tracking' ),
			)
		) );
		/*
		 * time format
		...
		$wp_customize->add_control( 'woo_orders_tracking_settings[timeline_track_info_time_format]', array(
			...
			'choices'  => array(
				'g:i a' => esc_html__( date( 'g:i a' ) . ' ( g:i a)', 'woocommerce-orders-tracking' ),
				'g:i A' => esc_html__( date( 'g:i A' ) . ' ( g:i A )', 'woocommerce-orders-tracking' ),
				'H:i'   => esc_html__( date( 'H:i' ) . ' ( H:i )', 'woocommerce-orders-tracking' ),
			)
		) );
		/*
		 * template
