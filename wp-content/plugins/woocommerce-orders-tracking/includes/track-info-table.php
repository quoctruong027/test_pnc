...
class VI_WOOCOMMERCE_ORDERS_TRACKING_Table_Tracking {
	...
	public static function vi_wot_add_tracking_code_to_track_service( $data ) {
		...
		$result   = VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_TRACK_ORDER_DATA::tracking_batch( $api, $data );
	...
	public static function vi_wot_add_track_info_to_database( $data ) {
		...
		foreach ( $data as $item ) {
			...
			if ( $check_exit_tracking > 0 ) {
				...
				$result[ 'order-' . $item['order_id'] . '-' . $item['tracking_code'] ] = $wpdb->query( $wpdb->prepare( $sql, $item['order_id'], $item['tracking_code'], '', $item['carrier_id'], $item['carrier_name'], $item['shipping_country_code'], '', '', $date, '' ) );
	...
	public static function refresh_track_info_database( $data, $insert = false ) {
		...
		if ( $check_exit_tracking > 0 ) {
			$track_info = VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_TRACK_ORDER_DATA::tracking_info( $data['tracking_code'], $service_carrier_api_key, $data['carrier_id'], $data['carrier_name'], $data['shipping_country_code'] );
			$database   = array();
			if ( $track_info ) {
				...
				switch ( $track_info['status'] ) {
					case 'success':
						$database[ 'update-order-' . $data['order_id'] . '-' . $data['tracking_code'] ] = $wpdb->query( $wpdb->prepare( $sql, $track_info['data']['status'], $data['carrier_slug'], $data['carrier_name'], $data['shipping_country_code'], json_encode( $track_info['data'] ), $track_info['data']['last_event'], $date, $data['tracking_code'] ) );
						break;
					case 'error':
						$database[ 'update-order-' . $data['order_id'] . '-' . $data['tracking_code'] ] = $wpdb->query( $wpdb->prepare( $sql, '', $data['carrier_slug'], $data['carrier_name'], $data['shipping_country_code'], '', json_encode( $track_info['data'] ), $date, $data['tracking_code'] ) );
						break;
				}
			}
			...
			if ( $insert ) {
				...
				$tt         = VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_TRACK_ORDER_DATA::tracking_batch( $service_carrier_api_key, $data_batch );
				if ( ! $tt ) {
					return false;
				}
				$track_info = VI_WOOCOMMERCE_ORDERS_TRACKING_ADMIN_TRACK_ORDER_DATA::tracking_info( $data['tracking_code'], $service_carrier_api_key, $data['carrier_id'], $data['carrier_name'], $data['shipping_country_code'] );
