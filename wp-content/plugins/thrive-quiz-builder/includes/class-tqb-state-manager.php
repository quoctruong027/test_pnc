...
class TQB_State_Manager extends TQB_Request_Handler {
	...
	public function api_update() {
		...
		TQB_Variation_Manager::save_child_variation( array(
			...
			'tcb_fields' => array(
				'min'        => $min,
				'max'        => $max,
				'width'      => $child_width - $diff_width,
				'inline_css' => ! empty( $child_variation['tcb_fields']['inline_css'] ) ? $child_variation['tcb_fields']['inline_css'] : array(),
			),
			'post_title' => $min . ' - ' . $max,
		) );
	...
	public function api_get_child_variation() {
		...
		if ( ! empty( $child_variation_id ) && is_numeric( $child_variation_id ) ) {
			...
			$child_variation['content']    = tve_do_wp_shortcodes( tve_thrive_shortcodes( $child_variation['content'], true ), true );
			$child_variation['custom_css'] = ( ! empty( $child_variation[ Thrive_Quiz_Builder::FIELD_INLINE_CSS ] ) ) ? $child_variation[ Thrive_Quiz_Builder::FIELD_INLINE_CSS ] : '';
			/*Only outputs tve_custom_style. This is for the dynamic content content CSS*/
			$child_variation['parent_variation_custom_css'] = TCB_Hooks::tqb_editor_output_custom_css( $this->variation, true );
