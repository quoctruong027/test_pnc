...
if ( ! class_exists( 'XLWCTY_Power_Pack_License_Handler' ) ) :

	/**
	 * EDD_License Class
	 */
	class XLWCTY_Power_Pack_License_Handler {
		...
		private function includes() {
			if ( ! class_exists( 'XLWCTY_Power_Pack_Plugin_Updater' ) ) {
				require_once 'class-xlwcty-power-pack-plugin-updater.php';
			}
		}
		...
		private function auto_updater() {
			...
			// Setup the updater
			$edd_updater = new XLWCTY_Power_Pack_Plugin_Updater( $this->api_url, $this->file, array(
		...
		public function activate_license( $license ) {
			...
			if ( class_exists( 'XL_admin_notifications' ) && $license_data->license == 'valid' ) {
				do_action( 'xl_license_activated', XLWCTY_POWER_PACK_PLUGIN_BASENAME );
		...
		public function deactivate_license() {
			...
			return ( 'deactivated' == $license_data->license ) ? true : false;
		}
