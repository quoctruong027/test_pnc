...
final class BWFAN_Fluent_Form_Submit extends BWFAN_Event {
	...
	private function __construct() {
		$this->event_merge_tag_groups = array( 'fluentforms' );
		...
		$this->event_rule_groups      = array( 'fluentforms', 'bwf_contact' );
	...
	public function get_form_fields( $form_id ) {
		...
		foreach ( $form_fields as $field_key => $fields ) {
			if ( isset( $fields->attributes ) && isset( $fields->attributes->type ) && ( ( 'submit' === $fields->attributes->type ) || ( 'hidden' === $fields->attributes->type ) ) ) {
				continue;
			}
			...
			if ( is_array( $fields ) && 1 < count( $fields ) ) {
				foreach ( $fields as $f_key => $field ) {
					if ( isset( $field->attributes ) && isset( $field->attributes->type ) ) {
						...
						$fields_data[ $field->attributes->name ] = $field->settings->label;
					}
					...
					$fields_data[ $field->attributes->name ] = isset( $field->settings->label ) ? $field->settings->label : '';

					if ( isset( $field->fields ) && ! empty( $field->fields ) ) {
						foreach ( $field->fields as $fiel ) {
							if ( false === $fiel->settings->visible ) {
								continue;
							}
							$fields_data[ $field->attributes->name . ':' . $fiel->attributes->name ] = $fiel->settings->label;
