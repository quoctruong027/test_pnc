...
final class BWFAN_WCS_Before_End extends BWFAN_Event {
	...
	private function __construct() {
		...
		$this->event_name             = esc_html__( 'Subscriptions Before End', 'autonami-automations-pro' );
		$this->event_desc             = esc_html__( 'This event runs every day and checks for the subscriptions which are about to expire/ end.', 'autonami-automations-pro' );
	...
	public function load_hooks() {
		add_action( 'bwfan_subscription_before_end_triggered', array( $this, 'capture_subscription' ), 10, 1 );
	...
	public function get_view( $db_eventmeta_saved_value ) {
        ...
        <script type="text/html" id="tmpl-event-<?php echo esc_html__( $this->get_slug() ); ?>">
            ...
            <div class="bwfan-col-sm-12 bwfan-p-0">
                <label for="bwfan-days_before" class="bwfan-label-title"><?php echo esc_html__( 'Days before subscription  end', 'autonami-automations-pro' ); ?></label>
	...
	public function make_task_data( $automation_id, $automation_details ) {
		...
		foreach ( $query->posts as $subscription_id ) {
			do_action( 'bwfan_subscription_before_end_triggered', $subscription_id );
		}
	}
