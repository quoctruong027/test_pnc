...
final class BWFAN_TVE_Lead_Form_Submit extends BWFAN_Event {
	...
	private function __construct() {
		$this->event_merge_tag_groups = array( 'thrive-forms' );
		...
		$this->event_rule_groups      = array( 'thrive-forms', 'bwf_contact' );
	...
	public function get_view_data() {
		...
		foreach ( $posts as $post ) {
			$form_variation_data = tve_leads_get_form_variations( $post );
			/**  ignore shortcode which having contact form */
			$form_variation_content = isset( $form_variation_data[0]['content'] ) ? $form_variation_data[0]['content'] : '';
			preg_match_all( '/<[^>]*class="[^"]*\bthrv-contact-form\b[^"]*"[^>]*>/i', $form_variation_content, $matches );  // getting

			/** also ignore leads without forms */
			preg_match_all( '#<\s*?form\b[^>]*>(.*?)</form\b[^>]*>#s', $form_variation_content, $form_matches );

			if ( ! empty( array_filter( $matches ) ) || empty( array_filter( $form_matches ) ) ) {
				continue;
			}
                ...
                jQuery.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (!_.isEmpty(response)) {
                            var label_details = jQuery("#tve_labels", response).val();
                            if (_.isEmpty(label_details)) {
                                jQuery('.bwfan-tve-forms-map .bwfan_spinner').addClass('bwfan_hide');
                                jQuery(".bwfan-tve-field-map").hide();
                                return false;
                            }

                            // get the fields of the current form using ajax
                            jQuery.ajax({
                                method: 'post',
                                url: "<?php echo admin_url( 'admin-ajax.php' ); ?>",
                                datatype: "JSON",
                                data: {
                                    action: 'bwfan_get_tve_get_fields',
	...
	public function bwfan_get_tve_form_contents() {
		...
		$variations = tve_leads_get_form_variations( $form_id, array(
			'tracking_data' => false,
		...
		$form_variation = isset( $variations[0]['content'] ) ? $variations[0]['content'] : '';
	...
	public function process( $post_data ) {
		...
		$form_type_id          = ! empty( $tl_data ) && isset( $tl_data['form_type_id'] ) ? $tl_data['form_type_id'] : '';
	...
	public function add_form_data( $event_js_data, $automation_meta ) {
		...
		$fields = maybe_unserialize( base64_decode( $label_code ) );
