...
final class BWFAN_UpStroke_Product_Accepted extends BWFAN_Event {
	...
	private function __construct() {
		$this->event_merge_tag_groups = array( 'wc_customer', 'wc_order', 'wc_funnel', 'wc_offer', 'wc_offer_product' );
		$this->optgroup_label         = esc_html__( 'Offer', 'autonami-automations-pro' );
		$this->event_name             = esc_html__( 'Offer Accepted', 'autonami-automations-pro' );
		$this->event_desc             = esc_html__( 'This event runs after an offer is accepted by the customer.', 'autonami-automations-pro' );
	...
	public function load_hooks() {
		add_action( 'wfocu_product_accepted_event', [ $this, 'product_accepted' ], 999, 1 );
	}
	...
	public function product_accepted( $details ) {
	...
	public function get_event_data() {
		...
		$data_to_send['global']['accepted_product_id']   = isset( $this->details['product_id'] ) ? $this->details['product_id'] : '';
		$data_to_send['global']['accepted_product_name'] = isset( $this->details['product_title'] ) ? $this->details['product_title'] : '';
	...
	public function set_merge_tags_data( $task_meta ) {
		...
		if ( empty( $wc_order_id ) || intval( $wc_order_id ) !== intval( $task_meta['global']['order_id'] ) ) {
			$set_data = array(
				...
				'accepted_product_id'   => intval( $task_meta['global']['accepted_product_id'] ),
				'accepted_product_name' => $task_meta['global']['accepted_product_name'],
...
if ( bwfan_is_woocommerce_active() && bwfan_is_woofunnels_upstroke_active() ) {
	return 'BWFAN_UpStroke_Product_Accepted';
}
