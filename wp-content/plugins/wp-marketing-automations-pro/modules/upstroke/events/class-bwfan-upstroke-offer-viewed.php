...
final class BWFAN_UpStroke_Offer_Viewed extends BWFAN_Event {
	...
	private function __construct() {
		...
		$this->event_name             = esc_html__( 'Offer Viewed', 'autonami-automations-pro' );
		$this->event_desc             = esc_html__( 'This event runs after an offer is viewed by the customer.', 'autonami-automations-pro' );
	...
	public function load_hooks() {
		add_action( 'wfocu_offer_viewed_event', [ $this, 'offer_viewed' ], 999, 4 );
	}
	...
	public function offer_viewed( $offer_id, $order_id, $funnel_id, $type_of_offer ) {
		$details = array(
			'funnel_id'     => $funnel_id,
			'offer_id'      => $offer_id,
	...
	public function capture_async_data() {
		...
		$this->funnel_id  = $details['funnel_id'];
		$this->offer_id   = $details['offer_id'];
		$this->offer_type = $details['type_of_offer'];
...
if ( bwfan_is_woocommerce_active() && bwfan_is_woofunnels_upstroke_active() ) {
	return 'BWFAN_UpStroke_Offer_Viewed';
}
