...
class BWFAN_LD_Quiz_Highest_Points extends BWFAN_Merge_Tag {

	private static $instance = null;

	public function __construct() {
		$this->tag_name        = 'ld_quiz_highest_points';
		$this->tag_description = __( 'User\'s highest points in a Quiz', 'autonami-automations-pro' );
		add_shortcode( 'bwfan_ld_quiz_highest_points', array( $this, 'parse_shortcode' ) );
	}
	...
	public function parse_shortcode( $attr ) {
		...
		if ( ! empty( $quizzes ) && is_array( $quizzes ) ) {
			foreach ( $quizzes as $quiz ) {
				...
				/** If no quiz is selected ( i.e.: $quiz_id === 0 ), then check highest points among all quizzes attempts, otherwise check against the specified quiz_id */
				$highest_points = ( ( $quiz_id === 0 || $quiz_id === $current_quiz ) && $points > $highest_points ) ? $points : $highest_points;
			}
		}

		return $this->parse_shortcode_output( $highest_points, $attr );
	}
...
if ( bwfan_is_learndash_active() ) {
	BWFAN_Merge_Tag_Loader::register( 'learndash_user', 'BWFAN_LD_Quiz_Highest_Points' );
}
