...
if(!class_exists('XT_Framework_Admin_Tabs')) {
	...
	abstract class XT_Framework_Admin_Tabs {
		...
		protected function init() {
			...
			$this->core->plugin_loader()->add_action( 'admin_menu', $this, 'tabs_admin_menu' );

			if ( $this->is_admin_tabs_page() ) {

				$this->core->plugin_loader()->add_action( 'admin_enqueue_scripts', $this, 'enqueue_styles', 1 );
				$this->core->plugin_loader()->add_action( 'admin_enqueue_scripts', $this, 'enqueue_scripts', 1 );
				$this->core->plugin_loader()->add_action( 'admin_body_class', $this, 'admin_body_class', 1 );
				$this->core->plugin_loader()->add_filter( 'admin_footer_text', $this, 'footer_text', 999 );
				$this->core->plugin_loader()->add_filter( 'update_footer', $this, 'footer_version', 999 );
		...
		public function footer_text() {
            ...
            return '<span id="footer-thankyou">Developed by <a href="https://xplodedthemes.com" target="_blank"><strong>XplodedThemes</strong></a></span>';
		}
		...
		public function enqueue_styles() {
			...
			wp_enqueue_style( $this->core->framework_prefix( 'admin-tabs' ), xtfw_dir_url( XTFW_DIR_ADMIN_TABS_ASSETS ) . '/css/admin-tabs.css', array(), XTFW_VERSION, 'all' );
		...
		public function enqueue_scripts() {
			...
			wp_enqueue_script( 'xt-follow', '//xplodedthemes.s3.amazonaws.com/widgets/xt-follow/xt-follow-min.js', array(), XTFW_VERSION, false );
		}
		...
		public function show_nav_tab( $tab ) {
			...
			echo '<a title="' . sanitize_text_field( $tab['title'] ) . '" href="' . esc_url( $url ) . '" class="nav-tab ' . implode( " ", $classes ) . '" target="' . esc_attr( $target ) . '">';
			...
			echo sanitize_text_field( $tab['title'] );

			if( !empty($tab['badges'])) {
				foreach($tab['badges'] as $badge) {

					$badge['title'] = !empty($badge['title']) ? $badge['title'] : '';
					echo '<span title="'.esc_attr($badge['title']).'" class="update-plugins">'.sanitize_text_field($badge['content']).'</span>';
				}
			}
		...
		public function get_global_badge() {
			...
			if($notifications > 0) {
				return ' <span class="update-plugins">'.sanitize_text_field($notifications).'</span>';
			}
