...
if(!class_exists('XT_Framework_Customizer')) {
    ...
    class XT_Framework_Customizer {
        ...
        public function hooks() {
            ...
            $this->loader()->add_action( 'customize_preview_init', $this, 'customizer_preview_assets' );
            $this->loader()->add_action( 'customize_controls_enqueue_scripts', $this, 'customizer_controls_assets' );

            $this->core->framework_loader()->add_filter( 'upload_mimes', $this, 'allow_myme_types', 1, 1 );
            $this->core->framework_loader()->add_filter( 'wp_check_filetype_and_ext', $this, 'check_filetype_and_ext', 10, 4 );

            $this->core->framework_loader()->add_filter('body_class', $this, 'customizer_preview_class', 1, 1);
        ...
        public function loader() {
            ...
            return $this->is_module ? $this->module->loader() : $this->core->plugin_loader();
        }
        ...
        public function customizer_controls($wp_customize ) {
            ...
            $controls = apply_filters($this->core->framework_prefix('customizer_controls'), $controls);
        ...
        public function customizer_tab_section() {
            ...
            if(!$this->is_module && $this->core->has_modules()) {

                $total_modules = $this->core->modules()->count();

	            foreach($this->core->modules()->all() as $module) {

                    echo '<h4 class="xtfw-modules-title">'._n( 'Shared Module', 'Shared Modules', $total_modules, 'xt-framework' ).'</h4>';
        ...
        public function render_customizer_tab_panels(&$customizer = null) {
	        ...
	        if($has_no_panels) {
		        ...
		        foreach ( $customizer->panels as $panel ) {
			        ...
			        if ( ! $has_one_panel ) {
				        echo '<li>';
				        echo '    <a title="' . esc_html__( 'Open in Customizer', 'xt-framework' ) . '" href="' . $customizer->customizer_link( $panel['id'] ) . '"><span class="dashicons ' . $panel['icon'] . '"></span> ' . $panel['title'] . '</a>';
			        }
	    ...
	    public function render_customizer_tab_sections(&$customizer = null, $panel = null) {
		    ...
		    foreach($customizer->sections as $section) {
			    ...
			    if(!$has_no_panels && !$has_one_panel && (empty($section['panel']) || (!empty($panel) && $section['panel'] !== $panel['id']))) {
				    continue;
			    }

			    echo '<li><a title="'.esc_html__('Open in Customizer', 'xt-framework').'" href="'.$customizer->customizer_link(null, $section['id']).'"><span class="dashicons '.$section['icon'].'"></span> '.$section['title'].'</a></li>';
		    }
        ...
        public function customizer_controls_assets() {
            ...
            $handle = $this->core->framework_prefix('customizer-controls');
