	...
	$(function() {
		...
		function initEvents() {
			...
			cartTriggers.forEach(function(trigger) {
				...
				$(trigger).on(clickSelector, function(evt){
					evt.preventDefault();
					toggleCart();
				});
		...
		function setCartHeight() {
			...
			if(XT_WOOFC.can_use_premium_code === "1" && XT_WOOFC.cart_autoheight === "1") {
				...
				if (cartActive && !cartTransitioning) {
					...
					if(paymentButtonsEnabled && !cartIsEmpty && !$(paymentButtons).is(':empty')) {
						autoHeight += $(paymentButtons).outerHeight(true) + 2; // +2px from body height
					}
		...
		function refreshCartVisibility(noAnimations) {
			...
			if($(paymentButtons).length) {
				if(paymentButtonsEnabled && cartActive && !cartIsEmpty && !$(paymentButtons).is(':empty')) {
					var buttonsHeight = $(paymentButtons).outerHeight(true) + 2;  // +2px from body height
					if(!noAnimations) {
						$$(cartWrapper).animate({paddingBottom: buttonsHeight});
					}else{
						$$(cartWrapper).css({paddingBottom: buttonsHeight});
					}
				}else{
					if(!noAnimations) {
						$$(cartWrapper).animate({paddingBottom: 0});
					}else{
						$$(cartWrapper).css({paddingBottom: 0});
					}
				}
			}
		}
