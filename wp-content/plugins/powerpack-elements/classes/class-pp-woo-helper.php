...
class PP_Woo_Helper {
	...
	public function default( $addons = '', $settings = array() ) {
		...
		if ( 'product' === get_post_type() ) {
			...
			if ( function_exists( 'woo_get_last_product_id' ) ) {
				self::$product_id = woo_get_last_product_id();
				$product = wc_get_product( woo_get_last_product_id() );
			}
		}
		...
		switch ( $addons ) {
			...
			case 'wl-single-product-rating':
				...
				<?php else : ?>
					<?php echo '<div class="wl-nodata">' . __( 'No Ratting Available','powerpack' ) . '</div>'; ?>
				<?php endif; 
				break;
