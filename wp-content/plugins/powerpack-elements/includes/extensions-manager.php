...
class Extensions_Manager {
	...
	const BACKGROUND_EFFECTS 	= 'background-effects';
	...
	public function register_extensions() {
		...
		foreach ( $available_extensions as $index => $extension_id ) {
			...
			$extension_filename = POWERPACK_ELEMENTS_PATH . "extensions/{$extension_filename}.php";

			require( $extension_filename );

			$class_name = str_replace( '-', '_', $extension_id );

			//$class_name = 'PowerpackElements\Extensions\Extension_' . ucwords( $class_name );
			
			$class_name = __NAMESPACE__ . '\\Extensions\\' . 'Extension_' . $class_name;
	 ...
	 *
	 * @param $extension_id
	 * @return bool|\PowerpackElements\Extension_Base
	 */
	...
	private function require_files() {
		require( POWERPACK_ELEMENTS_PATH . 'base/extension-base.php' );
		require( POWERPACK_ELEMENTS_PATH . 'base/condition.php' );
	}
