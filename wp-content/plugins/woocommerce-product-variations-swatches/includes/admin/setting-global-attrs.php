...
class VIWPVS_WOOCOMMERCE_PRODUCT_VARIATIONS_SWATCHES_Admin_Setting_Global_Attrs {
	...
	public function save_attr() {
		...
		$term_woo_widget = isset( $data_changes['term_woo_widget'] ) ? viwpvs_sanitize_fields( $data_changes['term_woo_widget'] ) : array();
		if ( ! $slug ) {
		...
		$taxonomy_woo_widget                    = $vi_wpvs_settings['taxonomy_woo_widget'] ?? array();
		...
		$taxonomy_woo_widget[ 'pa_' . $slug ]   = $term_woo_widget;
		$args ['taxonomy_woo_widget']           = $taxonomy_woo_widget;
			...
			if ( $attribute_taxonomies ) {
				if ( $keyword ) {
					...
					foreach ( $attribute_taxonomies as $attr ) {
						if ( strlen( strstr( $attr->attribute_name, $keyword ) ) ) {
				...
				$taxonomy_woo_widget       = $this->settings->get_params( 'taxonomy_woo_widget' ) ?: array();
				...
				foreach ( $global_attrs as $attribute ) {
					...
					$attribute_woo_widget   = $taxonomy_woo_widget[ $attribute_name ] ?? array();
					?>
                    <div class="vi-ui styled fluid accordion active vi-wpvs-accordion-wrap vi-wpvs-accordion-attr-wrap vi-wpvs-accordion-attr-wrap-<?php echo esc_attr( $attribute_name ); ?>"
                                                        ...
                                                        <div class="vi-wpvs-attribute-value-content-wrap vi-wpvs-attribute-value-content-image-wrap <?php echo $attribute->attribute_type === 'image' ? '' : esc_attr( 'vi-wpvs-hidden' ); ?>">
                                                            <table class="form-table">
                                                                ...
                                                                <span class="vi-attribute-edit-image-preview vi-attribute-image-preview">
                                                                    <?php
                                                                    echo $terms_img_id ? wp_kses_post( wp_get_attachment_image( $terms_img_id ) ) : wp_kses_post( wc_placeholder_img() );
                                                                    ?>
                                                                </span>
                                                                            <div><?php esc_html_e( 'Upload / Add image', 'woocommerce-product-variations-swatches' ); ?></div>
                                                                        </div>
                                                                        <p class="description">
									...
									if ( $term_custom_settings && is_array( $term_custom_settings ) && $count_rule = count( $term_custom_settings ) ) {
										for ( $i = 0; $i < $count_rule; $i ++ ) {
                                            ...
                                            <tr class="vi-wpvs-term-custom-cats-wrap">
                                                ...
                                                <td>
                                                    <div class="vi-ui toggle checkbox">
                                                        <input type="hidden" name="taxonomy_custom_cats[<?php echo esc_attr( $attribute_name ); ?>][loop_enable][]"
                        ...
                        <div class="title">
                            <i class="dropdown icon"></i>
							<?php esc_html_e( 'Design for Woo Widget', 'woocommerce-product-variations-swatches' ); ?>
                        </div>
                        <div class="content">
                            <div class="field">
								<?php
								$attribute_woo_widget_pd_count_enable = $attribute_woo_widget['pd_count_enable'] ?? '';
								$attribute_woo_widget_display_type    = $attribute_woo_widget['display_type'] ?? '';
								$attribute_woo_widget_profile         = $attribute_woo_widget['profile'] ?? '';
								?>
                                <div class="equal width fields">
                                    <div class="field">
                                        <label><?php esc_html_e( 'Product count enable', 'woocommerce-product-variations-swatches' ); ?></label>
                                        <div class="vi-ui toggle checkbox">
                                            <input type="hidden" name="taxonomy_woo_widget[<?php echo esc_attr( $attribute_name ) ?>][pd_count_enable]"
                                                   class="attribute_woo_widget_pd_count_enable" value="<?php echo esc_attr( $attribute_woo_widget_pd_count_enable ); ?>">
                                            <input type="checkbox" class="attribute_woo_widget_pd_count_enable-checkbox" <?php checked( $attribute_woo_widget_pd_count_enable, 1 ); ?>>
                                        </div>
                                    </div>
                                    <div class="field">
                                        <label><?php esc_html_e( 'Display type', 'woocommerce-product-variations-swatches' ); ?></label>
                                        <select name="taxonomy_woo_widget[<?php echo esc_attr( $attribute_name ) ?>][display_type]" id="attribute_woo_widget_display_type"
                                                class="vi-ui fluid dropdown attribute_woo_widget_display_type">
                                            <option value="button" <?php selected( $attribute_woo_widget_display_type, 'button' ) ?>>
												<?php esc_html_e( 'Button', 'woocommerce-product-variations-swatches' ); ?>
                                            </option>
                                            <option value="color" <?php selected( $attribute_woo_widget_display_type, 'color' ) ?>>
												<?php esc_html_e( 'Color', 'woocommerce-product-variations-swatches' ); ?>
                                            </option>
                                            <option value="image" <?php selected( $attribute_woo_widget_display_type, 'image' ) ?>>
												<?php esc_html_e( 'Image', 'woocommerce-product-variations-swatches' ); ?>
                                            </option>
                                        </select>
                                    </div>
                                    <div class="field">
                                        <label><?php esc_html_e( 'Swatches profile', 'woocommerce-product-variations-swatches' ); ?></label>
                                        <select name="taxonomy_profile"
                                                class="vi-ui fluid dropdown attribute_woo_widget_profile">
											<?php
											foreach ( $vi_wpvs_ids as $k => $id ) {
												?>
                                                <option value="<?php echo esc_attr( $id ) ?>" <?php selected( $attribute_woo_widget_profile,
													$id ) ?>><?php echo esc_html( $vi_wpvs_names[ $k ] ); ?></option>
												<?php
											}
											?>
