...
class VIWPVS_WOOCOMMERCE_PRODUCT_VARIATIONS_SWATCHES_Frontend_Woo_Widget {
	...
	public function __construct() {
		...
		if ( $this->settings->get_params( 'woo_widget_enable' ) ) {
			...
			add_action( 'in_widget_form', array( $this, 'viwpvs_in_widget_form' ), 9999999999, 3 );
		}
	}

	public function init() {
		remove_all_filters( 'woocommerce_layered_nav_term_html' );
		add_filter( 'woocommerce_layered_nav_term_html', array( $this, 'viwpvs_woocommerce_layered_nav_term_html' ), 999999, 4 );
	}

	public function viwpvs_woocommerce_layered_nav_term_html( $term_html, $term, $link, $count ) {
		$term_name = $term->name ?? '';
		...
		if ( ! wp_style_is( 'vi-wpvs-frontend-widget', 'enqueued' ) ) {
			wp_enqueue_style( 'vi-wpvs-frontend-widget' );
		}
		if ( ! wp_script_is( 'vi-wpvs-frontend-widget', 'enqueued' ) ) {
		...
		$taxonomy_woo_widget        = $this->settings->get_params( 'taxonomy_woo_widget' );
		$profile_default            = $this->settings->get_params( 'attribute_profile_default' );
		$profile_ids                = $this->settings->get_params( 'ids' );
		$pd_count                   = $taxonomy_woo_widget[ $taxonomy ]['pd_count_enable'] ?? $this->settings->get_params( 'woo_widget_pd_count_enable' );
		$type                       = $taxonomy_woo_widget[ $taxonomy ]['display_type'] ?? '';
		$type                       = $type ?: 'button';
		$profile                    = $taxonomy_woo_widget[ $taxonomy ]['profile'] ?? '';
				...
				switch ( $type ) {
					case 'color':
                        ...
                        <div class="<?php echo esc_attr( $term_class ); ?>" data-attribute_value="<?php echo esc_attr( $term->slug ); ?>">
                            <span class="vi-wpvs-option vi-wpvs-option-color" data-option_color="<?php echo esc_attr( $term_color ); ?>"></span>
					...
					case 'image':
                        ...
                        <div class="<?php echo esc_attr( $term_class ); ?>" data-attribute_value="<?php echo esc_attr( $term->slug ); ?>">
                            <img src="<?php echo esc_url( $img_url ); ?>"
                                 alt="<?php echo esc_attr( $term->slug ); ?>" class="vi-wpvs-option vi-wpvs-option-image">
                            <div class="vi-wpvs-option-tooltip vi-wpvs-option-tooltip-<?php echo esc_attr( $attribute_tooltip_position ); ?>"
                                 data-attribute_label="<?php echo esc_attr( $term_name ); ?>">
                                    <span>
                                        <?php echo wp_kses_post( $term_tooltip ); ?>
	...
	public function wvps_wp_enqueue_scripts() {
		...
		$css .= $this->add_inline_style(
			array( '.widget_layered_nav .woocommerce-widget-layered-nav-list.vi-wpvs-woocommerce-widget-layered-nav-list .wc-layered-nav-term.vi-wpvs-wc-layered-nav-term' ),
		...
		$css .= $this->add_inline_style(
			array( '.widget_layered_nav .woocommerce-widget-layered-nav-list.vi-wpvs-woocommerce-widget-layered-nav-list .wc-layered-nav-term.vi-wpvs-wc-layered-nav-term.vi-wpvs-wc-layered-nav-term-hover' ),
		...
		$css .= $this->add_inline_style(
			array( '.widget_layered_nav .woocommerce-widget-layered-nav-list.vi-wpvs-woocommerce-widget-layered-nav-list .wc-layered-nav-term.vi-wpvs-wc-layered-nav-term.chosen' ),
			array( 'color', 'bg_color' ),
