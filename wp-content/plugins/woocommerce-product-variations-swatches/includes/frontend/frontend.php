...
class VIWPVS_WOOCOMMERCE_PRODUCT_VARIATIONS_SWATCHES_Frontend_Frontend {
	...
	public function __construct() {
		...
		if ( $this->settings->get_params( 'out_of_stock_variation_disable' ) ) {
			add_filter( 'woocommerce_available_variation', array( $this, 'wvps_woocommerce_available_variation' ), 9999999999, 3 );
		}
		add_action( 'wp_enqueue_scripts', array( $this, 'wvps_wp_enqueue_scripts' ), 99 );
		add_action( 'init', array( $this, 'init' ), 9999999999 );
	}

	public function wvps_woocommerce_available_variation( $result, $object, $variation ) {
		if ( ! $variation->is_in_stock() || ( $variation->managing_stock() && $variation->get_stock_quantity() <= get_option( 'woocommerce_notify_no_stock_amount', 0 ) && 'no' === $variation->get_backorders() ) ) {
			$result = false;
		}
	...
	public function init() {
		...
		add_filter( 'woocommerce_dropdown_variation_attribute_options_html', array( $this, 'variation_attribute_options_html' ), 999999, 2 );
		remove_all_filters( 'woocommerce_ajax_variation_threshold' );
		add_filter( 'woocommerce_ajax_variation_threshold', array( $this, 'viwpvs_ajax_variation_threshold' ), 999999, 2 );
	}

	public function viwpvs_ajax_variation_threshold( $limit, $product ) {
		$settings = new  VIWPVS_WOOCOMMERCE_PRODUCT_VARIATIONS_SWATCHES_DATA();
	...
	public function variation_attribute_options_html( $html, $args ) {
		...
		if ( $is_taxonomy ) {
			...
			$taxonomy_custom_cats = $settings->get_params( 'taxonomy_custom_cats' )[ $attribute ] ?? '';
			...
			if ( ! $vi_attribute_type ) {
				...
				if ( ! in_array( $vi_attribute_type, array( 'button', 'color', 'image', 'variation_img', 'radio' ) ) && ! isset( $settings->get_params( 'taxonomy_profiles' )[ $attribute ] ) ) {
			...
			$custom_attribute_id = $settings->get_params( 'custom_attribute_id' ) ?: array();
			$index               = self::get_custom_cats_index( $attribute, count( $custom_attribute_id ), $custom_attribute_id, $settings->get_params( 'custom_attribute_name' ), $settings->get_params( 'custom_attribute_category' ), $product_cats );
			if ( $index === false ) {
				if ( ! $vi_attribute_type && $settings->get_params( 'attribute_display_default' ) !== 'none' ) {
					$vi_attribute_type = $settings->get_params( 'attribute_display_default' );
				}
			} else {
				$vi_attribute_type         = $vi_attribute_type ?: $settings->get_current_setting( 'custom_attribute_type', $index );
				$vi_attribute_profile      = $vi_attribute_profile ?: $settings->get_current_setting( 'custom_attribute_profiles', $index );
				$vi_attribute_display_type = $vi_attribute_display_type ?: $settings->get_current_setting( 'custom_attribute_display_type', $index );
			}
		}
		...
		if ( empty( $options ) ) {
			...
			if ( $attribute_double_click ) {
				$new_html = '<div class="vi-wpvs-variation-wrap-wrap vi-wpvs-hidden" data-vpvs_double_click="true">';
			} else {
				$new_html = '<div class="vi-wpvs-variation-wrap-wrap vi-wpvs-hidden" data-vpvs_double_click="false">';
			}
			$new_html .= '<div class="vi-wpvs-select-attribute vi-wpvs-select-attribute-attribute_' . esc_attr( $attribute ) . '">';
			$new_html .= $html;
			$new_html .= '</div>';
			$new_html .= self::get_attribute_option_html( $attribute, $product, $options, $vi_attribute_settings, $vi_args, $vi_attribute_type,
				$vi_attribute_profile, $vi_attribute_display_type, $use_taxonomy_type );
			$new_html .= '</div>';
	...
	public static function get_attribute_option_html( $attribute, $product, $options, $vi_attribute_settings, $vi_args, $type, $profile, $display_type, $use_taxonomy_type = '' ) {
		...
		$attribute_display_type     = $display_type ?: 'vertical';
		$type                       = $type ?: 'select';
		...
		$display_type_class         = array(
			...
			'vi-wpvs-variation-style-' . $attribute_display_type
		);
		$display_type_class[]       = is_rtl() ? 'vi-wpvs-variation-style-rtl' : '';
		$display_type_class         = implode( ' ', $display_type_class );
		ob_start();
		?>
        <div class="<?php echo esc_attr( trim( $div_class ) ); ?>"
             data-display_type="<?php echo esc_attr( trim( $display_type_class ) ) ?>"
			...
			if ( taxonomy_exists( $attribute ) ) {
				...
				switch ( $type ) {
					case 'button':
						...
						if ( ! empty( $vi_args['viwpvs_link_more'] ) ) {
                            ...
                            <a href="<?php echo esc_attr( esc_url( $product_link ) ) ?>"
                               class="vi-wpvs-option-wrap vi-wpvs-option-wrap-default vi-wpvs-product-link" data-product_link="<?php echo esc_attr( esc_url( $product_link ) ) ?>">
                                <span class="vi-wpvs-option vi-wpvs-option-button">
						...
						foreach ( $terms as $term ) {
                            ...
                            <div class="<?php echo esc_attr( $term_class ); ?>"
                                ...
                                <span class="vi-wpvs-option vi-wpvs-option-color"
                                      data-option_color="<?php echo esc_attr( $term_color ); ?>"
                                      data-option_separator="<?php echo esc_attr( $term_color_separator ); ?>"></span>
                                <div class="vi-wpvs-option-tooltip vi-wpvs-option-tooltip-<?php echo esc_attr( $attribute_tooltip_position ); ?>"
                                     data-attribute_label="<?php echo esc_attr( $term_name ); ?>">
                                    <span>
                                        <?php echo wp_kses_post( $term_tooltip ); ?>
					...
					case 'image':
						foreach ( $terms as $term ) {
                            ...
                            <div class="<?php echo esc_attr( $term_class ); ?>"
								...
								$img_url      = $terms_img_id ? wp_get_attachment_image_url( $terms_img_id, 'woocommerce_gallery_thumbnail' ) : wc_placeholder_img_src();
                               ...
                               class="vi-wpvs-option-wrap vi-wpvs-option-wrap-default vi-wpvs-product-link" data-product_link="<?php echo esc_attr( esc_url( $product_link ) ) ?>">
                                <label for="<?php echo esc_attr( $option_radio_id ); ?>" class="vi-wpvs-option">
                                    <input type="radio" value=""
                                           class="vi-wpvs-option vi-wpvs-option-radio" id="<?php echo esc_attr( $option_radio_id ); ?>">
									<?php echo wp_kses_post( $link_more_text ); ?>
                                </label>
                                <div class="vi-wpvs-option-tooltip vi-wpvs-option-tooltip-<?php echo esc_attr( $attribute_tooltip_position ); ?>"
                                     data-attribute_label="<?php esc_attr_e( 'See More', 'woocommerce-product-variations-swatches' ); ?>">
					...
					default:
						...
						$option_selected_name = apply_filters( 'woocommerce_variation_option_name', $option_selected, null, $attribute, $product );
						?>
                        <div class="vi-wpvs-variation-wrap-select-wrap">
                            <div class="vi-wpvs-variation-button-select">
                        <span>
                            <?php
                            echo $option_selected ? esc_html( $option_selected_name ) : esc_html( $show_option_none_text );
                            ?>
                        </span>
                            </div>
                            <div class="vi-wpvs-variation-wrap-option vi-wpvs-select-hidden">
								...
								foreach ( $terms as $term ) {
									...
									$term_class   = $option_selected === $term->slug ? 'vi-wpvs-option-wrap vi-wpvs-option-wrap-selected' : 'vi-wpvs-option-wrap vi-wpvs-option-wrap-default';
									?>
                                    <div class="<?php echo esc_attr( $term_class ); ?>" data-attribute_value="<?php echo esc_attr( $term->slug ); ?>">
                                        <span class="vi-wpvs-option vi-wpvs-option-select">
                                            <?php echo wp_kses_post( $term_tooltip ); ?>
                                        </span>
                                    </div>
								...
								if ( ! empty( $vi_args['viwpvs_link_more'] ) ) {
									...
									$product_link   = $product->get_permalink();
									?>
                                    <a href="<?php echo esc_attr( esc_url( $product_link ) ) ?>" class="vi-wpvs-option-wrap vi-wpvs-option-wrap-default vi-wpvs-product-link"
                                       data-product_link="<?php echo esc_attr( esc_url( $product_link ) ) ?>" title="<?php esc_attr_e( 'See More', 'woocommerce-product-variations-swatches' ); ?>">
				...
				switch ( $type ) {
					case 'button':
						foreach ( $options as $k => $option ) {
                            ...
                            <div class="<?php echo esc_attr( $option_class ); ?>"
                                 ...
                                 data-attribute_label="<?php echo esc_attr( $option ); ?>">
                                <span class="vi-wpvs-option vi-wpvs-option-button">
					...
					case 'color':
						foreach ( $options as $k => $option ) {
                            ...
                            <div class="<?php echo esc_attr( $option_class ); ?>"
								...
								?>
                                <span class="vi-wpvs-option vi-wpvs-option-color" data-option_color="<?php echo esc_attr( $option_color ); ?>"></span>
                                <div class="vi-wpvs-option-tooltip vi-wpvs-option-tooltip-<?php echo esc_attr( $attribute_tooltip_position ); ?>"
                                     data-attribute_label="<?php echo esc_attr( $option ); ?>">
                                    <span>
                                        <?php echo wp_kses_post( $option_tooltip ); ?>
					...
					case 'image':
						foreach ( $options as $k => $option ) {
                            ...
                            <div class="<?php echo esc_attr( $option_class ); ?>"
								...
								$img_url      = $option_img ? wp_get_attachment_image_url( $option_img, 'woocommerce_gallery_thumbnail' ) : wc_placeholder_img_src();
					...
					default:
                        ...
                        <div class="vi-wpvs-variation-wrap-select-wrap">
                            <div class="vi-wpvs-variation-button-select">
                                <span>
                                    <?php echo $option_selected ? esc_html( $option_selected_name ) : esc_html( $show_option_none_text ); ?>
                                </span>
                            </div>
                            <div class="vi-wpvs-variation-wrap-option vi-wpvs-select-hidden">
								<?php
		...
		if ( $ids && is_array( $ids ) && $count_ids = count( $ids ) ) {
			...
			for ( $i = 0; $i < $count_ids; $i ++ ) {
				...
				$css .= $this->add_inline_style(
					...
					$i,
					array( 'attribute_height', 'attribute_width', 'attribute_padding', 'attribute_fontsize', 'attribute_default_border_radius' ),
				...
				if ( $reduce_loop ) {
					$css .= $this->add_inline_style_reduce(
						'.vi-wpvs-variation-wrap-loop.vi-wpvs-variation-wrap.vi-wpvs-variation-wrap-' . $id . ' .vi-wpvs-option-wrap',
						$i,
					...
					if ( ! $attribute_width ) {
						$attribute_width_t = $attribute_height ? ( $attribute_height * $reduce_loop / 100 ) : 48 * $reduce_loop / 100;
						$css               .= 'width: ' . $attribute_width_t . 'px !important;';
					}
					if ( ! $attribute_height ) {
						$attribute_height_t = $attribute_width ? ( $attribute_width * $reduce_loop / 100 ) : 48 * $reduce_loop / 100;
						$css                .= 'height:' . $attribute_height_t . 'px !important;';
					}
					$css .= '}';
					$css .= $this->add_inline_style_reduce(
