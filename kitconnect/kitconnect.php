...
//        }
    ...
    public function getRealPath($path) {
        return NC_STORE_BASE_DIR . ltrim($path, '/');
    }
...
class NC_Cart_Magento extends NC_Cart {
    ...
    public function loadConfig() {
        if (file_exists(NC_STORE_BASE_DIR . 'app/etc/local.xml') || file_exists(NC_STORE_BASE_DIR . '../app/etc/local.xml')) {
            ...
            preg_match("/\<host\>\<\!\[CDATA\[(.+)\]\]>\<\/host\>/", $dbconfig, $match);
            $this->host = $match[1];
            preg_match("/\<username\>\<\!\[CDATA\[(.+)\]\]>\<\/username\>/", $dbconfig, $match);
            $this->username = $match[1];
            preg_match("/\<password\>\<\!\[CDATA\[(.*)\]\]>\<\/password\>/", $dbconfig, $match);
            $this->password = $match[1];
            preg_match("/\<dbname\>\<\!\[CDATA\[(.+)\]\]>\<\/dbname\>/", $dbconfig, $match);
            $this->database = $match[1];
            if ($this->check) {
                preg_match("/\<table_prefix\>\<\!\[CDATA\[(.*)\]\]>\<\/table_prefix\>/", $dbconfig, $match);
                $this->tablePrefix = $match[1];
                ...
                } elseif (file_exists(NC_STORE_BASE_DIR . '../app/Mage.php')) {
                    $ver = file_get_contents(NC_STORE_BASE_DIR . '../app/Mage.php');
                    if (preg_match("/getVersionInfo[^}]+\'major\' *=> *\'(\d+)\'[^}]+\'minor\' *=> *\'(\d+)\'[^}]+\'revision\' *=> *\'(\d+)\'[^}]+\'patch\' *=> *\'(\d+)\'[^}]+}/s", $ver, $match) == 1) {
                        $mageVersion = $match[1] . '.' . $match[2] . '.' . $match[3] . '.' . $match[4];
                        $this->version = $mageVersion;
            ...
            if ($this->check) {
                ...
                } elseif (file_exists(NC_STORE_BASE_DIR . '../composer.json')) {
                    $ver = file_get_contents(NC_STORE_BASE_DIR . '../composer.json');
                    if (preg_match("/\"version\": \"(.*)\",/", $ver, $match) == 1) {
                        $this->version = $match[1];
                        if ($this->isMagentoEE()) {
